...
UNION
select CONCAT(no_coa,oth_doc) reff_doc, sum(credit - debit) ded, tgl_journal, sum(credit - debit) ded_idr from (select no_journal, tgl_journal, no_coa, nama_coa,curr, rate, debit, credit from tbl_list_journal where type_journal = 'Settlement' and nama_coa like '%UANG MUKA PEMBELIAN%') a INNER JOIN (select no_pci, oth_doc from (select no_pci, oth_doc from c_petty_cashin_h where reff = 'Settlement' and status != 'Cancel' GROUP BY no_pci, oth_doc
UNION
...
LEFT JOIN (select reff_doc, if(curr = 'USD',sum((credit * rate) - (debit * rate)),(credit - debit)) tot_gm_bfr from tbl_list_journal where nama_coa like '%uang muka pembelian%' and no_journal like '%GM/%' and (reff_doc not like '%BK%' and reff_doc not like '%KK%') and tgl_journal < '$start_date' GROUP BY reff_doc
    UNION
    select CONCAT(no_coa,reff_doc) reff_doc, if(curr = 'USD',sum((credit * rate) - (debit * rate)),(credit - debit)) tot_gm_bfr from tbl_list_journal where nama_coa like '%uang muka pembelian%' and (no_journal like '%GM/%' or no_journal like '%BM/%') and (reff_doc like '%BK%' OR reff_doc like '%KK%') and tgl_journal < '$start_date' GROUP BY reff_doc
) d on d.reff_doc = a.no_ftr
LEFT JOIN (select reff_doc, if(curr = 'USD',sum((credit * rate) - (debit * rate)),(credit - debit)) tot_gm from tbl_list_journal where nama_coa like '%uang muka pembelian%' and no_journal like '%GM/%' and (reff_doc not like '%BK%' and reff_doc not like '%KK%') and tgl_journal BETWEEN '$start_date' and '$end_date' GROUP BY reff_doc
    UNION
    select CONCAT(no_coa,reff_doc) reff_doc, if(curr = 'USD',sum((credit * rate) - (debit * rate)),(credit - debit)) tot_gm from tbl_list_journal where nama_coa like '%uang muka pembelian%' and (no_journal like '%GM/%' or no_journal like '%BM/%') and (reff_doc like '%BK%' OR reff_doc like '%KK%') and tgl_journal BETWEEN '$start_date' and '$end_date' GROUP BY reff_doc) e on e.reff_doc = a.no_ftr) a where (saldoawal_ + addition_) > 0 and no_bankout not in ('BK/BCA1979/NAG/1024/00282')");
}else{
    $sql = mysqli_query($conn2,"SELECT *, if(ded = 0 OR ((addition_ + saldoawal_) - ded) > 0,0,((saldo_awal + addition) - ded - ded_gm)) forex, (saldo_awal + addition - ded - ded_gm - if(ded = 0 OR ((addition_ + saldoawal_) - ded) > 0,0,((saldo_awal + addition) - ded - ded_gm))) saldo_akhir from (select no_coa, no_ftr, tgl_ftr, no_pi, a.no_po, tgl_po, supp, no_bankout, tgl_bankout,(saldo_awal - COALESCE(ded_bfr,0) - COALESCE(tot_gm_bfr,0)) saldoawal_, addition addition_, (saldo_awal_idr - COALESCE(ded_bfr_idr,0) - COALESCE(tot_gm_bfr,0)) saldo_awal, addition_idr addition, COALESCE(ded_idr,0) ded, COALESCE(ded,0) ded_,  COALESCE(tot_gm,0) ded_gm, deskripsi from (select * from (select CASE
                    WHEN reff_doc like '%CBD%' OR reff_doc like '%DP%' THEN
                    no_ftr_cbd
