...
$sql = mysqli_query($conn1,"select a.no_coa, CONCAT(a.no_coa,' ',nama_coa) coa, no_costcntr, cc_name, buyer,no_ws,curr,IF(debit > 0,round(debit,0),'') credit, IF(credit > 0,round(credit,0),'') debit, curr, upper(deskripsi) deskripsi from c_petty_cashout_none a inner join mastercoa_v2 b on b.no_coa = a.no_coa LEFT JOIN b_master_cc c on c.no_cc = a.no_costcntr where no_pco = '$no_pco' GROUP BY a.id");
