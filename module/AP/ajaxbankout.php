...
if ($refdoc == 'Payment Voucher' || $refdoc == 'List Payment') {
    $sql = mysqli_query($conn1,"select no_coa id_coa,nama_coa coa_name,reff_doc,IF(reff_date = '0000-00-00','-',reff_date) reff_date,nama_costcenter cost_center,buyer,no_ws,curr,debit,credit, (debit * rate) debit_idr, (credit * rate) credit_idr,keterangan from tbl_list_journal where no_journal = '$no_ob'");
...
if($refdoc == 'None'){
    ...
    <th style="width:50px;">Debit IDR</th>  
    <th style="width:50px;">Credit IDR</th> 
    ...
    while ($row = mysqli_fetch_assoc($sql)) {
        ...
        $total_debit_idr += $row['debit_idr'];
        $total_credit_idr += $row['credit_idr'];
        $table .= '<tr style="text-align:left;">
        ...
        <td style="width:50px;text-align: right;" value="'.$row['debit_idr'].'">'.number_format($row['debit_idr'],2).'</td> 
        <td style="width:50px;text-align: right;" value="'.$row['credit_idr'].'">'.number_format($row['credit_idr'],2).'</td>
        <td style="width:100px;" value="'.$row['keterangan'].'">'.$row['keterangan'].'</td>
        </tr>
        ';
        $table .= '</tbody>';
    ...
    <th style="width:50px;text-align: right;" value="'.$total_debit.'">'.number_format($total_debit,2).'</th> 
    <th style="width:50px;text-align: right;" value="'.$total_credit.'">'.number_format($total_credit,2).'</th>
    <th style="width:50px;text-align: right;" value="'.$total_debit_idr.'">'.number_format($total_debit_idr,2).'</th> 
    <th style="width:50px;text-align: right;" value="'.$total_credit_idr.'">'.number_format($total_credit_idr,2).'</th>
    <th></th>
    </tr>
    ';
    $table .= '</table>';
