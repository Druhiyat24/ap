...
include 'query_bpb2.php'; // sets $sql, $where, $start_date, $end_date
...
/* ── footer totals ── */
$cols_sum = "SUM(saldo_awal) saldo_awal, SUM(in_bpb) in_bpb, SUM(reverse_bpb) reverse_bpb,
    SUM(ded_kontrabon) ded_kontrabon, SUM(reverse_kontrabon) reverse_kontrabon, SUM(gm) gm,
...
$cols_all = "SUM(saldo_awal*rate) saldo_awal, SUM(in_bpb*rate) in_bpb,
    SUM(reverse_bpb*rate) reverse_bpb, SUM(ded_kontrabon*rate) ded_kontrabon,
    SUM(reverse_kontrabon*rate) reverse_kontrabon, SUM(gm*rate) gm,
    SUM(saldo_akhir*rate) saldo_akhir, SUM(saldo_akhir_idr) saldo_akhir_idr,
    SUM(due_current) due_current, SUM(due_1_30) due_1_30, SUM(due_31_60) due_31_60,
    SUM(due_61_90) due_61_90, SUM(due_91_120) due_91_120, SUM(due_121_180) due_121_180,
    SUM(due_181_360) due_181_360, SUM(due_gt_360) due_gt_360, SUM(total_due) total_due,
    SUM(pro_due) pro_due, SUM(pro_due0) pro_due0, SUM(pro_due1) pro_due1,
...
$q_idr = mysqli_query($conn2, "SELECT $cols_sum FROM ($sql) x WHERE curr = 'IDR'");
$q_usd = mysqli_query($conn2, "SELECT $cols_sum FROM ($sql) x WHERE curr != 'IDR'");
$q_all = mysqli_query($conn2, "SELECT $cols_all FROM ($sql) x");
