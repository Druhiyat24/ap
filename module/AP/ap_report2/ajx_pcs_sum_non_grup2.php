...
// Combined raw data (both GROUP and NON GROUP)
$combined = "
SELECT item_type2, curr, relasi, saldo_akhir, saldo_akhir_idr,
...
// Main: NON GROUP only, grouped by item_type2 + curr
$sql = "
...
FROM ($combined) c
WHERE relasi = 'NON GROUP'
GROUP BY item_type2, curr
ORDER BY item_type2, curr ASC
";
...
// Footer totals for NON GROUP only
...
// Summary totals across ALL relasi (GROUP + NON GROUP)
$sql_all = "
SELECT curr,
    SUM(IF(UPPER(curr) != 'IDR', saldo_akhir, 0)) saldo_akhir,
...
$q_sum_idr = mysqli_query($conn2, "SELECT $footer_cols FROM ($sql_all) x WHERE curr = 'IDR'");
$q_sum_usd = mysqli_query($conn2, "SELECT $footer_cols FROM ($sql_all) x WHERE curr != 'IDR'");
$q_sum_all = mysqli_query($conn2, "SELECT $footer_cols FROM ($sql_all) x");
...
echo json_encode([
    ...
    "footer_sum_idr"    => mysqli_fetch_assoc($q_sum_idr),
    "footer_sum_usd"    => mysqli_fetch_assoc($q_sum_usd),
    "footer_sum_all"    => mysqli_fetch_assoc($q_sum_all),
]);
