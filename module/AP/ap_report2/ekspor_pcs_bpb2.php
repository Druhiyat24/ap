...
include 'query_bpb2.php'; // sets $sql, $start_date, $end_date
...
?>
<html>
<head><title>AP Report - BPB</title></head>
<body>
<style>
...
<h4>Payable Card Statement - BPB<br>PERIODE: <?= $label_start ?> - <?= $label_end ?></h4>
...
<tr>
    ...
    <th rowspan="2" class="h-default">BPB Number</th>
    <th rowspan="2" class="h-default">BPB Date</th>
    ...
    <th rowspan="2" class="h-default">Reverse BPB</th>
    <th rowspan="2" class="h-default">Deduction KB</th>
    <th rowspan="2" class="h-default">Reverse KB</th>
...
<tr style="font-size:10px;">
    ...
    <td><?= htmlspecialchars($row['no_bpb']) ?></td>
    <td><?= date("d-M-Y", strtotime($row['tgl_bpb'])) ?></td>
    <td><?= date("d-M-Y", strtotime($row['duedate'])) ?></td>
    <td><?= htmlspecialchars($row['curr']) ?></td>
    <td class="num"><?= $nf($row['saldo_awal']) ?></td>
    <td class="num"><?= $nf($row['in_bpb']) ?></td>
    <td class="num"><?= $nf($row['reverse_bpb']) ?></td>
    <td class="num"><?= $nf($row['ded_kontrabon']) ?></td>
    <td class="num"><?= $nf($row['reverse_kontrabon']) ?></td>
    <td class="num"><?= $nf($row['gm']) ?></td>
