...
/*
 * POTONGAN PPN (CTE ppn / ppn_before) -> DIGABUNG KE "Deduction Others"
 ...
 *
 * Atas permintaan user TIDAK dibuat kolom baru: nilainya ditambahkan ke kolom
 * `potongan` (Deduction Others) di CTE mutasi, sehingga semua tab & ekspor yang
 * sudah menampilkan Deduction Others ikut benar tanpa perubahan tampilan.
 * Nilainya BERTANDA: negatif = memotong utang (34 PV), positif = menambah utang
 * (6 PV, PPN tertagih yang tidak ada di GR/IR).
 *
...
UNION
select a.nama_supp, b.no_kbon_det, DATE_FORMAT(a.create_date,'%Y-%m-%d') tgl_kbon, b.tgl_tempo, a.curr, (b.dpp + b.ppn) total, IFNULL(d.rate,1) rate, a.no_coa, a.nama_coa, item_type1, item_type2, relasi from kontrabon_h a INNER JOIN kontrabon_h_installment_detail b on b.no_kbon = a.no_kbon INNER JOIN mastercoa_v2 c on c.no_coa = a.no_coa left join rate d on d.tanggal = DATE_FORMAT(a.create_date,'%Y-%m-%d') and d.curr = a.curr where DATE_FORMAT(a.create_date,'%Y-%m-%d') > '2026-06-30' and DATE_FORMAT(a.create_date,'%Y-%m-%d') BETWEEN '$start_date' and '$end_date' 
),
...
UNION
select a.nama_supp, b.no_kbon_det, DATE_FORMAT(a.create_date,'%Y-%m-%d') tgl_kbon, b.tgl_tempo, a.curr, (b.dpp + b.ppn) total, IFNULL(d.rate,1) rate, a.no_coa, a.nama_coa, item_type1, item_type2, relasi from kontrabon_h a INNER JOIN kontrabon_h_installment_detail b on b.no_kbon = a.no_kbon INNER JOIN mastercoa_v2 c on c.no_coa = a.no_coa left join rate d on d.tanggal = DATE_FORMAT(a.create_date,'%Y-%m-%d') and d.curr = a.curr where DATE_FORMAT(a.create_date,'%Y-%m-%d') > '2026-06-30' and DATE_FORMAT(a.create_date,'%Y-%m-%d') < '$start_date'
),
...
ded_bank_before as (select reff_doc, curr, sum(debit-credit) total from tbl_list_journal where tgl_journal > '2026-06-30' and tgl_journal < '$start_date' and type_journal like '%Payment Voucher%' and no_journal like '%BK%' and nama_coa like '%UTANG USAHA%' GROUP BY reff_doc),

ded_cash as (select reff_doc, curr, sum(debit-credit) total from tbl_list_journal where tgl_journal > '2026-06-30' and tgl_journal between '$start_date' and '$end_date' and type_journal like '%Payment Voucher%' and no_journal like '%KKK%' and nama_coa like '%UTANG USAHA%' GROUP BY reff_doc
),

ded_cash_before as (select reff_doc, curr, sum(debit-credit) total from tbl_list_journal where tgl_journal > '2026-06-30' and tgl_journal < '$start_date' and type_journal like '%Payment Voucher%' and no_journal like '%KKK%' and nama_coa like '%UTANG USAHA%' GROUP BY reff_doc),

ded_nonbank as (select reff_doc, curr, sum(debit-credit) total from tbl_list_journal where tgl_journal > '2026-06-30' and tgl_journal between '$start_date' and '$end_date' and type_journal like '%Payment Non Bank%' and no_journal like '%PAY%' and nama_coa like '%UTANG USAHA%' GROUP BY reff_doc
),

ded_nonbank_before as (select reff_doc, curr, sum(debit-credit) total from tbl_list_journal where tgl_journal > '2026-06-30' and tgl_journal < '$start_date' and type_journal like '%Payment Non Bank%' and no_journal like '%PAY%' and nama_coa like '%UTANG USAHA%' GROUP BY reff_doc),
...
mutasi as (select supplier, no_kbon, tgl_kbon, duedate, curr, (saldo_awal + reverse_kontrabon_before + uang_muka_before + potongan_before + ppn_before - (if(ded_bank_before > 0,(ded_bank_before),0) + ded_gm_before + if(ded_cash_before > 0,(ded_cash_before),0) + if(ded_nonbank_before > 0,(ded_nonbank_before),0))) saldo_awal, total_in, pph, uang_muka, (potongan + ppn) potongan, if(ded_bank > 0,(ded_bank),0) ded_bank, if(ded_cash > 0,(ded_cash),0) ded_cash, if(ded_nonbank > 0,(ded_nonbank),0) ded_nonbank, ded_gm, reverse_kontrabon, rate, no_coa, nama_coa, item_type1, item_type2, relasi from data_detail),
