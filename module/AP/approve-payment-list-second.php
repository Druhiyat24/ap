...
<div class="container-fluid mt-4 p-4">
    ...
    <div class="card shadow border-0">
        ...
        <h5 class="mb-0"><i class="fa fa-thumbs-up" aria-hidden="true"></i> SECOND APPROVAL - PAYMENT LIST</h5>
    </div>
    </div>
...
<script type="text/javascript">
    let approveTable = $('#table-approve-pl').DataTable({
        ...
        ajax: {
            url: 'ajx_approve-payment-list.php',
            type: 'POST',
            data: function (d) {
                d.stage = 'second';
            }
    ...
    // Klik baris (kolom No Payment List) untuk lihat detail PV di dalamnya
    ...
    // Bulk Second Approve - semua PL terpilih dikirim ke second_approve_payment_list.php
    $('#btnApproveSelected').on('click', function () {
        ...
        Swal.fire({
            title: 'Approve ' + plNumbers.length + ' Payment List?',
            icon: 'question',
            ...
            const requests = plNumbers.map(pl_number => $.ajax({
                type: 'POST',
                url: 'second_approve_payment_list.php',
                data: { pl_number: pl_number }
            }));
            ...
            Promise.allSettled(requests).then(results => {
                ...
                let msg = success + ' Payment List Berhasil Di Approve';
                if (errors.length > 0) {
    ...
    // Bulk Cancel - semua PL terpilih dikirim ke cancel_payment_list.php
    $('#btnCancelSelected').on('click', function () {
        ...
        Swal.fire({
            title: 'Cancel ' + plNumbers.length + ' Payment List?',
            ...
            Promise.allSettled(requests).then(results => {
                ...
                let msg = success + ' Payment List Berhasil Di Cancel';
                if (errors.length > 0) {
                    msg += '<br><br><b>Gagal:</b><br>' + errors.join('<br>');
                }
