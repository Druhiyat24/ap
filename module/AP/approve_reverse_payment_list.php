...
while ($rowDet = mysqli_fetch_assoc($sqlDet)) {
    $pl_number_esc = mysqli_real_escape_string($conn2, $rowDet['doc_number']);

    mysqli_query($conn2, "UPDATE pv_payment_list_h SET status='Draft', first_approve_user=NULL, first_approve_date=NULL, second_approve_user=NULL, second_approve_date=NULL WHERE pl_number='$pl_number_esc'");

    $sqlPvDet = mysqli_query($conn2, "SELECT type_pv, no_kbon FROM pv_payment_list_det WHERE pl_number='$pl_number_esc' AND status != 'Cancel'");
    while ($rowPv = mysqli_fetch_assoc($sqlPvDet)) {
        updateStatusPl($conn2, $rowPv['type_pv'], $rowPv['no_kbon'], null);
    }
}
