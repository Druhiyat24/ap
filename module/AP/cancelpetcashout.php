...
try {
    ...
    $sqlChk = dbExec($conn2, "SELECT status FROM c_petty_cashout_h WHERE no_pco = '$no_pci' LIMIT 1 FOR UPDATE");
    ...
    dbExec($conn2, "UPDATE c_petty_cashout_h SET cancel_by='$cancel_user', cancel_date='$cancel_date', status='$status' WHERE no_pco='$no_pci'");
