...
<div class="box body">
    <div class="row">
        ...
        <div class="col-md-12">
            ...
            if ($ref_num == 'Payment') {
                $sql = mysqli_query($conn2,"select no_kbon,curr,nama_supp,no_payment,tgl_payment,tgl_tempo,curr,(sum(amount) - (select COALESCE(sum(for_balance),0) from b_bankout_det where no_reff = no_payment)) as total_kbon,sum(pph_value) as pph_value, 'REG' as kode from list_payment where status = 'Closed' and nama_supp = '$nama_supp' and tgl_payment BETWEEN '$start_date' and '$end_date'  group by no_payment
                    union
...
<script type="text/javascript">
   ...
   function addRow(tableID) {
    ...
    <td><input style="font-size: 12px;width: 150px;" type="text" class="form-control" name="keterangan[]" placeholder="" autocomplete="off"></td>
    <td>
    <select class="form-control select2add" name="currenc" data-live-search="true">
    <option value="IDR">IDR</option>
    <option value="USD">USD</option>
    </select>
    </td>
    <td><input style="text-align: right; font-size: 12px;width: 150px;" type="number" min="0" class="form-control" id="txt_amount" name="txt_amount" oninput="input_debit(value)" autocomplete="off"></td>
    <td><input style="text-align: right; font-size: 12px;width: 150px;" type="number" min="1" class="form-control" id="txt_credit" name="txt_credit" oninput="input_credit(value)" autocomplete="off"></td>
    <td><input style="font-size: 12px;width: 150px;" type="text" class="form-control" name="keterangan[]" placeholder="" autocomplete="off"></td>
    <td><input name="chk_a[]" type="checkbox" class="checkall_a" value=""></td>
    </tr>
    `;
                        ...
                        $("input[type=checkbox]:checked").each(function () {
                            ...
                            if(amount != '' && nama_supp != '' && akun != '' && balance == '0' && debit != '' || amount != '' && nama_supp != '' && akun != '' && balance == '0' && credit != ''){   
                                $.ajax({
                                    ...
                                    data: {'no_bankout':no_bankout, 'tgl_bankout':tgl_bankout, 'reff_doc':reff_doc, 'no_coa':no_coa, 'no_costcntr':no_costcntr, 'buyer':buyer, 'no_ws':no_ws, 'curr':curr, 'debit':debit, 'credit':credit, 'deskripsi':deskripsi, 'rate':rate, 'prof_ctr':prof_ctr},
                                    cache: 'false',
                                    close: function(e){
                                        e.preventDefault();
                                    },
        ...
        $.ajax({
            ...
            success: function(response){
                $("input[type=checkbox]:checked").each(function () {
                    ...
                    var bankout_date = document.getElementById('tgl_active').value;
                    var no_coa = $(this).closest('tr').find('td:eq(1)').find('select[name=nomor_coa] option').filter(':selected').val(); 
                    var prof_ctr = $(this).closest('tr').find('td:eq(2)').find('select[id=nomor_profit] option').filter(':selected').val();
                    var no_coc = $(this).closest('tr').find('td:eq(3)').find('select[id=nomor_coc] option').filter(':selected').val();    
                    var reff_doc = $(this).closest('tr').find('td:eq(4) input').val();
