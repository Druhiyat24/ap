...
<!-- MAIN -->
<div class="col p-4">
    <h2 class="text-center">FORM PAYMENT VOUCHER FTR</h2>
    <div class="box">
        <div class="box header">
            <form id="form-data" method="post">
                <div style="padding-left: 10px;padding-top: 5px;">
                    ...
                    <button style="-ms-transform: skew(10deg);-webkit-transform: skew(10deg);transform: skew(15deg);" id="btnpve" type="button" class="btn-secondary btn-xs"><span>Payment Voucher EXIM</span></button>
                    <button style="-ms-transform: skew(10deg);-webkit-transform: skew(10deg);transform: skew(15deg);" id="btnpvftr" type="button" class="btn-primary btn-xs"><span></span>Payment Voucher FTR</button>
                </div>
                <div class="form-row">
...
<!--               <select class="form-control selectpicker" name="nama_supp" id="nama_supp" data-dropup-auto="false" data-live-search="true">
            ...
            $sql = mysqli_query($conn2,"select DISTINCT supplier from tbl_pv_ftr_temp where user = '$user' GROUP BY no_memo limit 1");
            $row = mysqli_fetch_array($sql);
    ...
    <div class="col-md-4 mb-3">            
        ...
        <div class="col-md-3 mb-3" style="padding-top: 8px;">
          ...
          inner join mastersupplier mb on a.id_buyer = mb.id_supplier
                    ...
                    inner join tbl_pv_ftr_temp mtemp on mtemp.no_memo = a.nm_memo
        ...
        <div class="col-md-2 mb-3" style="padding-top: 8px;">
            ...
            <input type="text" readonly style="font-size: 14px;" class="form-control" id="forpay" name="forpay" value="Uang Muka (DP)">           
        </div>
        ...
        <div class="col-md-1 mb-3"> 
            ...
            <input type="text" readonly style="font-size: 14px;" class="form-control" id="curre" name="curre" value="<?php 
            $sql = mysqli_query($conn2,"select DISTINCT curr from tbl_pv_ftr_temp a INNER JOIN list_payment_cbd b on b.no_payment = a.no_memo where user = '$user' limit 1");
            $row = mysqli_fetch_array($sql);
            $curre = isset($row['curr']) ? $row['curr'] : null;           
            if(!empty($curre)) {
                echo $curre;
            }
            else{
        ...
        <div class="col-md-1 mb-3">            
            <label for="total" class="col-form-label" style="width: 200px;"><b>Select No FTR</b></label>
            <input style="border: 0;
            line-height: 1;
            padding: 10px 10px;
            font-size: 1rem;
    ...
    <div class="modal fade" id="mymodal2" tabindex="-1" role="dialog" aria-labelledby="edit" aria-hidden="true">
        <div class="modal-dialog">
            <div class="modal-content">
                <div class="modal-header">
                    <button type="button" class="close" data-dismiss="modal" aria-hidden="true"><span class="fa fa-times"></span></button>
                    <h4 class="modal-title" id="Heading">Choose FTR</h4>
                </div>
                <div class="modal-body">
                  <div class="form-group">
            ...
            <div class="modal-footer">
                <div class="col-md-2 mb-3">
                    <button type="submit" id="saveftr" name="saveftr" class="btn btn-warning btn-lg" style="width: 100%;"><span class="fa fa-floppy-o" aria-hidden="true"></span>
                        Save
                    </button>
                </div>
                <div class="col-md-10 mb-3">
        ...
        <div class="col-md-12">
            ...
            <table id="mytable" class="table table-striped table-bordered" cellspacing="0" width="100%" style="font-size: 12px;text-align:center;">
                ...
                <tbody id="tbody2">
                    ...
                    $sqlpv = mysql_query("select * from tbl_pv_ftr_temp where user = '$user'",$conn1);
                    ...
                    while($row = mysql_fetch_array($sqlpv)){
                        ...
                        <td>
                        <select style="font-size: 12px;" class="form-control selectpicker" name="nomor_coa" id="nomor_coa" data-width="150px" data-live-search="true" data-size="5"><option value="-" > - </option>';  $sql = mysqli_query($conn1,"select no_coa as id_coa,concat(no_coa,' ', nama_coa) as coa from mastercoa_v2 where nama_coa like '%uang muka pembelian%'order by no_coa asc"); foreach ($sql as $cc) : echo'<option value="'.$cc["id_coa"].'"> '.$cc["coa"].' </option>'; endforeach; ?>
                        <?php
                        echo '
                        </select>
                        </td>
                        ...
                        <td >
                        <input style="font-size: 12px;" type="text" class="form-control" name="keterangan[]" placeholder="" value="'.$row['no_memo'].'" autocomplete="off" >
                        </td>
                        <td>
                        <input type="text" style="font-size: 15px;" name="tgl_active" id="tgl_active" class="form-control tanggal"
                        value="'.$reff_date.'" autocomplete="off" placeholder="dd-mm-yyyy">
                        </td>
                        <td>
...
<!--             <div class="input-group" >
    ...
    <div class="input-group" >
        ...
        $sql = mysqli_query($conn2," select sum(biaya) biaya from tbl_pv_ftr_temp where user = '$user' ");
        $row = mysqli_fetch_array($sql);
        $biaya = $row['biaya'];           
        if(!empty($biaya)) {
...
<script type="text/javascript">
    $("#modal-form5").on("click", "#send5", function(){
        ...
        <script>
            $("#modal-form2").on("click", "#saveftr", function(){
                $("input[type=checkbox]:checked").each(function () {
