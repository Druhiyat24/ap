...
<div class="col p-4">
    ...
    <div class="box">
        <div class="box header">
            <form id="form-data" method="post">
                <div class="form-row">
                    <div class="col-md-3 mb-3">            
                        ...
                        <input type="text" readonly style="font-size: 12px;" class="form-control-plaintext" id="no_urut" name="no_urut" value="'.sprintf("%05s", $urutan).'" hidden>
                        <input type="text" readonly style="font-size: 12px;" class="form-control-plaintext" id="bulan" name="bulan" value="'.$bln.'" hidden>
                        <input type="text" readonly style="font-size: 12px;" class="form-control-plaintext" id="tahun" name="tahun" value="'.$thn.'" hidden>';
                        ?>
                    </div>

                    <div class="col-md-3 mb-3">            
                        <label for="total" class="col-form-label" style="width: 150px;"><b>Date</b></label>
                        <input type="text" style="font-size: 12px;" name="tgl_active" id="tgl_active" class="form-control tanggal" 
            ...
            }elseif ($ref == 'None' || $ref == 'Settlement') {
                echo '
                </div>
                </br>
                <div class="form-row">
...
<div class="box body">
    <div class="row">
        ...
        if ($ref_num == 'AR Collection') {
            echo '';
        }elseif($ref_num == 'None' || $ref_num == 'Settlement'){
            ...
            while($row = mysqli_fetch_array($sql)){
                ...
                if(!empty($no_reff)) {
                    ...
                    echo '<tr>
                    <td id="select" style="width:10px;"><input type="checkbox" id="select" name="select[]" value="" checked disabled></td>                      
...
function updateCostCenter(profCtr, noCoa, row) {
   ...
   function addRow(tableID) {
    ...
    <td><input type="number" min="1" class="form-control" name="txt_amount[]" style="text-align: right; font-size: 12px;width: 150px;" oninput="modal_input_amt(value)" autocomplete="off"></td>
    <td><input type="number" min="1" class="form-control" name="txt_credit[]" style="text-align: right; font-size: 12px;width: 150px;" oninput="modal_input_cre(value)" autocomplete="off"></td>
    <td><input type="text" class="form-control" name="keterangan[]" style="font-size: 12px;width: 150px;" autocomplete="off"></td>
...
<script type="text/javascript">
    $("input[name=txt_credit").keyup(function(){
        ...
        $("input[type=checkbox]:checked").each(function () {        
            var amount2 = parseFloat($(this).closest('tr').find('td:eq(7) input').val(),10) || 0;
            sum_amount2 += amount2;
...
<script type="text/javascript">
    $("input[name=nominal_h1]").keyup(function(){
        ...
        $("input[type=text]").each(function () {         
            ...
            if (reff_doc == 'None' || reff_doc == 'Settlement') {
                var credit = 0;
...
<script type="text/javascript">
    $("#form-simpan").on("click", "#simpan", function(){
        ...
        if (refer == 'None' || refer == 'Settlement') {
            ...
            var amount = document.getElementById('nominal_h1').value || 0;
            ...
            if (prof_ctr === "NAK") {
                total_nak += (credit_PC - debit_PC);
            } else if (prof_ctr === "NAG") {
                total_nag += (credit_PC - debit_PC);
            }
        });
                ...
                $.ajax({
                    ...
                    success: function(response){
                        $("#select input[type=checkbox]:checked").each(function () {
                            ...
                            var cost_ctr = $(this).closest('tr').find('td:eq(3)').find('select[id=cost_ctr] option').filter(':selected').val();                   
                            var buyer = $(this).closest('tr').find('td:eq(4) input').val();
                            var no_ws = $(this).closest('tr').find('td:eq(5) input').val();
                            var curre = $(this).closest('tr').find('td:eq(6)').find('select[name=currenc] option').filter(':selected').val();                   
                            var debit = $(this).closest('tr').find('td:eq(7) input').val();
                            var credit = $(this).closest('tr').find('td:eq(8) input').val();    
                            var keterangan = $(this).closest('tr').find('td:eq(9) input').val();
                            ...
                            if(refer == 'None' && coa_akun != '' && amount != '' && amount != '0' && balance == '0' || refer == 'Settlement' && coa_akun != '' && balance == '0'){   
                                $.ajax({
                                    ...
                                    data: {'no_pci':no_pci, 'tgl_pci':tgl_pci, 'nomor_coa':nomor_coa, 'cost_ctr':cost_ctr, 'buyer':buyer, 'no_ws':no_ws, 'curre':curre, 'debit':debit, 'credit':credit, 'keterangan':keterangan, 'oth_doc':oth_doc, 'referen':referen, 'prof_ctr':prof_ctr},
                                    cache: 'false',
                                    close: function(e){
                                        e.preventDefault();
                                    },
...
if(document.getElementById('akun').value == ''){
...
}else if(document.getElementById('nominal_h1').value == '' && refer == 'None'){
    document.getElementById('nominal_h1').focus();
    alert("Please Enter Amount");
}else if(document.getElementById('nominal_h1').value == '0' && refer == 'None'){
    document.getElementById('nominal_h1').focus();
    ...
    var create_by = '<?php echo $user; ?>';
    var h_profit_center = $('select[name=h_profit_center] option').filter(':selected').val();  
    // alert(h_profit_center);
    if(coa_akun != '' && amount != '' && amount != '0'){
        $.ajax({
            type:'POST',
            url:'insert_petty_in_h.php',
            data: {'no_pci':no_pci, 'tgl_pci':tgl_pci, 'reff':reff, 'reff_doc':reff_doc, 'oth_doc':oth_doc, 'coa_akun':coa_akun, 'curr':curr, 'amount':amount, 'create_by':create_by, 'pesan':pesan, 'h_profit_center':h_profit_center},
            cache: 'false',
            close: function(e){
                e.preventDefault();
            },
