...
<style>
    ...
    /* Styling untuk table-wrapper jika perlu */
    .table-wrapper {
        max-width: 100%;
        overflow-x: auto;
    }
...
<div class="col p-4">
                    ...
                    if ($ref != 'Settlement') { ?>
                        ...
                        <div class="col-md-4">
                            ...
                            <select class="form-control selectpicker" name="nama_supp" id="nama_supp" data-dropup-auto="false" data-live-search="true">
                                ...
                                $sql = mysqli_query($conn1,"select * from (select distinct(Supplier) from mastersupplier where tipe_sup = 'S' order by Supplier ASC) a UNION select cc_name as cost_name from b_master_cc where status = 'Active'");
                                while ($row = mysqli_fetch_array($sql)) {
                                    $data = $row['Supplier'];
    ...
    <?php } else{ ?>
        ...
        <div class="col-md-3">
            <label for="nama_supp"><b>Reff Document</b></label>            
            <select class="form-control selectpicker" name="oth_doc" id="oth_doc" data-dropup-auto="false" data-live-search="true" onchange="getdataadvance(this.value)">
             <option value="" disabled selected="true">Select Reference</option>
             <?php
             $oth_doc ='';
             if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    <div class="col-md-3">
        <label for="nama_supp"><b>Supplier</b></label>            
        <input type="text" style="font-size: 12px;" class="form-control" id="nama_supp" name="nama_supp" readonly value=""> 
    </div>
    <input type="hidden" style="font-size: 14px;" class="form-control tanggal" id="start_date" name="start_date" 
    value="<?php
    $start_date ='';
    if ($_SERVER['REQUEST_METHOD'] == 'POST') {
      $start_date = date("Y-m-d",strtotime($_POST['start_date']));
...
placeholder="Tanggal Awal">
<input type="hidden" style="font-size: 14px;" class="form-control tanggal" id="end_date" name="end_date" 
value="<?php
$end_date ='';
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
  $end_date = date("Y-m-d",strtotime($_POST['start_date']));
                    ...
                    if ($ref_num == 'List Payment'){
                        ...
                        while($row = mysqli_fetch_array($sql)){
                            ...
                            if ($ref_num == 'Advance') {
                                ...
                                <select class="form-control" name="nomor_coa" id="nomor_coa" style="width: 250px"> <option value="1.49.98" >1.49.98 UANG MUKA PEMBELIAN - KAS KECIL</option>';?> <?php $sql = mysqli_query($conn1,"select no_coa as id_coa,concat(no_coa,' ', nama_coa) as coa from mastercoa_v2 where no_coa != '1.49.98'"); foreach ($sql as $cc) : echo'<option value="'.$cc["id_coa"].'"> '.$cc["coa"].' </option>'; endforeach; 
                                echo'</select>
                                </td>';
                            }else{
                                echo '<td style="width: 200px;">
                        ...
                        if($ref_num == 'List Payment'){
                            ...
                            if ($ref_num == 'Advance') {
                                ...
                                <button type="button" class="btn btn-primary" onclick=addRow_adv("tbody2");>Add Row</button>
                                <button type="button" class="btn btn-warning" onclick=InsertRow_adv("tbody2");>Interject Row</button>
                                <button type="button" class="btn btn-danger" onclick=hapus2_adv();>Delete Row</button>
                                </td>
                                </tr>
                                </tfoot>
                        ...
                        <div class="box footer">   
                            <form id="form-simpan">
                                <?php
                                $ref_num = isset($_POST['ref_num']) ? $_POST['ref_num']: null;;
                                if ($ref_num == 'None' OR $ref_num == 'Advance' OR $ref_num == 'Settlement') {
                                    echo '
                                ...
                                <table <?php echo $data ?> id="mytable2" class="table table-striped table-bordered" cellspacing="0" width="100%">
                                    <thead>
...
<script type="text/javascript">
  function getdataadvance(val){
    ...
    $.ajax({
        type: 'POST', 
        url: 'get_supp_advance.php', 
        data: {'no_pco':no_pco},
        success: function(response) { 
                // alert(response);
                $('#nama_supp').val(response); 
            }
        });

    $.ajax({
        type:'POST',
        url:'load-pco-advance.php',
        data: {'no_pco':no_pco},
        cache: 'false',
        close: function(e){
            e.preventDefault();
...
function input_credit(){ 
    ...
    for (var i = 1; i < (table.rows.length); i++) {
        ...
        var curr = document.getElementById("tbody2").rows[i].cells[6].children[0].value;
...
function updateCostCenter(profCtr, noCoa, row) {
   ...
   function addRow(tableID) {
    ...
    <td><input type="text" class="form-control" name="keterangan[]" style="font-size: 12px;width: 150px;" autocomplete="off"></td>
    <td><input type="text" class="form-control" name="keterangan[]" style="font-size: 12px;width: 150px;" autocomplete="off"></td>
    <td>
    ...
    <td><input type="number" min="1" class="form-control" name="txt_amount[]" style="text-align: right; font-size: 12px;width: 150px;" oninput="input_debit(value)" autocomplete="off"></td>
    <td><input type="number" min="1" class="form-control" name="txt_credit[]" style="text-align: right; font-size: 12px;width: 150px;" oninput="input_credit(value)" autocomplete="off"></td>
    <td><input type="text" class="form-control" name="keterangan[]" style="font-size: 12px;width: 150px;" autocomplete="off"></td>
    <td><input type="checkbox" class="checkall_a" name="chk_a[]" value=""></td>
    </tr>
    `;
...
function InsertRow_adv(tableID)
{
    try{
        var table = document.getElementById(tableID);
        var rowCount = table.rows.length;
...
function hitungulang2(){
    ...
    for(var i=0; i<rowCount2; i++){
        ...
        var price2 = parseFloat(document.getElementById("tbody2").rows[i].cells[8].children[0].value,10) || 0;
...
<script type="text/javascript">
    $("input[name=nominal_h]").keyup(function(){
        ...
        $("input[type=text]").each(function () {
           ...
           // alert(refer);   
           var curr = $(this).closest('tr').find('td:eq(6)').find('select[name=currenc] option').filter(':selected').val();        
           var rate = parseFloat(document.getElementById('rate').value,10) || 1;
           var ttl_h = parseFloat(document.getElementById('nominal_h').value,10) || 0;
           var val = document.getElementById('valuta').value;
           valu = val;
...
<script type="text/javascript">
    $("#form-simpan").on("click", "#simpan", function(){
        var refer = $('select[name=ref_num] option').filter(':selected').val();
        if (refer == 'None' || refer == 'Advance' || refer == 'Settlement') {
            ...
            if (refer == 'Settlement') {
                var oth_doc = $('select[name=oth_doc] option').filter(':selected').val();
            ...
            var curr = document.getElementById('valuta').value; 
            var amount = document.getElementById('nominal_h').value || 0;        
            ...
            $("#mytable input[type=checkbox]:checked").each(function (i) {
             if (i === 0) return true;
            ...
            if(refer == 'Settlement' && nama_supp != '' && akun != '' && balance == '0' || amount != '' && nama_supp != '' && akun != '' && balance == '0'){
                $.ajax({
                    type:'POST',
                    url:'insert_petty_cashout_h.php',
                    data: {'no_pco':no_pco, 'tgl_pco':tgl_pco, 'reff':reff, 'nama_supp':nama_supp, 'akun':akun, 'curr':curr, 'amount':amount, 'deskripsi':deskripsi, 'create_by':create_by, 'oth_doc':oth_doc, 'total_nak':total_nak, 'total_nag':total_nag, 'h_profit_center':h_profit_center},
                    cache: 'false',
                    close: function(e){
                        e.preventDefault();
                    },
                    success: function(response){
                        $(" #select input[type=checkbox]:checked").each(function () {
                            ...
                            var no_costcntr = $(this).closest('tr').find('td:eq(3)').find('select[id=cost_ctr] option').filter(':selected').val();                   
                            var buyer = $(this).closest('tr').find('td:eq(4) input').val();
                            var no_ws = $(this).closest('tr').find('td:eq(5) input').val();
                            var curr = $(this).closest('tr').find('td:eq(6)').find('select[name=currenc] option').filter(':selected').val();                   
                            var debit = $(this).closest('tr').find('td:eq(7) input').val();
                            var credit = $(this).closest('tr').find('td:eq(8) input').val();    
                            var deskripsi = $(this).closest('tr').find('td:eq(9) input').val();
                            var nama_supp = $('select[name=nama_supp] option').filter(':selected').val();
                            var akun = document.getElementById('akun').value;
                            ...
                            if(amount != '' && nama_supp != '' && akun != '' && balance == '0' && debit != '' || amount != '' && nama_supp != '' && akun != '' && balance == '0' && credit != '' || refer == 'Settlement' && nama_supp != '' && akun != '' && balance == '0' && debit != '' || refer == 'Settlement' && nama_supp != '' && akun != '' && balance == '0' && credit != ''){   
                                $.ajax({
                                    type:'POST',
                                    url:'insert_petty_cashout_none.php',
                                    data: {'no_pco':no_pco, 'tgl_pco':tgl_pco, 'reff_doc':reff_doc, 'no_coa':no_coa, 'no_costcntr':no_costcntr, 'buyer':buyer, 'no_ws':no_ws, 'curr':curr, 'debit':debit, 'credit':credit, 'deskripsi':deskripsi, 'oth_doc':oth_doc, 'prof_ctr':prof_ctr},
                                    cache: 'false',
                                    close: function(e){
                                        e.preventDefault();
                                    },
...
if($('select[name=nama_supp] option').filter(':selected').val() == '' || $('select[name=nama_supp] option').filter(':selected').val() == '-'){
...
}else if(document.getElementById('nominal_h').value == '' && refer != 'Settlement'){
    ...
    if(amount != '' && nama_supp != '' && akun != '' && balance == '0'){
        $.ajax({
            type:'POST',
            url:'insert_petty_cashout_h.php',
            data: {'no_pco':no_pco, 'tgl_pco':tgl_pco, 'reff':reff, 'nama_supp':nama_supp, 'akun':akun, 'curr':curr, 'amount':amount, 'deskripsi':deskripsi, 'create_by':create_by, 'oth_doc':oth_doc, 'h_profit_center':h_profit_center},
            cache: 'false',
            close: function(e){
                e.preventDefault();
            },
                ...
                $("input[type=checkbox]:checked").each(function () {
                    ...
                    if(amount != '' && nama_supp != '' && akun != '' && balance == '0'){
                        $.ajax({
                            ...
                            data: {'no_pco':no_pco, 'tgl_pco':tgl_pco, 'no_coa':no_coa, 'reff_doc':reff_doc, 'reff_date':reff_date, 'deskripsi':deskripsi, 'debit':debit, 'credit':credit, 'no_pay':no_pay, 'pay_date':pay_date, 'due_date':due_date, 'dpp':dpp, 'ppn':ppn, 'pph':pph, 'total':total, 'total_idr':total_idr, 'curr':curr, 'kodelp':kodelp, 'amount_lp':amount_lp, 'pesan':pesan, 'h_profit_center':h_profit_center},
                            cache: 'false',
                            close: function(e){
                                e.preventDefault();
                            },
