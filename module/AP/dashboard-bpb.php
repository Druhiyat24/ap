...
union (select nama_supp, no_bpb, tgl_bpb, top, duedate, curr, total from tbl_tamb_bpb2 where tgl_bpb between '$start_date' and '$end_date')) as b order by b.Supplier asc");
    ...
    $sqldel = "delete from dsb_ap_bpb";
    $querydel = mysqli_query($conn1,$sqldel);

    $sqldel2 = "ALTER TABLE dsb_ap_bpb AUTO_INCREMENT = 1;";
...
select reff_doc,reff_date,reff_date from tbl_list_journal where reff_doc = '$no_bpb' and no_journal like '%GM/NAG%' and debit != 0 and tgl_journal >= '2024-09-01' and tgl_journal < '$start_date' and type_journal = 'ACCOUNT PAYABLE' GROUP BY reff_doc");
    ...
    if($sa_awal == '0' and $tambah == '0' and $kurang == '0' and $sa_akhir == '0'){
        ...
        $relasi = isset($rowcoa['relasi']) ? $rowcoa['relasi'] : null;
             ...
             $queryin = "INSERT INTO dsb_ap_bpb (nama_supp,no_bpb,tgl_bpb,due_date,curr,beg_balance,addition,deduction,end_balance,end_balance_idr,create_date,due_0,due_1,due_2,due_3,due_4,due_5,due_6,due_7,due_total,produe_0,produe_1,produe_2,produe_3,produe_4,produe_5,produe_6,produe_total,filter_awal,filter_akhir,no_coa,item_type1,item_type2,relasi) 
                        VALUES 
