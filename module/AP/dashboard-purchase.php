 <?php
    ...
    function flush_purchase_batch($conn1, &$values) {
        if (empty($values)) return;
        $sql_ins = "INSERT INTO dsb_ap_purchase (no_bpb,tgl_bpb,nama_supp,tipe,tipe_item,jml_qty,qty_good,qty_reject,curr,price,total,rate,dpp,ppn,total_idr,jenis_transaksi,create_date)
    ...
    mysqli_query($conn1, "delete from dsb_ap_purchase where status is null");
    ...
    while($row = mysqli_fetch_array($sql)){
        ...
        //     echo '</tr>';
            ...
            if (count($values) >= $batch_size) {
                flush_purchase_batch($conn1, $values);
            }
