                ...
                // echo '<option value="'.$namasupp.'"'.$isSelected.'">'. $namasupp .'</option>'; 
                    ...
                    <div class="input-group-append col">                                   
                        ...
                        if ($_SERVER['REQUEST_METHOD'] == 'POST') {
                            ...
                            echo '<a target="_blank" href="ekspor_e_statement.php?thn_from='.$thn_from.'&& bln_from='.$bln_from.'&& thn_to='.$thn_to.'&& bln_to='.$bln_to.'"><button type="button" class="btn btn-success " style= "margin-top: 30px; margin-bottom: 5px;margin-right: 15px;"><i class="fa fa-file-excel-o" aria-hidden="true" style="padding-right: 10px; padding-left: 5px;font-size: 1rem;color: #fff;text-shadow: 1px 1px 1px #000"> Excel</i></button></a>';
                        ?> 
                    </div>                                                            
                </div>
                <br/>
                ...
                <table id="datatable" class="table table-striped table-bordered" role="grid" cellspacing="0" width="100%">
                    <thead>
                        <tr class="thead-dark">
                            ...
                            <th style="text-align: center;vertical-align: middle;width: 20%;">Bank</th>
                            <th style="text-align: center;vertical-align: middle;width: 18%;">Account</th>
                            <th style="text-align: center;vertical-align: middle;width: 12%;">Periode</th>
                            <th style="text-align: center;vertical-align: middle;width: 15%;">Status</th>
                            <th style="text-align: center;vertical-align: middle;width: 15%;">Upload Date</th>
...
//                         ) b on b.kode_tanggal = a.kode_tanggal and b.account_bank = bank_account order by bank_account, b.id desc) a LEFT JOIN (select kode_tanggal tgl_kode, account_bank akun, MAX(created_date) tgl_upload from b_estatement GROUP BY kode_tanggal, account_bank) b on b.tgl_kode = a.kode_tanggal and b.akun = a.bank_account where status_bank != 'Deactive' OR stat_upload != 'Belum Upload'  ";
                    ...
                    while($row = mysqli_fetch_array($sql)){
                        ...
                        if ($akun2 == $akun && $kode_tgl == $kode_tgl2) { 
                            ...
                            echo '<tr style="font-size:12px;text-align:left;'.$color.'">
