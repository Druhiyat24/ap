<?php include '../header.php' ?>
    ...
    <!-- MAIN -->
    <div class="col p-4">
        <h2 class="text-center">FORM UPDATE MASTER BANK</h2>
<div class="box">
...
<div class="box footer">   
        <form id="form-simpan">
            ...
            <div class="col-md-3 mb-3">  
                <?php
                $doc_number = base64_decode($_GET['doc_number']);
                $sql = mysqli_query($conn2,"select active_date from b_masterbank where doc_number = '$doc_number'");
                $row = mysqli_fetch_array($sql);
                ?>                             
                <input type="text" style="font-size: 15px;" name="tgl_active" id="tgl_active" class="form-control tanggal" 
            value="<?= $row['active_date']; ?>" autocomplete='off'>
            </div>
            </div>
            <div class="form-row col">
            ...
            <?php
            $doc_number = base64_decode($_GET['doc_number']);
            $sql = mysqli_query($conn2,"select id,doc_number, SUBSTR(doc_number,12,2) bln, SUBSTR(doc_number,14,2) thn,SUBSTR(doc_number,17,5) no_urut from b_masterbank where doc_number = '$doc_number'");
            $row = mysqli_fetch_array($sql);

            echo'<input type="text" readonly style="font-size: 14px;text-transform:uppercase" class="form-control-plaintext" id="no_doc" name="no_doc" value="'.$row['doc_number'].'">
            <input type="text" readonly style="font-size: 14px;" class="form-control-plaintext" id="no_urut" name="no_urut" value="'.$row['no_urut'].'" hidden>
            <input type="text" readonly style="font-size: 14px;" class="form-control-plaintext" id="bulan" name="bulan" value="'.$row['bln'].'" hidden>
            <input type="text" readonly style="font-size: 14px;" class="form-control-plaintext" id="tahun" name="tahun" value="'.$row['thn'].'" hidden>
            <input type="text" readonly style="font-size: 14px;" class="form-control-plaintext" id="maxid" name="maxid" value="'.$row['id'].'" hidden>'
            ?>                            
                
            </div>
            <?php
            $doc_number = base64_decode($_GET['doc_number']);
            $sql = mysqli_query($conn2,"select jurnal_sb, UPPER(bank_name) bank_name, sob,bank_account,curr,deskripsi from b_masterbank where doc_number = '$doc_number'");
            $row = mysqli_fetch_array($sql);
                if ($row['jurnal_sb'] == 'N') {
                    $checked = '';
                }else{
            ...
            ?> 
            &nbsp;<input type="checkbox" class="checkbox_sb1" id="to_sb1" name="to_sb1" value="1" style="margin-top: -15px;" <?= $checked; ?>>
                <label for="to_sb1" style="margin-top: 8px;"><b>Include</b></label><br>
                <input type="hidden" id="fil_sb1" name="fil_sb1" value="<?= $row['jurnal_sb']; ?>">
            </div>          
            <!-- <div class="form-row col">
            ...
            <div class="col-md-5 mb-2">
                ...
                <div class="input-group" >
                    ...
                    <input type="text" style="font-size: 13px;text-transform:uppercase" class="form-control" id="nama_bank" name="nama_bank" value="<?= $row['bank_name']; ?>"> 
                </div>
                </br>
                <div class="input-group" >
                    <label for="nama_supp" class="col-form-label" style="width: 150px;"><b>SOB<i style="color: red;">*</i></b></label>                  
                    <input type="text" style="font-size: 14px;text-transform:uppercase" class="form-control" id="sob" name="sob" maxlength="3" value="<?= $row['sob']; ?>"> 
                </div>
                </br>
          
            <div class="input-group">
                <label for="accountid" class="col-form-label" style="width: 150px;" ><b>Account </b></label>
                <input type="text" style="font-size: 14px;" class="form-control" id="accountid" name="accountid" value="<?= $row['bank_account']; ?>">  
                ...
                <div class="input-group" >
                    ...
                    <option value="<?= $row['curr']; ?>" selected="selected"><?= $row['curr']; ?></option>  
                    <?php if($row['curr'] == 'IDR') {?>
                 ...
                 <div class="input-group" >
                    ...
                    <textarea style="font-size: 15px; text-align: left;" cols="30" rows="5" type="text" class="form-control " name="pesan" id="pesan" placeholder="descriptions..." required><?= $row['deskripsi']; ?></textarea>         
                </div>
                </br>
