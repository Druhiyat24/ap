...
$sqlH = mysqli_query($conn2, "SELECT pl_number, pl_date, deskripsi, from_account, status, created_by FROM pv_payment_list_h WHERE pl_number = '$pl_esc' LIMIT 1");
$plHeader = mysqli_fetch_assoc($sqlH);

if (!$plHeader) {
    echo '<div class="alert alert-danger m-4">Payment List tidak ditemukan.</div>';
    exit;
}

if ($plHeader['status'] !== 'Draft') {
    echo '<div class="alert alert-warning m-4">This Payment List has status <b>' . htmlspecialchars($plHeader['status']) . '</b> and cannot be edited. Only <b>Draft</b> documents can be edited.</div>';
    echo '<a href="payment-list.php" class="btn btn-secondary ml-4"><i class="fa fa-angle-double-left"></i> Back to Payment List</a>';
    exit;
}

// Current PVs in this PL
$sqlDet = mysqli_query($conn2, "SELECT id, type_pv, no_kbon, tgl_kbon, nama_supp, curr, total, deskripsi, profit_center, status FROM pv_payment_list_det WHERE pl_number = '$pl_esc' AND status != 'Cancel' ORDER BY id ASC");
$currentPVs = [];
...
if (!empty($type_pv)) {
    ...
    // Baris yang SUDAH ada di PL ini (disembunyikan dari Section B)
    $currentKbonsSet = [];
    foreach ($currentPVs as $cur) {
        $currentKbonsSet[$plKey($cur['no_kbon'], $cur['profit_center'] ?? '')] = true;
    }

    // Exclude PVs already in other PLs (non-cancelled)
    $alreadyListed = [];
    $sqlListed = mysqli_query($conn2, "SELECT no_kbon, profit_center FROM pv_payment_list_det WHERE pl_number != '$pl_esc' AND status != 'Cancel'");
...
<div class="container-fluid mt-2 p-3" style="padding-left: 2rem; padding-right: 2rem;">
    ...
    <div class="card shadow mb-3" style="border: 1px solid #e2e8f0; border-radius: 10px;">
        <div class="card-header d-flex justify-content-between align-items-center"
            ...
            <span><i class="fa fa-edit mr-2"></i> EDIT PAYMENT LIST</span>
        </div>
    ...
    <div class="card shadow mb-3" style="border: 1px solid #e2e8f0; border-left: 5px solid #7b1c1c; border-radius: 10px;">
        <div class="card-header bg-white d-flex justify-content-between align-items-center"
            ...
            <span><i class="fa fa-list"></i> PVs in this Payment List</span>
            <i class="fa fa-chevron-up"></i>
        </div>
        <div class="collapse show" id="section_a_body">
        <div class="card-body p-2">
            <p class="text-muted" style="font-size: 12px; margin-bottom: 6px;">
                <i class="fa fa-info-circle"></i> Check a row to <b>remove</b> that PV from this Payment List.
            </p>
            <div class="table-responsive">
                <table id="tbl_current" class="table table-striped table-bordered" style="font-size: 13px; text-align: center; width: 100%;">
                    ...
                    <tbody>
                        ...
                        <tr><td colspan="9" class="text-center text-muted">No PVs in this Payment List.</td></tr>
                        <?php else: ?>
    ...
    <div class="card shadow mb-3" style="border: 1px solid #e2e8f0; border-left: 5px solid #1E3A8A; border-radius: 10px;">
        <div class="card-header bg-white d-flex justify-content-between align-items-center"
            ...
            <span><i class="fa fa-plus-circle"></i> Add PV to Payment List</span>
            <i class="fa fa-chevron-up"></i>
        </div>
                ...
                <div class="form-row mb-2">
                    <div class="col-md-3 mb-2">
                        ...
                        <select class="form-control form-control-sm select2bs4" name="h_type_pv" id="type_pv" data-dropup-auto="false">
                            ...
                            <option value="Regular-Installment" <?= $selType === 'Regular-Installment' ? 'selected' : '' ?>>Regular-Installment</option>
                            <option value="Biaya-DP-CBD" <?= $selType === 'Biaya-DP-CBD' ? 'selected' : '' ?>>Biaya-DP-CBD</option>
                        </select>
                    </div>
                    <div class="col-md-3 mb-2">
    ...
    <div class="card shadow mb-4" style="border: 1px solid #e2e8f0; border-radius: 10px;">
        <div class="card-body p-3 d-flex align-items-center" style="gap: 10px;">
            ...
            <a href="payment-list.php" class="btn btn-danger btn-sm" style="border-radius: 6px;">
