...
<body>
    ...
    <table style="width:100%;font-size:10px;" border="1" width="100%">
        ...
        $sql = mysqli_query($conn2,"select a.*, coalesce(total_adj,0) total_adj, coalesce(price_adj,0) price_adj, coalesce(qty_adj,0) qty_adj, coalesce(total_adj_bfr,0) total_adj_bfr, coalesce(price_adj_bfr,0) price_adj_bfr, coalesce(qty_adj_bfr,0) qty_adj_bfr from (SELECT 
            ...
            UNION 
            select *,CASE WHEN tipe_pembelian IN ('Pembelian Lokal','Perubahan Revisi Harga')
 THEN qty END qty_a, CASE WHEN tipe_pembelian IN ('Pembelian Lokal','Perubahan Revisi Harga')
 THEN price END price_a, CASE WHEN tipe_pembelian IN ('Pembelian Lokal','Perubahan Revisi Harga')
 THEN total END total_a, CASE WHEN tipe_pembelian = 'Pembelian Impor' THEN qty END qty_b, CASE WHEN tipe_pembelian = 'Pembelian Impor' THEN price END price_b, CASE WHEN tipe_pembelian = 'Pembelian Impor' THEN total END total_b, CASE WHEN tipe_pembelian IN ('Pengembalian dari Subkontraktor CMT','Pengembalian dari Subkontraktor Jasa')
...
while($row2 = mysqli_fetch_array($sql)){
    ...
    if ($total_sal_qty == 0) {
        ...
        $total_sal_nilai = $total_sal_nilai; 
    }
