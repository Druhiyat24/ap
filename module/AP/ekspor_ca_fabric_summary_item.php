...
mutasi as (select a.*, COALESCE(out_prod_qty,0) out_prod_qty,   COALESCE(out_prod_total,0) out_prod_total,   COALESCE(out_prod_price,0) out_prod_price,   COALESCE(out_subcont_qty,0) out_subcont_qty,   COALESCE(out_subcont_total,0) out_subcont_total,   COALESCE(out_subcont_price,0) out_subcont_price,   COALESCE(out_lokal_qty,0) out_lokal_qty,   COALESCE(out_lokal_total,0) out_lokal_total,   COALESCE(out_lokal_price,0) out_lokal_price,   COALESCE(out_impor_qty,0) out_impor_qty,   COALESCE(out_impor_total,0) out_impor_total,   COALESCE(out_impor_price,0) out_impor_price,   COALESCE(out_sample_qty,0) out_sample_qty,   COALESCE(out_sample_total,0) out_sample_total,   COALESCE(out_sample_price,0) out_sample_price,   COALESCE(out_salnongroup_qty,0) out_salnongroup_qty,   COALESCE(out_salnongroup_total,0) out_salnongroup_total,   COALESCE(out_salnongroup_price,0) out_salnongroup_price,   COALESCE(out_salgroup_qty,0) out_salgroup_qty,   COALESCE(out_salgroup_total,0) out_salgroup_total,   COALESCE(out_salgroup_price,0) out_salgroup_price,   COALESCE(out_other_qty,0) out_other_qty,   COALESCE(out_other_total,0) out_other_total,   COALESCE(out_other_price,0) out_other_price,   COALESCE(jumlah_out_qty,0) jumlah_out_qty,   COALESCE(jumlah_out_total,0) jumlah_out_total,   COALESCE(jumlah_out_price,0) jumlah_out_price from pemasukan_fix a left join pengeluaran_fix b on b.id_roll = a.barcode_mapping),
    ...
    -- SALDO AWAL
    SUM(ROUND(saldo_awal_qty,2))   AS saldo_awal_qty,
    SUM(ROUND(saldo_awal_total,2)) AS saldo_awal_total,
    COALESCE(
        SUM(ROUND(saldo_awal_total,2)) 
        / NULLIF(SUM(ROUND(saldo_awal_qty,2)),0), 
        0
    ) AS saldo_awal_price,

    -- IN LOKAL
    SUM(ROUND(in_lokal_qty,2))   AS in_lokal_qty,
    SUM(ROUND(in_lokal_total,2)) AS in_lokal_total,
    COALESCE(
        SUM(ROUND(in_lokal_total,2)) 
        / NULLIF(SUM(ROUND(in_lokal_qty,2)),0), 
        0
    ) AS in_lokal_price,

    -- IN IMPOR
    SUM(ROUND(in_impor_qty,2))   AS in_impor_qty,
    SUM(ROUND(in_impor_total,2)) AS in_impor_total,
    COALESCE(
        SUM(ROUND(in_impor_total,2)) 
        / NULLIF(SUM(ROUND(in_impor_qty,2)),0), 
        0
    ) AS in_impor_price,

    -- IN SUBCONT
    SUM(ROUND(in_subcont_qty,2))   AS in_subcont_qty,
    SUM(ROUND(in_subcont_total,2)) AS in_subcont_total,
    COALESCE(
        SUM(ROUND(in_subcont_total,2)) 
        / NULLIF(SUM(ROUND(in_subcont_qty,2)),0), 
        0
    ) AS in_subcont_price,

    -- IN PRODUKSI
    SUM(ROUND(in_produksi_qty,2))   AS in_produksi_qty,
    SUM(ROUND(in_produksi_total,2)) AS in_produksi_total,
    COALESCE(
        SUM(ROUND(in_produksi_total,2)) 
        / NULLIF(SUM(ROUND(in_produksi_qty,2)),0), 
        0
    ) AS in_produksi_price,

    -- IN SAMPLE
    SUM(ROUND(in_sample_qty,2))   AS in_sample_qty,
    SUM(ROUND(in_sample_total,2)) AS in_sample_total,
    COALESCE(
        SUM(ROUND(in_sample_total,2)) 
        / NULLIF(SUM(ROUND(in_sample_qty,2)),0), 
        0
    ) AS in_sample_price,

    -- JUMLAH IN
    SUM(ROUND(jumlah_in_qty,2))   AS jumlah_in_qty,
    SUM(ROUND(jumlah_in_total,2)) AS jumlah_in_total,
    COALESCE(
        SUM(ROUND(jumlah_in_total,2)) 
        / NULLIF(SUM(ROUND(jumlah_in_qty,2)),0), 
        0
    ) AS jumlah_in_price,

    -- OUT PRODUKSI
    SUM(ROUND(out_prod_qty,2))   AS out_prod_qty,
    SUM(ROUND(out_prod_total,2)) AS out_prod_total,
    COALESCE(
        SUM(ROUND(out_prod_total,2)) 
        / NULLIF(SUM(ROUND(out_prod_qty,2)),0), 
        0
    ) AS out_prod_price,

    -- OUT SUBCONT
    SUM(ROUND(out_subcont_qty,2))   AS out_subcont_qty,
    SUM(ROUND(out_subcont_total,2)) AS out_subcont_total,
    COALESCE(
        SUM(ROUND(out_subcont_total,2)) 
        / NULLIF(SUM(ROUND(out_subcont_qty,2)),0), 
        0
    ) AS out_subcont_price,

    -- OUT LOKAL
    SUM(ROUND(out_lokal_qty,2))   AS out_lokal_qty,
    SUM(ROUND(out_lokal_total,2)) AS out_lokal_total,
    COALESCE(
        SUM(ROUND(out_lokal_total,2)) 
        / NULLIF(SUM(ROUND(out_lokal_qty,2)),0), 
        0
    ) AS out_lokal_price,

    -- OUT IMPOR
    SUM(ROUND(out_impor_qty,2))   AS out_impor_qty,
    SUM(ROUND(out_impor_total,2)) AS out_impor_total,
    COALESCE(
        SUM(ROUND(out_impor_total,2)) 
        / NULLIF(SUM(ROUND(out_impor_qty,2)),0), 
        0
    ) AS out_impor_price,

    -- OUT SAMPLE
    SUM(ROUND(out_sample_qty,2))   AS out_sample_qty,
    SUM(ROUND(out_sample_total,2)) AS out_sample_total,
    COALESCE(
        SUM(ROUND(out_sample_total,2)) 
        / NULLIF(SUM(ROUND(out_sample_qty,2)),0), 
        0
    ) AS out_sample_price,

    -- OUT SAL NON GROUP
    SUM(ROUND(out_salnongroup_qty,2))   AS out_salnongroup_qty,
    SUM(ROUND(out_salnongroup_total,2)) AS out_salnongroup_total,
    COALESCE(
        SUM(ROUND(out_salnongroup_total,2)) 
        / NULLIF(SUM(ROUND(out_salnongroup_qty,2)),0), 
        0
    ) AS out_salnongroup_price,

    -- OUT SAL GROUP
    SUM(ROUND(out_salgroup_qty,2))   AS out_salgroup_qty,
    SUM(ROUND(out_salgroup_total,2)) AS out_salgroup_total,
    COALESCE(
        SUM(ROUND(out_salgroup_total,2)) 
        / NULLIF(SUM(ROUND(out_salgroup_qty,2)),0), 
        0
    ) AS out_salgroup_price,

    -- OUT OTHER
    SUM(ROUND(out_other_qty,2))   AS out_other_qty,
    SUM(ROUND(out_other_total,2)) AS out_other_total,
    COALESCE(
        SUM(ROUND(out_other_total,2)) 
        / NULLIF(SUM(ROUND(out_other_qty,2)),0), 
        0
    ) AS out_other_price,

    -- JUMLAH OUT
    SUM(ROUND(jumlah_out_qty,2))   AS jumlah_out_qty,
    SUM(ROUND(jumlah_out_total,2)) AS jumlah_out_total,
    COALESCE(
        SUM(ROUND(jumlah_out_total,2)) 
        / NULLIF(SUM(ROUND(jumlah_out_qty,2)),0), 
        0
    ) AS jumlah_out_price,

    -- SALDO AKHIR
    SUM(ROUND(saldo_akhir_qty,2))   AS saldo_akhir_qty,
    SUM(ROUND(saldo_akhir_total,2)) AS saldo_akhir_total,
    COALESCE(
        SUM(ROUND(saldo_akhir_total,2)) 
        / NULLIF(SUM(ROUND(saldo_akhir_qty,2)),0), 
        0
    ) AS saldo_akhir_price

FROM mutasi_barcode
GROUP BY id_jo, id_item, goods_code, itemdesc, satuan, no_ws;
");
