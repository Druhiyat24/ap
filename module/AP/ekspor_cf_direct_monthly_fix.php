<html>
<head>
    <title>Export Data CF Direct Monthly</title>
</head>
<body>
    <style type="text/css">
    body{
    ...
    function computeCfDirectMonth($conn2, $bulan, $tahun, $kata_filter) {
        $bulan_awal = $bulan;
        $bulan_akhir = $bulan;
        $tahun_awal = $tahun;
        $tahun_akhir = $tahun;
...
BETWEEN DATE_FORMAT('$tanggal_awal','%Y-%m') AND DATE_FORMAT('$tanggal_akhir','%Y-%m') group by id
        ...
        while ($row = mysqli_fetch_array($sql)) {
            $val = isset($row['total']) ? $row['total'] : 0;
            $result['operating'][$row['sub_kategori']] = array('total' => $val, 'eng' => $row['sub_kategori_eng']);
        ...
        while ($row2 = mysqli_fetch_array($sql2)) {
            $val = isset($row2['total']) ? $row2['total'] : 0;
            $result['investing'][$row2['sub_kategori']] = array('total' => $val, 'eng' => $row2['sub_kategori_eng']);
...
select sub_kategori, sub_kategori_eng, sum(total_nag) total_nag, sum(total_nak) total_nak, sum(total_all) total_all from data_fix where sub_kategori = 'Penerimaan Pinjaman' AND periode BETWEEN DATE_FORMAT('$tanggal_awal','%Y-%m') AND DATE_FORMAT('$tanggal_akhir','%Y-%m')");
        ...
        $result['penerimaan_pinjaman'] = isset($row_penerimaan['total_all']) ? $row_penerimaan['total_all'] : 0;
...
select sub_kategori, sub_kategori_eng, sum(total_nag) total_nag, sum(total_nak) total_nak, sum(total_all) total_all from data_fix where sub_kategori = 'Pembayaran Pinjaman' AND periode BETWEEN DATE_FORMAT('$tanggal_awal','%Y-%m') AND DATE_FORMAT('$tanggal_akhir','%Y-%m')");
        ...
        $result['pembayaran_pinjaman'] = isset($row_Pembayaran['total_all']) ? $row_Pembayaran['total_all'] : 0;
        ...
        $result['beginning_cash'] = isset($rowb['total']) ? $rowb['total'] : 0;
    ...
    $monthly_data = array();
    foreach ($months as $m) {
        $monthly_data[] = computeCfDirectMonth($conn2, $m['bulan'], $m['tahun'], $m['kata_filter']);
    }

    $operating_keys = array();
    $investing_keys = array();
    foreach ($monthly_data as $md) {
        foreach ($md['operating'] as $key => $val) {
            if (!isset($operating_keys[$key])) {
                $operating_keys[$key] = $val['eng'];
            }
        }
        foreach ($md['investing'] as $key => $val) {
            if (!isset($investing_keys[$key])) {
                $investing_keys[$key] = $val['eng'];
            }
        }
    }
    ...
    <table style="font-size:15px;border-collapse:collapse;" >
        ...
        foreach ($operating_keys as $key => $eng) {
            echo '<tr><td style="text-align: left;vertical-align: middle;">'.$key.'</td>';
            $sum = 0;
            foreach ($monthly_data as $idx => $md) {
                $val = isset($md['operating'][$key]['total']) ? $md['operating'][$key]['total'] : 0;
                $sum += $val;
        ...
        <tr><td colspan="<?php echo $jml_bulan + 3; ?>">&nbsp;</td></tr>
        ...
        foreach ($investing_keys as $key => $eng) {
            echo '<tr><td style="text-align: left;vertical-align: middle;">'.$key.'</td>';
            $sum = 0;
            foreach ($monthly_data as $idx => $md) {
                $val = isset($md['investing'][$key]['total']) ? $md['investing'][$key]['total'] : 0;
                $sum += $val;
        ...
        echo '<tr style="line-height: 30px;"><th style="text-align: left;vertical-align: middle;border-top:2px solid #000000;">Arus kas yang digunakan untuk aktivitas investasi</th>';
        ...
        echo '<th style="text-align: right;vertical-align: middle;border-top:2px solid #000000;"><i>Cash flow used from investing activities</i></th></tr>';
        ?>
        ...
        <?php
        $total_pendanaan_per_bulan = array_fill(0, $jml_bulan, 0);

        echo '<tr><td style="text-align: left;vertical-align: middle;">Penerimaan pinjaman</td>';
        $sum = 0;
        foreach ($monthly_data as $idx => $md) {
            $val = $md['penerimaan_pinjaman'];
            $sum += $val;
            $total_pendanaan_per_bulan[$idx] += $val;
            echo '<td style="text-align: right;vertical-align: middle;">'.fmt_cf($val).'</td>';
        }
        echo '<td style="text-align: right;vertical-align: middle;">'.fmt_cf($sum).'</td>';
        echo '<td style="text-align: right;vertical-align: middle;"><i>Proceeds from loans</i></td></tr>';

        echo '<tr><td style="text-align: left;vertical-align: middle;">Pembayaran pinjaman</td>';
        $sum = 0;
        foreach ($monthly_data as $idx => $md) {
            $val = $md['pembayaran_pinjaman'];
            $sum += $val;
            $total_pendanaan_per_bulan[$idx] += $val;
            echo '<td style="text-align: right;vertical-align: middle;">'.fmt_cf($val).'</td>';
        }
        echo '<td style="text-align: right;vertical-align: middle;">'.fmt_cf($sum).'</td>';
        echo '<td style="text-align: right;vertical-align: middle;"><i>Payment of loans</i></td></tr>';

        $total_pendanaan_sum = array_sum($total_pendanaan_per_bulan);
        echo '<tr style="line-height: 30px;"><th style="text-align: left;vertical-align: middle;border-top:2px solid #000000;">Arus kas yang digunakan untuk aktivitas pendanaan</th>';
        ...
        echo '<th style="text-align: right;vertical-align: middle;border-top:2px solid #000000;"><i>Cash flow used from financing activities</i></th></tr>';

        $net_change_per_bulan = array();
        for ($i = 0; $i < $jml_bulan; $i++) {
            $net_change_per_bulan[$i] = $total_operasi_per_bulan[$i] + $total_investasi_per_bulan[$i] + $total_pendanaan_per_bulan[$i];
        }
        $net_change_sum = array_sum($net_change_per_bulan);
        ...
        foreach ($net_change_per_bulan as $v) {
            echo '<th style="text-align: right;vertical-align: middle;">'.fmt_cf($v).'</th>';
        }
