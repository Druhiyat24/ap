...
SELECT nm_memo, tgl_memo,no_invoice,supplier,buyer,total_memo from (SELECT b.id,a.id_supplier,d.supplier,g.supplier buyer,a.nm_memo, a.tgl_memo,IF(b.inv_vendor is null,'-',b.inv_vendor) no_invoice,'IDR' curr,sum(b.biaya * IFNULL(mr.rate,1)) total_memo,'' id_book from memo_h a inner join mastersupplier d on d.id_supplier =a.id_supplier inner join mastersupplier g on g.id_supplier =a.id_buyer inner join memo_det b on b.id_h = a.id_h LEFT JOIN (SELECT curr, tanggal, rate FROM masterrate where v_codecurr = 'PAJAK' GROUP BY curr, tanggal) mr on mr.curr = a.curr and mr.tanggal = a.tgl_memo where  a.nm_memo >= 'MEMO/NAG/2310/01039' and a.tgl_memo BETWEEN '2023-01-01' and '$end_date' and b.cancel = 'N' and a.status NOT IN ('DRAFT','CANCEL') GROUP BY a.nm_memo order by a.nm_memo asc ) a) a left join
