...
<body>
    ...
    <table style="width:100%;font-size:10px;" border="1" width="100%">
        ...
        $sql = mysqli_query($conn2,"SELECT nm_memo,tgl_memo,no_invoice,supplier,buyer,IF(tgl_memo < '$start_date',total_memo * rate,0) beg_balance, IF(tgl_memo >= '$start_date',total_memo * rate,0) addition,(total_bk) deduction, (total_bk2) deduction2 FROM (SELECT a.nm_memo,tgl_memo,no_invoice,supplier,buyer,total_memo,COALESCE(ttl_amount,0) total_bk, COALESCE(ttl_amount2,0) total_bk2, a.curr, IFNULL(mr.rate,1) rate FROM (
SELECT nm_memo, tgl_memo,no_invoice,supplier,buyer,total_memo, curr from (SELECT b.id,a.id_supplier,d.supplier,g.supplier buyer,a.nm_memo, a.tgl_memo,IF(b.inv_vendor is null,'-',b.inv_vendor) no_invoice,a.curr,sum(b.biaya) total_memo,'' id_book from memo_h a inner join mastersupplier d on d.id_supplier =a.id_supplier inner join mastersupplier g on g.id_supplier =a.id_buyer inner join memo_det b on b.id_h = a.id_h where  a.nm_memo >= 'MEMO/NAG/2310/01039' and a.tgl_memo BETWEEN '2023-01-01' and '$end_date' and b.cancel = 'N' and a.status NOT IN ('DRAFT','CANCEL') GROUP BY a.nm_memo order by a.nm_memo asc ) a) a left join

(select nm_memo,bankout_date,SUM(amount * rate) amount,SUM(ttl_ppn * rate) ttl_ppn, SUM(ttl_pph * rate) ttl_pph, SUM(ttl_amount * rate) ttl_amount, SUM(inc_tax * rate) inc_tax from (select reff_doc nm_memo,bankout_date,amount,ttl_ppn,ttl_pph,(amount + ttl_ppn - ttl_pph)ttl_amount,(ttl_ppn - ttl_pph) inc_tax, rate from (select a.reff_doc,d.bankout_date, a.amount,a.ppn,round( a.amount * a.ppn /100,2) ttl_ppn,a.pph, round( a.amount * a.pph /100,2) ttl_pph, IFNULL(mr.rate,1) rate from tbl_pv a INNER JOIN tbl_pv_h h on h.no_pv = a.no_pv inner join b_bankout_det c on c.no_reff = a.no_pv inner join b_bankout_h d on d.no_bankout = c.no_bankout LEFT JOIN (SELECT curr, tanggal, rate FROM masterrate where v_codecurr = 'PAJAK' GROUP BY curr, tanggal) mr on mr.curr = h.curr and mr.tanggal = d.bankout_date where d.status != 'CANCEL' and a.reff_doc like '%MEMO/%') a) a where bankout_date BETWEEN '$start_date' and '$end_date' GROUP BY nm_memo) b on b.nm_memo = a.nm_memo left join

(select nm_memo_bfr,bankout_date,amount2,ttl_ppn2,ttl_pph2,ttl_amount2,inc_tax2 from (select nm_memo nm_memo_bfr,bankout_date,SUM(amount * rate) amount2,SUM(ttl_ppn * rate) ttl_ppn2, SUM(ttl_pph * rate) ttl_pph2, SUM(ttl_amount * rate) ttl_amount2, SUM(inc_tax * rate) inc_tax2 from (select reff_doc nm_memo,bankout_date,amount,ttl_ppn,ttl_pph,(amount + ttl_ppn - ttl_pph)ttl_amount,(ttl_ppn - ttl_pph) inc_tax, rate from (select a.reff_doc,d.bankout_date, a.amount,a.ppn,round( a.amount * a.ppn /100,2) ttl_ppn,a.pph, round( a.amount * a.pph /100,2) ttl_pph, IFNULL(mr.rate,1) rate from tbl_pv a INNER JOIN tbl_pv_h h on h.no_pv = a.no_pv inner join b_bankout_det c on c.no_reff = a.no_pv inner join b_bankout_h d on d.no_bankout = c.no_bankout LEFT JOIN (SELECT curr, tanggal, rate FROM masterrate where v_codecurr = 'PAJAK' GROUP BY curr, tanggal) mr on mr.curr = h.curr and mr.tanggal = d.bankout_date where d.status != 'CANCEL' and a.reff_doc like '%MEMO/%') a) a where bankout_date < '$start_date' GROUP BY nm_memo union
select reff_doc,tgl_journal,sum((debit * rate) - (credit * rate)) total, '0','0',((debit * rate) - (credit * rate)) total2, '0' from tbl_list_journal where no_coa = '2.18.02' and no_journal like '%GM/%' and tgl_journal < '$start_date') a) c on c.nm_memo_bfr = a.nm_memo LEFT JOIN (SELECT curr, tanggal, rate FROM masterrate where v_codecurr = 'PAJAK' GROUP BY curr, tanggal) mr on mr.curr = a.curr and mr.tanggal = a.tgl_memo) a");
