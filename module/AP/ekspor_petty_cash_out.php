...
<body>
    ...
    header("Content-Disposition: attachment; filename=Petty Cash Out.xls");
    $reference =$_GET['reference'];
    $doc_num =$_GET['doc_num'];
    $start_date = date("d F Y",strtotime($_GET['start_date']));
    $end_date = date("d F Y",strtotime($_GET['end_date'])); ?>

    <h4>LIST PETTY CASH OUT <br/> <?php if($reference == 'ALL'){ echo strtoupper($reference); echo " REFERENCE ";} else{ echo strtoupper($reference); } ?> <br/> PERIODE <?php echo strtoupper($start_date); ?> - <?php echo strtoupper($end_date); ?></h4>
    ...
    <table style="width:100%;font-size:10px;" border="1" >
        ...
        $sql = mysqli_query($conn2,"select a.no_pco,a.tgl_pco,a.reff,a.nama_supp,b.nama_coa,a.curr, a.amount,a.deskripsi,a.status, a.create_by, a.create_date from c_petty_cashout_h a left join mastercoa_v2 b on b.no_coa = a.coa_akun $where group by a.no_pco");
