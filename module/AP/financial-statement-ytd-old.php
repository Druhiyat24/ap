...
<div style="border:1px solid black;margin-top: 5px;">  
    <table style="font-size: 16px; margin:auto" border="0" role="grid" cellspacing="0" width="85%">
        ...
        if($tanggal_akhir < $tanggal_awal){
            ...
            while($row = mysqli_fetch_array($sql)){
                $aktivitas_operasi = isset($row['total']) ? $row['total'] : 0;
                if ($aktivitas_operasi > 0) {
                    $aktivitas_operasi = number_format($aktivitas_operasi,2);
                }else{
                    $aktivitas_operasi = '('.number_format(abs($aktivitas_operasi),2).')';
                }

                $total_aktivitas_operasi += isset($row['total']) ? $row['total'] : 0;
