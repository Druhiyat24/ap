...
<!-- DH-SKIN-END -->
    ...
    <!-- MAIN -->    
    <div class="col p-4">
        <h3 class="text-center">ACCEPT TRANSFER INVOICE FROM ACCOUNTING TO PURCHASING</h3>
<div class="box">
    <div class="box header">
<form id="form-data" action="form_approve_pch.php" method="post">
        <div class="form-row">
            <div class="col-md-6">
    ...
    <div class="box body">
        <div class="row">
            ...
            <table id="mytable" class="table table-striped table-bordered" cellspacing="0" width="100%" style="font-size: 12px;text-align:center;">
                    <thead>
                        <tr class="thead-dark">
                            ...
                            <th style="width:15%;">Total Kontrabon</th>
            ...
            $sql = mysqli_query($conn2,"select *,sum(total_amount) ttl_amount from (select doc_number,tgl_penerimaan,nama_supp,total_amount, status, CONCAT(created_by,' (',created_date,')') create_user,updated_at from ir_invoice_supp_h where status = 'Post Acc To Pch' and nama_supp = '$nama_supp') a INNER JOIN (select nama_trans,no_trans,tgl_trans,doc_number doc_num from ir_trans_invoice_supp where status = 'post' and nama_trans = 'TATP' GROUP BY doc_number and nama_supp = '$nama_supp') b on b.doc_num = a.doc_number group by no_trans");
            }
...
<div class="box footer">   
        <form id="form-simpan">
           <div class="form-row col">
            ...
            <button type="button" style="border-radius: 6px" class="btn-outline-warning" name="batal" id="batal" onclick="location.href='document_handover.php?type=invoice'"><span class="fa fa-angle-double-left"></span> Back</button>         
            </div>
            </div>                                   
        </form>
        </div>        
    ...
    $("#form-simpan").on("click", "#approve", function(){
        ...
        $.ajax({
            ...
            success: function(response){                
                ...
                window.location = 'form_approve_pch.php';
...
<script type="text/javascript">
     $("#form-simpan").on("click", "#approve", function(){
        ...
        var no_dok = $(this).closest('tr').find('td:eq(1)').attr('value');
        var no_kbon = $(this).closest('tr').find('td:eq(3)').attr('value');
        var filter = $(this).closest('tr').find('td:eq(8)').attr('value');
        var approve_user = '<?php echo $user ?>';

        $.ajax({
            type:'POST',
            url:'cancel_acctopch.php',
            data: {'no_dok':no_dok, 'no_kbon':no_kbon, 'filter':filter, 'approve_user':approve_user},
            close: function(e){
                e.preventDefault();
            },
            success: function(response){                
