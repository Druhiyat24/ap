...
<div class="container-fluid mt-4 p-4">
  <div class="card shadow border-0">
    ...
    <h5 class="mb-0"><i class="fas fa-thumbs-up"></i> APPROVAL REVERSE PAYMENT LIST <span class="badge bg-danger ms-2">
        <i class="fas fa-bell"></i>
        <?php
        $sql = mysqli_query($conn2,"SELECT COUNT(id) jml FROM ap_reverse_h WHERE rvs_number LIKE 'RVS/PL/%' AND status = 'DRAFT'");
...
<div class="card shadow border-0 mt-4">
    <div class="card-body p-4">
      ...
      <tbody>
        ...
        $sql = mysqli_query($conn2,"SELECT rvs_number, rvs_date, deskripsi, created_by, created_date FROM ap_reverse_h WHERE status = 'DRAFT' AND rvs_number LIKE 'RVS/PL/%'");
...
$("#form-simpan").on("click", "#approve", function(e){
    ...
    selected.each(function () {
        ...
        $.ajax({
            type: 'POST',
            url: 'approve_reverse_payment_list.php',
            data: data,
                ...
                if (processed === total) {
                    if (errors === 0) {
                        Swal.fire({ icon: 'success', title: 'Success!', text: total + ' reverse(s) approved successfully' }).then(() => {
                            window.location = 'form_approve_reverse_payment_list.php';
                        });
                    } else {
