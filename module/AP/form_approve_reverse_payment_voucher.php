...
<style type="text/css">
    ...
    #mytable tbody tr td:first-child { cursor: pointer; color: #1d4ed8; }
</style>
...
<div class="container-fluid mt-4 p-4">
  <div class="card shadow border-0">
    ...
    <h5 class="mb-0"><i class="fas fa-thumbs-up"></i> APPROVAL REVERSE PAYMENT VOUCHER
        <span class="badge bg-danger ms-2">
            <i class="fas fa-bell"></i>
            <?php
            $sql = mysqli_query($conn2,
                "SELECT COUNT(id) jml FROM ap_reverse_h
...
<div class="card shadow border-0 mt-4">
    <div class="card-body p-4">
      ...
      <tbody>
        ...
        $sql = mysqli_query($conn2,
            "SELECT rvs_number, rvs_date, deskripsi, created_by, created_date
             FROM ap_reverse_h
             WHERE status = 'DRAFT'
             AND rvs_number LIKE 'RVS/PV/%'
             AND rvs_number NOT LIKE 'RVS/PVL/%'
...
$("#form-simpan").on("click", "#approve", function(e){
    ...
    Swal.fire({
        title: 'Are you sure?',
        text: 'Selected reverse PV(s) will be approved and PVs will be reset to draft!',
        icon: 'warning',
        ...
        selected.each(function () {
            ...
            $.ajax({
                type: 'POST',
                url: 'approve_reverse_pv_doc.php',
                data: data,
                    ...
                    if (processed === total) {
                        if (errors === 0) {
                            Swal.fire({ icon: 'success', title: 'Success!', text: total + ' reverse(s) approved successfully' }).then(() => {
                                window.location = 'form_approve_reverse_payment_voucher.php';
                            });
                        } else {
...
$("#form-simpan").on("click", "#cancel", function(e){
    ...
    Swal.fire({
        title: 'Are you sure?',
        text: 'Selected reverse PV(s) will be cancelled!',
        icon: 'warning',
        showCancelButton: true,
        ...
        selected.each(function () {
            ...
            $.ajax({
                ...
                success: function(response){
                    if (response.indexOf('Error') !== -1) {
                        errors++;
                        console.error("Cancel error:", response);
                    }
                },
                error: function(xhr){ console.error(xhr.responseText); errors++; },
                complete: function(){
...
<script type="text/javascript">
$('#mytable tbody').on('click', 'td:eq(0)', function(){
