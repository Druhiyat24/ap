                    ...
                    <div class="table-responsive">
                        <table id="mytablenone" class="table table-striped table-bordered" cellspacing="0" width="100%" style="font-size: 12px;text-align:center;">
                            ...
                            <tbody id="tbody2">
                                <?php
                                $doc_num = base64_decode($_GET['doc_num']); 
                                $sql_none = mysql_query("select no_bankout no_doc, a.no_coa id_coa, no_costcntr id_cost_center, buyer, no_ws, curr, debit t_debit, credit t_credit, a.deskripsi keterangan, a.profit_center, concat(b.no_coa,' ', b.nama_coa) as nama_coa,CONCAT(a.no_costcntr,' - ',d.cc_name) cc_name, CONCAT(mp.id_pc,' - ',nama_pc) nama_pc from b_bankout_none a left join mastercoa_v2 b on b.no_coa = a.no_coa left join b_master_cc d on d.no_cc = a.no_costcntr LEFT JOIN master_pc mp on mp.kode_pc = a.profit_center where no_bankout = '$doc_num'",$conn1);
