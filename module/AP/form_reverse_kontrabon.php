...
<div class="container-fluid mt-4 p-4">
  <div class="card border-secondary mb-3">
    <div class="card-header text-left" style="background: linear-gradient(90deg, #191970, #1e90ff);">
        <h5 class="mb-0 text-white" ><i class="fab fa-wpforms"></i> FORM REVERSE KONTRABON</h5>
    </div>
    <form id="form-data" method="post">
        <div class="form-row mt-2">
            <div class="col-md-3 mb-3">            
                ...
                <?php
                $sql = mysqli_query($conn2,"select CONCAT('RVS/SI/',DATE_FORMAT(CURRENT_DATE(), '%m%y'),'/',LPAD((COALESCE(max(SUBSTR(rvs_number,13)),0) + 1),5,0)) nomor from ap_reverse_h WHERE rvs_number like '%SI%' and YEAR(rvs_date) = YEAR (CURRENT_DATE())");
        ...
        <div class="table-responsive p-1">
            <table id="mytable" class="table table-striped table-bordered" cellspacing="0" width="100%" style="font-size: 13px;;text-align:center;">
                ...
                <tbody>
                    ...
                    if ($doc_type === 'KONTRABON REGULAR') {
                        if ($nama_supp == 'ALL') {
                            $sql = mysqli_query($conn2,"select a.* from (SELECT no_kbon, tgl_kbon, nama_supp, curr, total, status_closing, c.nama_pc FROM kontrabon_h a LEFT JOIN tbl_closing_periode b on a.tgl_kbon BETWEEN b.tgl_awal AND b.tgl_akhir left join master_pc c on c.kode_pc = a.profit_center where tgl_kbon BETWEEN '$start_date' and '$end_date' and a.status = 'Approved' GROUP BY no_kbon) a LEFT JOIN
                                (select doc_number from ap_reverse_h a INNER JOIN ap_reverse_det b on b.rvs_number = a.rvs_number where a.status = 'DRAFT') b on b.doc_number = a.no_kbon where b.doc_number is null");
                        }else{
                            $sql = mysqli_query($conn2,"select a.* from (SELECT no_kbon, tgl_kbon, nama_supp, curr, total, status_closing, c.nama_pc FROM kontrabon_h a LEFT JOIN tbl_closing_periode b on a.tgl_kbon BETWEEN b.tgl_awal AND b.tgl_akhir left join master_pc c on c.kode_pc = a.profit_center where tgl_kbon BETWEEN '$start_date' and '$end_date' and nama_supp = '$nama_supp' and a.status = 'Approved' GROUP BY no_kbon) a LEFT JOIN
                                (select doc_number from ap_reverse_h a INNER JOIN ap_reverse_det b on b.rvs_number = a.rvs_number where a.status = 'DRAFT') b on b.doc_number = a.no_kbon where b.doc_number is null");

                        }
                    }elseif($doc_type == 'KONTRABON CBD'){
                        if ($nama_supp == 'ALL') {
        ...
        $.ajax({
            ...
            success: function (response) {
                ...
                Swal.fire({
                    ...
                    window.location = 'reverse_kontrabon.php';
                });
