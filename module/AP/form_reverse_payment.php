...
<div class="container-fluid mt-4 p-4">
  <div class="card border-secondary mb-3">
    <div class="card-header text-left" style="background: linear-gradient(90deg, #191970, #1e90ff);">
        <h5 class="mb-0 text-white" ><i class="fab fa-wpforms"></i> FORM REVERSE PAYMENT</h5>
    </div>
    <form id="form-data" method="post">
        <div class="form-row mt-2">
            <div class="col-md-3 mb-3">            
                ...
                <?php
                $sql = mysqli_query($conn2,"select CONCAT('RVS/PAY/',DATE_FORMAT(CURRENT_DATE(), '%m%y'),'/',LPAD((COALESCE(max(SUBSTR(rvs_number,14)),0) + 1),5,0)) nomor from ap_reverse_h WHERE rvs_number like '%PAY%' and YEAR(rvs_date) = YEAR (CURRENT_DATE())");
...
<!-- <div class="col-md-3 mb-3">            
    ...
    <div class="col-md-5 mb-3">
        <label for="txt_supp"><b>Supplier</b></label>
        ...
        <div class="table-responsive p-1">
            <table id="mytable" class="table table-striped table-bordered" cellspacing="0" width="100%" style="font-size: 13px;;text-align:center;">
                ...
                <tbody>
                    ...
                    if ($doc_type === 'PAYMENT REGULAR') {
                        if ($nama_supp == 'ALL') {
                            $sql = mysqli_query($conn2,"select a.* from (select payment_ftr_id, tgl_pelunasan, nama_supp, valuta_ftr curr, sum(ttl_bayar) total, status_closing, c.nama_pc from payment_ftr a LEFT JOIN tbl_closing_periode b on a.tgl_pelunasan BETWEEN b.tgl_awal AND b.tgl_akhir left join master_pc c on c.kode_pc = a.profit_center where tgl_pelunasan BETWEEN '$start_date' and '$end_date' and a.status = 'Approved' GROUP BY payment_ftr_id) a LEFT JOIN
                                (select doc_number from ap_reverse_h a INNER JOIN ap_reverse_det b on b.rvs_number = a.rvs_number where a.status = 'DRAFT') b on b.doc_number = a.payment_ftr_id where b.doc_number is null");
                        }else{
                            $sql = mysqli_query($conn2,"select a.* from (select payment_ftr_id, tgl_pelunasan, nama_supp, valuta_ftr curr, sum(ttl_bayar) total, status_closing, c.nama_pc from payment_ftr a LEFT JOIN tbl_closing_periode b on a.tgl_pelunasan BETWEEN b.tgl_awal AND b.tgl_akhir left join master_pc c on c.kode_pc = a.profit_center where tgl_pelunasan BETWEEN '$start_date' and '$end_date' and a.status = 'Approved' and nama_supp = '$nama_supp' GROUP BY payment_ftr_id) a LEFT JOIN
                                (select doc_number from ap_reverse_h a INNER JOIN ap_reverse_det b on b.rvs_number = a.rvs_number where a.status = 'DRAFT') b on b.doc_number = a.payment_ftr_id where b.doc_number is null");

                        }
                    }elseif($doc_type == 'PAYMENT CBD'){
                        if ($nama_supp == 'ALL') {
                            $sql = mysqli_query($conn2,"select payment_ftr_id, tgl_pelunasan, nama_supp, valuta_ftr curr, sum(ttl_bayar) total, status_closing, '' nama_pc from payment_ftrcbd a LEFT JOIN tbl_closing_periode b on a.tgl_pelunasan BETWEEN b.tgl_awal AND b.tgl_akhir where tgl_pelunasan BETWEEN '$start_date' and '$end_date' and status = 'Approved' GROUP BY payment_ftr_id");
                        }else{
                            $sql = mysqli_query($conn2,"select payment_ftr_id, tgl_pelunasan, nama_supp, valuta_ftr curr, sum(ttl_bayar) total, status_closing, '' nama_pc from payment_ftrcbd a LEFT JOIN tbl_closing_periode b on a.tgl_pelunasan BETWEEN b.tgl_awal AND b.tgl_akhir where tgl_pelunasan BETWEEN '$start_date' and '$end_date' and status = 'Approved' and nama_supp = '$nama_supp' GROUP BY payment_ftr_id");
                    ...
                    while($row = mysqli_fetch_array($sql)){ 
                        ...
                        <td style="width:50px;" value="'.$row['payment_ftr_id'].'">'.$row['payment_ftr_id'].'</td>
                        <td style="width:100px;" value="'.$row['tgl_pelunasan'].'">'.date("d-M-Y",strtotime($row['tgl_pelunasan'])).'</td>
                        <td style="width:50px;" value="'.$row['nama_supp'].'">'.$row['nama_supp'].'</td>
        ...
        <form id="form-simpan">
            <div class="form-row col mt-3">
                <div class="col-md-3 mb-3">                              
                    ...
                    <button type="button" style="border-radius: 6px" class="btn-danger btn-sm" name="batal" id="batal" onclick="location.href='reverse_payment.php'"><span class="fa fa-angle-double-left"></span> Back</button>           
                </div>
            </div>                                    
        </form>
    </div>
        ...
        $.ajax({
            ...
            success: function (response) {
                ...
                Swal.fire({
                    ...
                    window.location = 'reverse_payment.php';
                });
