...
<div class="container-fluid mt-4 p-4">
  <div class="card border-secondary mb-3">
    <div class="card-header text-left" style="background: linear-gradient(90deg, #191970, #1e90ff);">
        <h5 class="mb-0 text-white"><i class="fab fa-wpforms"></i> FORM REVERSE PAYMENT LIST</h5>
    </div>
    ...
    <form id="form-header" method="post">
        <div class="form-row mt-2">
            ...
            <div class="col-md-3 mb-3">
                ...
                <?php
                $sql = mysqli_query($conn2,"SELECT CONCAT('RVS/PL/', DATE_FORMAT(CURRENT_DATE(), '%m%y'), '/', LPAD(COALESCE(MAX(CAST(RIGHT(rvs_number, 5) AS UNSIGNED)), 0) + 1, 5, '0')) nomor FROM ap_reverse_h WHERE rvs_number LIKE CONCAT('RVS/PL/', DATE_FORMAT(CURRENT_DATE(), '%m%y'), '/%')");
                        ...
                        if (!empty($start_date) && !empty($end_date)) {
                            $sql = mysqli_query($conn2,"SELECT h.pl_number, h.pl_date, h.deskripsi, h.from_account,
                                ...
                                AND h.pl_number NOT IN (
                                    ...
                                    WHERE rh.status = 'DRAFT' AND rh.rvs_number LIKE 'RVS/PL/%'
                                )");

                            while ($row = mysqli_fetch_array($sql)) {
                                $plDate   = (!empty($row['pl_date']) && $row['pl_date'] != '0000-00-00') ? date("d-M-Y", strtotime($row['pl_date'])) : '-';
                                $isPaid   = (int)$row['paid_count'] > 0;
                                $rowStyle = $isPaid ? ' style="background:#fef9f9;"' : '';
                                $cbAttr   = $isPaid ? ' disabled title="This PL has PVs already paid via Bank Out"' : '';
                                $paidNote = $isPaid ? '<br><span style="color:#dc2626;font-size:10px;font-weight:bold;">&#9888; Some PV(s) already paid</span>' : '';
                                $taAttr   = $isPaid ? ' disabled' : '';

                                echo '<tr'.$rowStyle.'>
                                    <td style="width:10px;text-align:center;"><input type="checkbox" name="select[]" value=""'.$cbAttr.'></td>
                                    <td value="'.htmlspecialchars($row['pl_number']).'" class="td-pl-number" style="cursor:pointer;color:#1d4ed8;">'.htmlspecialchars($row['pl_number']).$paidNote.'</td>
                                    <td value="'.htmlspecialchars($row['pl_date']).'">'.$plDate.'</td>
                                    <td style="text-align:left;" value="'.htmlspecialchars($row['from_account_label'] ?? '').'">'.htmlspecialchars($row['from_account_label'] ?? '-').'</td>
            ...
            <form id="form-simpan">
                <div class="form-row col mt-3">
                    <div class="col-md-3 mb-3">
                        ...
                        <button type="button" style="border-radius:6px;" class="btn-danger btn-sm" name="batal" id="batal" onclick="location.href='reverse_payment_list.php'"><span class="fa fa-angle-double-left"></span> Back</button>
                    </div>
                </div>
            </form>
        </div>
  ...
  <div class="modal-dialog modal-lg">
    <div class="modal-content shadow-lg rounded-3">
      <div class="modal-header text-white" style="background-color: #191970;">
        ...
        <h4 class="modal-title text-white">Search Payment List</h4>
      </div>
      <div class="modal-body">
        <form id="modal-form" method="post">
          <div class="form-group">
            <label><b>PL Date</b></label>
...
<!-- Modal Detail PV per PL -->
<div class="modal fade" id="modal-pl-detail" tabindex="-1" role="dialog" aria-hidden="true">
  <div class="modal-dialog modal-xl modal-dialog-centered">
    <div class="modal-content">
      <div class="modal-header text-white" style="background-color: #1E3A8A;">
        <button type="button" class="close" data-dismiss="modal" aria-hidden="true"><span class="fa fa-times"></span></button>
        <h4 class="modal-title" id="modal_pl_number"></h4>
      </div>
      <div class="container-fluid">
        <div class="row">
          <div id="modal_pl_date" class="modal-body col-md-4" style="font-size:12px;padding:0.5rem;"></div>
          <div id="modal_pl_status" class="modal-body col-md-4" style="font-size:12px;padding:0.5rem;"></div>
          <div id="modal_pl_deskripsi" class="modal-body col-md-4" style="font-size:12px;padding:0.5rem;"></div>
...
// Klik No PL → tampil detail PV
$('#mytable tbody').on('click', 'td.td-pl-number', function() {
    var pl_number = $(this).attr('value');
    var pl_date   = $(this).closest('tr').find('td:eq(2)').text();
    var deskripsi = $(this).closest('tr').find('td:eq(4)').text();
    var status    = $(this).closest('tr').find('td:eq(5)').text();

    $('#modal_pl_number').html(pl_number);
    $('#modal_pl_date').html('<b>PL Date:</b> ' + pl_date);
    $('#modal_pl_status').html('<b>Status:</b> ' + status);
    $('#modal_pl_deskripsi').html('<b>Description:</b> ' + deskripsi);
    $('#modal_pl_tbody').html('<tr><td colspan="7" style="text-align:center;"><i class="fa fa-spinner fa-spin"></i> Loading...</td></tr>');
    $('#modal-pl-detail').modal('show');

    $.ajax({
        type: 'POST',
        url: 'get_pv_detail_for_reverse.php',
        data: { pl_number: pl_number },
        success: function(data) {
            $('#modal_pl_tbody').html(data);
        },
        error: function(xhr) {
            $('#modal_pl_tbody').html('<tr><td colspan="7" style="color:red;">Error: ' + xhr.responseText + '</td></tr>');
        }
    });
});
</script>
...
$("#form-simpan").on("click", "#simpan", function () {
    ...
    if (checked.length === 0) {
        Swal.fire({ icon: 'warning', title: 'Oops...', text: 'Please check at least one Payment List.' });
        return;
    }

    $.ajax({
        type: 'POST',
        url: 'insert_reverse_pl_h.php',
        data: {
                    ...
                    complete: function () {
                        ...
                        if (processed === total) {
                            Swal.fire({
                                ...
                                window.location = 'reverse_payment_list.php';
                            });
                        }
                    }
                });
