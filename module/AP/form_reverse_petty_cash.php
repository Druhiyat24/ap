...
<div class="container-fluid mt-4 p-4">
  <div class="card border-secondary mb-3">
    <div class="card-header text-left" style="background: linear-gradient(90deg, #191970, #1e90ff);">
        <h5 class="mb-0 text-white" ><i class="fab fa-wpforms"></i> FORM REVERSE PETTY CASH</h5>
    </div>
    <form id="form-data" method="post">
        <div class="form-row mt-2">
            <div class="col-md-3 mb-3">            
                ...
                <?php
                $sql = mysqli_query($conn2,"select CONCAT('RVS/PC/',DATE_FORMAT(CURRENT_DATE(), '%m%y'),'/',LPAD((COALESCE(max(SUBSTR(rvs_number,13)),0) + 1),5,0)) nomor from ap_reverse_h WHERE rvs_number like '%PC%' and YEAR(rvs_date) = YEAR (CURRENT_DATE())");
...
<!-- <div class="col-md-3 mb-3">            
    ...
    <div class="col-md-3 mb-3">
        <label for="txt_supp"><b>Search Data</b></label>
        ...
        <div class="table-responsive p-1">
            <table id="mytable" class="table table-striped table-bordered" cellspacing="0" width="100%" style="font-size: 13px;;text-align:center;">
                ...
                <tbody>
                    ...
                    if ($doc_type === 'PETTY CASH IN') {
                            $sql = mysqli_query($conn2,"select a.* from (select no_pci no_cash, tgl_pci cash_date, '-' customer, curr, amount, status_closing, 'NIRWANA ALABARE GARMENT' nama_pc from c_petty_cashin_h a LEFT JOIN tbl_closing_periode b on a.tgl_pci BETWEEN b.tgl_awal AND b.tgl_akhir where a.tgl_pci BETWEEN '$start_date' and '$end_date' and a.status = 'Approved' GROUP BY no_pci) a LEFT JOIN
                                (select doc_number from ap_reverse_h a INNER JOIN ap_reverse_det b on b.rvs_number = a.rvs_number where a.status = 'DRAFT') b on b.doc_number = a.no_cash where b.doc_number is null");
                    }else{
                        $sql = mysqli_query($conn2,"select a.* from (select no_pco no_cash, tgl_pco cash_date, '-' customer, curr, amount, status_closing, 'NIRWANA ALABARE GARMENT' nama_pc from c_petty_cashout_h a LEFT JOIN tbl_closing_periode b on a.tgl_pco BETWEEN b.tgl_awal AND b.tgl_akhir where a.tgl_pco BETWEEN '$start_date' and '$end_date' and a.status = 'Approved' GROUP BY no_pco) a LEFT JOIN
                                (select doc_number from ap_reverse_h a INNER JOIN ap_reverse_det b on b.rvs_number = a.rvs_number where a.status = 'DRAFT') b on b.doc_number = a.no_cash where b.doc_number is null");
                    }
                    ...
                    while($row = mysqli_fetch_array($sql)){ 
                        ...
                        <td style="width:50px;" value="'.$row['no_cash'].'">'.$row['no_cash'].'</td>
                        <td style="width:100px;" value="'.$row['cash_date'].'">'.date("d-M-Y",strtotime($row['cash_date'])).'</td>
                        <td style="display: none;" value="'.$row['customer'].'">'.$row['customer'].'</td>
        ...
        <form id="form-simpan">
            <div class="form-row col mt-3">
                <div class="col-md-3 mb-3">                              
                    ...
                    <button type="button" style="border-radius: 6px" class="btn-danger btn-sm" name="batal" id="batal" onclick="location.href='reverse_petty_cash.php'"><span class="fa fa-angle-double-left"></span> Back</button>           
                </div>
            </div>                                    
        </form>
    </div>
  ...
  <div class="modal-dialog modal-lg"> <!-- kasih modal-lg biar lega -->
    ...
    <div class="modal-body">
        <form id="modal-form" method="post">
          ...
          <!-- Supplier -->
            <input type="hidden" style="font-size: 13px;" name="nama_supp" id="nama_supp" class="form-control form-control-sm form-control form-control-sm-sm" value="ALL">
        ...
        <div class="form-group">
            <label><b>Petty Cash Date</b></label>
        ...
        $.ajax({
            ...
            success: function (response) {
                ...
                Swal.fire({
                    ...
                    window.location = 'reverse_petty_cash.php';
                });
