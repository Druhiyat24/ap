...
<!-- MAIN -->    
<div class="col p-4">
    <h2 class="text-center">VERIFIKASI BPB KNITTING</h2>
    <div class="box">
        <div class="box header">
            <form id="form-data" action="formverifikasibpb_knitting.php" method="post">
                <div class="form-row">
                    <div class="col-md-6">
    ...
    <div class="tableFix" style="height: 400px;">
        <table id="mytable" class="table table-head-fixed text-nowrap" cellspacing="0" width="100%" style="font-size: 13px;text-align:center;">
            ...
            <tbody>
                ...
                if($nama_supp == 'ALL'){
                    $sql = pg_query($conn4, "(select no_bpb bpbno_int, tanggal_bpb bpbdate, c.no_po pono, nama_supplier supplier, COALESCE(ROUND(term_of_payment, 0)::TEXT, '-') jml_pterms, currency curr, a.confirm_administrasi confirm_by, c.tanggal podate, '-' kode_pterms from bpb a inner join master_supplier b on b.id = a.supplier_id inner join purchase_orders c on c.id = a.no_po where status_bpb = 'approved' and a.tanggal_bpb between '$start_date' and '$end_date' order by a.id asc)
                        UNION
                        (select no_bpb bpbno_int, tgl_bpb bpbdate, c.no_po pono, nama_supplier supplier, COALESCE(ROUND(term_of_payment, 0)::TEXT, '-') jml_pterms, currency curr, a.confirm_administrasi confirm_by, c.tanggal podate, '-' kode_pterms from gp_penerimaan_greige_h a inner join master_supplier b on b.id = a.id_supplier_bpb inner join purchase_orders c on c.id = a.id_purchase_order_bpb where status_bpb = 'approved' and tipe_greige = 'subkon' and tgl_bpb between '$start_date' and '$end_date' order by a.id asc)
                        UNION
                        (select no_bpb bpbno_int, tanggal_bpb bpbdate, c.no_po pono, nama_supplier supplier, COALESCE(ROUND(term_of_payment, 0)::TEXT, '-') jml_pterms, currency curr, a.confirm_administrasi confirm_by, c.tanggal podate, '-' kode_pterms from bpb_celup a inner join master_supplier b on b.id = a.id_supplier_bpb inner join purchase_orders c on c.id = a.id_purchase_order_bpb where status_bpb = 'approved' and tanggal_bpb between '$start_date' and '$end_date' order by a.id asc)");                
                }else {
                    $sql = pg_query($conn4, "(select no_bpb bpbno_int, tanggal_bpb bpbdate, c.no_po pono, nama_supplier supplier, COALESCE(ROUND(term_of_payment, 0)::TEXT, '-') jml_pterms, currency curr, a.confirm_administrasi confirm_by, c.tanggal podate, '-' kode_pterms from bpb a inner join master_supplier b on b.id = a.supplier_id inner join purchase_orders c on c.id = a.no_po where status_bpb = 'approved' and nama_supplier='$nama_supp' and a.tanggal_bpb between '$start_date' and '$end_date' order by a.id asc)
                        UNION
                        (select no_bpb bpbno_int, tgl_bpb bpbdate, c.no_po pono, nama_supplier supplier, COALESCE(ROUND(term_of_payment, 0)::TEXT, '-') jml_pterms, currency curr, a.confirm_administrasi confirm_by, c.tanggal podate, '-' kode_pterms from gp_penerimaan_greige_h a inner join master_supplier b on b.id = a.id_supplier_bpb inner join purchase_orders c on c.id = a.id_purchase_order_bpb where status_bpb = 'approved' and tipe_greige = 'subkon' and nama_supplier='$nama_supp' and tgl_bpb between '$start_date' and '$end_date' order by a.id asc)
                        UNION
                        (select no_bpb bpbno_int, tanggal_bpb bpbdate, c.no_po pono, nama_supplier supplier, COALESCE(ROUND(term_of_payment, 0)::TEXT, '-') jml_pterms, currency curr, a.confirm_administrasi confirm_by, c.tanggal podate, '-' kode_pterms from bpb_celup a inner join master_supplier b on b.id = a.id_supplier_bpb inner join purchase_orders c on c.id = a.id_purchase_order_bpb where status_bpb = 'approved' and tanggal_bpb between '$start_date' and '$end_date' and nama_supplier='$nama_supp' order by a.id asc)");
                }
  ...
  <div class="box footer">   
    <form id="form-simpan">
     <div class="form-row col">
        ...
        <button type="button" style="border-radius: 6px" class="btn-outline-danger btn-sm" name="batal" id="batal" onclick="location.href='verifikasibpb_knitting.php'"><span class="fa fa-angle-double-left"></span> Back</button>               
    </div>
</div>                                   
</form>
</div>        
