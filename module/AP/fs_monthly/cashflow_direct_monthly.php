...
/* Class-class di bawah ini (laporan-outer, laporan-container, sfp-title-*,
   ...
   / cashflow_indirect_monthly.php - lihat catatan lengkap di file SFP untuk
   alasan tiap aturan (freeze scroll, lebar tabel eksplisit, dst). ID elemen
   dibedakan (prefix cfdirect- bukan sfp-/spl-/cfindirect-) karena ID wajib
   unik satu halaman. */
.laporan-outer {
...
#cfdirect-header-table tr:last-child th {
...
#cfdirect-monthly-table tr:has(td):hover td {
  background-color: rgba(37, 99, 235, 0.1);
}

#cfdirect-monthly-table tr:has(td) > td {
...
// (lihat catatan lengkap di fs_monthly_functions.php - bagian "Helper
// khusus Cash Flow Direct" - untuk penjelasan query & 3 perbaikan bug yang
// disepakati)

// --- Arus Kas dari Aktivitas Operasi ---
$operasiMonthData = [];
foreach ($periods as $p) {
    $operasiMonthData[$p['label']] = fsGetCashflowDirectOperasi($conn2, $bulan_awal, $tahun_awal, $p['bulan'], $p['tahun']);
}
$operasi = fsMonthlyAggregateSubKategori($operasiMonthData, $periods, $pcCols);

// --- Arus Kas dari Aktivitas Investasi ---
$investasiMonthData = [];
foreach ($periods as $p) {
    $investasiMonthData[$p['label']] = fsGetCashflowDirectFlat($conn2, 'Arus Kas dari Aktivitas Investasi', '', $bulan_awal, $tahun_awal, $p['bulan'], $p['tahun']);
}
$investasi = fsMonthlyAggregateSubKategori($investasiMonthData, $periods, $pcCols);

// --- Arus Kas dari Aktivitas Pendanaan (Pinjaman + antar divisi) ---
$pendanaanLoanMonthData = [];
$pendanaanAntarDivisiMonthData = [];
foreach ($periods as $p) {
    $pendanaanLoanMonthData[$p['label']] = fsGetCashflowDirectLoans($conn2, $bulan_awal, $tahun_awal, $p['bulan'], $p['tahun']);
    $pendanaanAntarDivisiMonthData[$p['label']] = fsGetCashflowDirectFlat($conn2, 'Arus Kas dari Aktivitas Pendanaan', ' and id not in (14,15)', $bulan_awal, $tahun_awal, $p['bulan'], $p['tahun']);
}
$pendanaanLoan = fsMonthlyAggregateSubKategori($pendanaanLoanMonthData, $periods, $pcCols);
$pendanaanAntarDivisi = fsMonthlyAggregateSubKategori($pendanaanAntarDivisiMonthData, $periods, $pcCols);

// Subtotal Pendanaan yang DITAMPILKAN = jumlah SEMUA baris di atas (Pinjaman
// + antar divisi) - perbaikan #3 (di versi YTD subtotal yang dicetak cuma
// baris Pinjaman, tidak termasuk antar divisi walau ditampilkan di atasnya
// dan tetap dijumlah ke grand total).
$pendanaanTotal = [];
foreach ($periods as $p) {
    foreach ($pcCols as $pcCol) {
        $pendanaanTotal[$p['label']][$pcCol] =
            ($pendanaanLoan['sectionTotal'][$p['label']][$pcCol] ?? 0)
            + ($pendanaanAntarDivisi['sectionTotal'][$p['label']][$pcCol] ?? 0);
    }
}

// --- Kenaikan / (Penurunan) Bersih Kas dan Setara Kas - murni PHP ---
$netChangeValues = [];
foreach ($periods as $p) {
    foreach ($pcCols as $pcCol) {
        $netChangeValues[$p['label']][$pcCol] =
            ($operasi['sectionTotal'][$p['label']][$pcCol] ?? 0)
            + ($investasi['sectionTotal'][$p['label']][$pcCol] ?? 0)
            + ($pendanaanTotal[$p['label']][$pcCol] ?? 0);
    }
}

// --- Kas dan Setara Kas pada Awal Periode - konstan di semua kolom bulan
// (mekanisme SAMA PERSIS dengan fsGetCashflowBeginningCash() yang dipakai
// CF Indirect Monthly - lihat $sql5 di fs_ytd/cashflow_direct.php, query-nya
// identik dgn $sql5 di fs_ytd/cashflow_indirect.php).
...
<div class="table-responsive mt-1">
  <div class="laporan-outer">
    <div class="sfp-title-block">
      ...
      <button type="button" id="btnExcel-cfdirect" class="sfpm-btn-export-excel">
    ...
    <div class="sfp-header-clip">
      <table id="cfdirect-header-table" class="laporan-table sfpm-tbl" border="0" cellspacing="0" style="width:<?= sfpmTableWidth($valueColCount); ?>px">
    ...
    <div class="laporan-container">
      <table id="cfdirect-monthly-table" class="laporan-table sfpm-tbl" border="0" role="grid" cellspacing="0" style="width:<?= sfpmTableWidth($valueColCount); ?>px">
        <?= sfpmColgroup($valueColCount); ?>
        <?php
        $renderSection = function ($sec, $label, $labelEn, $totalLabel, $totalLabelEn, $periods, $pcCols, $totalValues = null) {
            echo '<tr data-en="' . htmlspecialchars($labelEn) . '"><th class="subsection-left sfpm-freeze-left">' . htmlspecialchars($label) . '</th>'
               . sfpmBlankCells(count($periods) * count($pcCols) + count($pcCols), count($pcCols))
               . '</tr>';
            ...
            echo '<tr class="total-line" data-en="' . htmlspecialchars($totalLabelEn) . '"><th class="total-left sfpm-freeze-left">' . htmlspecialchars($totalLabel) . '</th>'
               . sfpmRenderRowValues($totalValues ?? $sec['sectionTotal'], $periods, $pcCols, 'td', 'total-right')
               . '</tr>';

            sfpmSpacerRow(count($periods) * count($pcCols) + count($pcCols), count($pcCols), 'spacer-small');
        };

        $renderSection(
            $operasi,
            'ARUS KAS DARI AKTIVITAS OPERASI', 'CASH FLOW FROM OPERATING ACTIVITIES',
            'ARUS KAS YANG DIGUNAKAN UNTUK AKTIVITAS OPERASI', 'CASH FLOW USED FROM OPERATING ACTIVITIES',
            $periods, $pcCols
        );
        ...
        $renderSection(
            ...
            'ARUS KAS YANG DIGUNAKAN UNTUK AKTIVITAS INVESTASI', 'CASH FLOW USED FROM INVESTING ACTIVITIES',
            $periods, $pcCols
        );

        // Section Pendanaan: baris Pinjaman (Penerimaan/Pembayaran) digabung
        // dengan baris antar divisi dalam 1 section, subtotalnya jumlah
        // KEDUANYA (perbaikan #3).
        echo '<tr data-en="CASH FLOW FROM FINANCING ACTIVITIES"><th class="subsection-left sfpm-freeze-left">ARUS KAS DARI AKTIVITAS PENDANAAN</th>'
           . sfpmBlankCells($valueColCount, $pcColCount)
           . '</tr>';

        foreach ($pendanaanLoan['subList'] as $subKategori => $subKategoriEn) {
            echo '<tr data-en="' . htmlspecialchars($subKategoriEn) . '"><td class="item-left sfpm-freeze-left">' . htmlspecialchars($subKategori) . '</td>'
               . sfpmRenderRowValues($pendanaanLoan['itemValuesBySub'][$subKategori], $periods, $pcCols, 'td', 'item-right')
               . '</tr>';
        }
        foreach ($pendanaanAntarDivisi['subList'] as $subKategori => $subKategoriEn) {
            echo '<tr data-en="' . htmlspecialchars($subKategoriEn) . '"><td class="item-left sfpm-freeze-left">' . htmlspecialchars($subKategori) . '</td>'
               . sfpmRenderRowValues($pendanaanAntarDivisi['itemValuesBySub'][$subKategori], $periods, $pcCols, 'td', 'item-right')
               . '</tr>';
        }

        echo '<tr class="total-line" data-en="CASH FLOW GENERATED FROM FINANCING ACTIVITIES"><th class="total-left sfpm-freeze-left">ARUS KAS YANG DIPEROLEH DARI AKTIVITAS PENDANAAN</th>'
           . sfpmRenderRowValues($pendanaanTotal, $periods, $pcCols, 'td', 'total-right')
           . '</tr>';
...
<script>
  ...
  // cashflow_indirect_monthly.php - cuma ID elemen yang beda (cfdirect-
  // bukan sfp-/spl-/cfindirect-).
  (function () {
