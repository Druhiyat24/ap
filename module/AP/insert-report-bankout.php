...
if($query_del) {
	...
	if($query) {
		...
		$query2 = mysqli_query($conn2,$sql2);
			...
			(select a.id, no_bankout, bankout_date, 'Payment Voucher' type_journal, d.no_coa coa, d.nama_coa, a.no_cc, b.cc_name,ob.no_reff, ob.reff_date,  '-' buyer, '-' no_ws, curr_pv, amount debit, ded_add credit, ob.status, deskripsi, ob.create_by, ob.create_date, ob.approve_by, ob.approve_date, a.profit_center from tbl_pv a left join b_master_cc b on b.no_cc = a.no_cc inner join (select b.no_bankout, b.bankout_date, c.curr curr_bk, b.status, b.create_by, b.create_date, b.approve_by, b.approve_date, a.no_reff, a.reff_date, a.curr curr_pv, a.profit_center FROM b_bankout_det a INNER JOIN b_bankout_h b on b.no_bankout = a.no_bankout INNER JOIN b_masterbank c on c.bank_account = b.akun where a.no_bankout = '$no_dok' GROUP BY no_reff) ob on ob.no_reff = a.no_pv INNER JOIN mastercoa_v2 d on d.no_coa = a.coa
			UNION
			select a.id, no_bankout, bankout_date, 'Payment Voucher' type_journal, d.no_coa, d.nama_coa, a.no_cc, b.cc_name,ob.no_reff, ob.reff_date,  '-' buyer, '-' no_ws, curr_pv, '0' debit, (amount * pph/100) credit, ob.status, deskripsi, ob.create_by, ob.create_date, ob.approve_by, ob.approve_date, a.profit_center from tbl_pv a left join b_master_cc b on b.no_cc = a.no_cc inner join (select b.no_bankout, b.bankout_date, c.curr curr_bk, b.status, b.create_by, b.create_date, b.approve_by, b.approve_date, a.no_reff, a.reff_date, a.curr curr_pv, a.profit_center FROM b_bankout_det a INNER JOIN b_bankout_h b on b.no_bankout = a.no_bankout INNER JOIN b_masterbank c on c.bank_account = b.akun where a.no_bankout = '$no_dok' GROUP BY no_reff) ob on ob.no_reff = a.no_pv INNER JOIN mtax d on d.idtax = a.id_pph
			UNION
