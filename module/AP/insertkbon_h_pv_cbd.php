...
// (table yang sama dipakai bersama oleh DP dan CBD, tidak ada table khusus CBD).
$no_coa = '-';
$nama_coa = '-';
if ($item_type !== '') {
...
) nomor from kontrabon_h_cbd WHERE YEAR(tgl_kbon) = YEAR('$tgl_kbon_h')");
$rowno = mysqli_fetch_array($sqlno);
$no_kbon_h = isset($rowno['nomor']) ? $rowno['nomor'] : $no_kbon_h;

$query = "INSERT INTO kontrabon_h_cbd (no_po, tgl_po, no_kbon, tgl_kbon, nama_supp, no_faktur, supp_inv, tgl_inv, tgl_tempo, subtotal, tax, pph, total, amount_update, balance, curr, post_date, update_date, status, profit_center, id_bank_account, create_user, create_date, from_account, from_bank, from_bank_curr, item_type, no_coa, nama_coa)
VALUES
	('$no_po_h', '$tgl_po_h', '$no_kbon_h', '$tgl_kbon_h', '$nama_supp_h', '$no_faktur_h', '$supp_inv_h', '$tgl_inv_h', '$tgl_tempo_h', '$sub_h', '$tax_h', '$pph_h', '$total_h', '$total_h', '$balance', '$curr_h', '$post_date', '$update_date', '$status', '$profit_center', $id_bank_account, '$create_user_h', '$create_date', '$from_account', '$from_bank', '$from_bank_curr', '$item_type', '$no_coa', '$nama_coa')";
