...
data_meals as (SELECT SUBSTR(a.tanggal,1,7) AS periode, a.tanggal, COALESCE((SELECT d.sub_dept_id FROM department_all d WHERE d.sub_dept_id = a.sub_dept LIMIT 1), 'Unknown') AS sub_dept_id, COALESCE((SELECT d.sub_dept_name FROM department_all d WHERE d.department_id = a.dept AND d.sub_dept_id = a.sub_dept LIMIT 1), 'No Sub Dept') AS sub_dept_name, 'NON STAFF' AS staff_non_staff, COALESCE(a.non_staff,0) AS jumlah_karyawan, 8000 AS harga, COALESCE(a.non_staff,0) * 8000 AS jumlah, a.keterangan AS shift FROM estimasi_anggaran_makan a WHERE a.tanggal BETWEEN '$tanggal' AND '$tanggal' 
UNION ALL 
SELECT SUBSTR(a.tanggal,1,7) AS periode, a.tanggal, COALESCE((SELECT d.sub_dept_id FROM department_all d WHERE d.sub_dept_id = a.sub_dept LIMIT 1), 'Unknown' ) AS sub_dept_id, COALESCE((SELECT d.sub_dept_name FROM department_all d WHERE d.department_id = a.dept AND d.sub_dept_id = a.sub_dept LIMIT 1), 'No Sub Dept' ) AS sub_dept_name, 'STAFF' AS staff_non_staff, COALESCE(a.staff,0) AS jumlah_karyawan, 10000 AS harga, COALESCE(a.staff,0) * 10000 AS jumlah, a.keterangan AS shift FROM estimasi_anggaran_makan a WHERE a.tanggal BETWEEN '$tanggal' AND '$tanggal' ORDER BY   tanggal,shift,   sub_dept_id, sub_dept_name, staff_non_staff),
