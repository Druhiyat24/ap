...
<table id="datatable" class="table table-striped table-bordered display nowrap" role="grid" cellspacing="0" width="100%">
    ...
    if(empty($start_date) and empty($end_date)){
     $sql = mysqli_query($conn2,"select * from (select * from (select id,no_journal, tgl_journal, type_journal, no_coa, nama_coa, CONCAT(no_coa,' ',nama_coa) coa, no_costcenter, nama_costcenter, reff_doc, reff_date, buyer, no_ws, curr, rate, debit, credit, debit_idr, credit_idr, status, keterangan, create_by, create_date, approve_by, approve_date, cancel_by, cancel_date, profit_center from tbl_list_journal where tgl_journal = '$date_now' and no_journal like '%GM/NAG%' and status = 'Post'
union
select id,no_journal, tgl_journal, type_journal, no_coa, nama_coa, CONCAT(no_coa,' ',nama_coa) coa, no_costcenter, nama_costcenter, reff_doc, reff_date, buyer, no_ws, curr, rate, debit, credit, debit_idr, credit_idr, status, keterangan, create_by, create_date, approve_by, approve_date, cancel_by, cancel_date, profit_center from tbl_list_journal where tgl_journal = '$date_now' and no_journal not like '%GM/NAG%' and no_journal not like '%KKK%'
union
select DISTINCT '' id,no_journal, tgl_journal, type_journal, no_coa, nama_coa, CONCAT(no_coa,' ',nama_coa) coa, no_costcenter, nama_costcenter, reff_doc, reff_date, buyer, no_ws, curr, rate, debit, credit, debit_idr, credit_idr, status, keterangan, create_by, create_date, approve_by, approve_date, cancel_by, cancel_date, profit_center from tbl_list_journal where tgl_journal = '$date_now' and no_journal like '%KKK%') a) a left join (select kode_pc, CONCAT(id_pc,' - ',nama_pc) nama_pc from master_pc where status = 'Active') b on b.kode_pc = a.profit_center");
    }
    else{
    $sql = mysqli_query($conn2,"select * from (select * from (select id,no_journal, tgl_journal, type_journal, no_coa, nama_coa, CONCAT(no_coa,' ',nama_coa) coa, no_costcenter, nama_costcenter, reff_doc, reff_date, buyer, no_ws, curr, rate, debit, credit, debit_idr, credit_idr, status, keterangan, create_by, create_date, approve_by, approve_date, cancel_by, cancel_date, profit_center from tbl_list_journal where tgl_journal between '$start_date' and '$end_date' and no_journal like '%GM/NAG%' and status = 'Post'
union
select id,no_journal, tgl_journal, type_journal, no_coa, nama_coa, CONCAT(no_coa,' ',nama_coa) coa, no_costcenter, nama_costcenter, reff_doc, reff_date, buyer, no_ws, curr, rate, debit, credit, debit_idr, credit_idr, status, keterangan, create_by, create_date, approve_by, approve_date, cancel_by, cancel_date, profit_center from tbl_list_journal where tgl_journal between '$start_date' and '$end_date' and no_journal not like '%GM/NAG%' and no_journal not like '%KKK%'
union
select DISTINCT '' id,no_journal, tgl_journal, type_journal, no_coa, nama_coa, CONCAT(no_coa,' ',nama_coa) coa, no_costcenter, nama_costcenter, reff_doc, reff_date, buyer, no_ws, curr, rate, debit, credit, debit_idr, credit_idr, status, keterangan, create_by, create_date, approve_by, approve_date, cancel_by, cancel_date, profit_center from tbl_list_journal where tgl_journal between '$start_date' and '$end_date' and no_journal like '%KKK%') a) a left join (select kode_pc, CONCAT(id_pc,' - ',nama_pc) nama_pc from master_pc where status = 'Active') b on b.kode_pc = a.profit_center");
}
...
WHERE  tbl_list_journal.create_by = '' and tbl_log.activity = 'Create invoice Manual'");
    ...
    $sqlinsert1 = mysqli_query($conn2," insert into tbl_list_journal (SELECT '' id, no_journal,tgl_journal, 'AP - Kontrabon' type_journal, '8.52.02' coa, 'LABA / (RUGI) SELISIH KURS BELUM TEREALISASI' nama_coa, 'DEP06SUB001' no_cc, 'FINANCE, ACCOUNTING & TAX' cc_name , '-' reff_doc, '' reff_date,'-' faktur_pajak, '' tgl_faktur, '-' buyer, '-' no_ws, 'IDR' curr, '1' rate, IF(amt_balance <= 0,ABS(amt_balance),0) debit ,IF(amt_balance > 0,amt_balance,0) credit, IF(amt_balance <= 0,ABS(amt_balance),0) debit_idr ,IF(amt_balance > 0,amt_balance,0) credit_idr, status, CONCAT('SELISIH KURS KONTRABON ',nama_supp) keterangan,create_user,create_date, confirm_user, confirm_date, '' cancel_by, '' cancel_date,'','',profit_center, '-' supplier FROM (select a.no_journal,a.tgl_journal,a.curr,sum(a.debit) debit,sum(a.credit) credit,IF(sum(a.debit) = sum(a.credit),'B','NB') balance,sum(ROUND(a.debit * a.rate,4)) debit_idr,sum(ROUND(a.credit * a.rate,4)) credit_idr,IF(sum(ROUND(a.debit * a.rate,4)) = sum(ROUND(a.credit * a.rate,4)),'B','NB') balance_idr,(sum(ROUND(a.debit * a.rate,4)) - sum(ROUND(a.credit * a.rate,4))) amt_balance,b.nama_supp,b.create_user,b.create_date,b.status,confirm_user,confirm_date, b.profit_center from tbl_list_journal a inner join kontrabon_h b on b.no_kbon = a.no_journal where type_journal = 'AP - Kontrabon' group by no_journal) a where a.balance_idr = 'NB' and a.tgl_journal >= '2026-01-01' and curr = 'USD')");
