<?php include '../header.php' ?>
    ...
    <!-- MAIN -->
    <div class="col p-4">
        <h3 class="text-center">LIST DATA FAKTUR</h3>
<div class="box">
    <div class="box header">
<form id="form-data" action="list_data_faktur.php" method="post">    
<div style="padding-left: 10px;padding-top: 5px;">
            <button style="-ms-transform: skew(6deg);-webkit-transform: skew(6deg);transform: skew(10deg);" id="btnbpb" type="button" class="btn-secondary btn-xs"><span></span>List Update BPB</button>
            <button style="-ms-transform: skew(6deg);-webkit-transform: skew(6deg);transform: skew(10deg);" id="btnfaktur" type="button" class="btn-primary active btn-xs"><span></span>List Data Faktur</button>
        </div>    
        <div class="form-row">
            <div class="col-md-6">
...
<tbody>
    ...
    if($nama_supp == 'ALL'){
    $sql = mysqli_query($conn2,"SELECT * from (select id,no_bpb,tgl_bpb,nama_supp,if(no_inv is null OR no_inv = '' OR no_inv = '-','-',no_inv) no_inv,if(no_inv is null OR no_inv = '' OR no_inv = '-','-',tgl_inv) tgl_inv,if(no_faktur = 0 OR no_faktur = '' OR no_faktur = '-','-',no_faktur) no_faktur,if(no_faktur = 0 OR no_faktur = '' OR no_faktur = '-','-',tgl_faktur) tgl_faktur,status, CONCAT(created_by,' (',created_date,')') user_create from bpb_faktur_inv where status != 'CANCEL' order by  no_bpb asc,id desc) a where no_faktur != '-' and tgl_bpb BETWEEN '$start_date' and '$end_date' order by no_bpb asc");
    }      
    else{
    $sql = mysqli_query($conn2,"SELECT * from (select id,no_bpb,tgl_bpb,nama_supp,if(no_inv is null OR no_inv = '' OR no_inv = '-','-',no_inv) no_inv,if(no_inv is null OR no_inv = '' OR no_inv = '-','-',tgl_inv) tgl_inv,if(no_faktur = 0 OR no_faktur = '' OR no_faktur = '-','-',no_faktur) no_faktur,if(no_faktur = 0 OR no_faktur = '' OR no_faktur = '-','-',tgl_faktur) tgl_faktur,status, CONCAT(created_by,' (',created_date,')') user_create from bpb_faktur_inv where status != 'CANCEL' order by  no_bpb asc,id desc) a where no_faktur != '-' and tgl_bpb BETWEEN '$start_date' and '$end_date' and nama_supp = '$nama_supp' order by no_bpb asc");
    }             
...
<div class="modal fade" id="mymodal" data-target="#mymodal" tabindex="-1" role="dialog" aria-labelledby="edit" aria-hidden="true">
        ...
        <div class="container">
        <div class="row">
          <div id="txt_nfaktur" class="modal-body col-6" style="font-size: 12px; padding: 0.5rem;"></div>      
          <div id="txt_tgl" class="modal-body col-6" style="font-size: 12px; padding: 0.5rem;"></div>
          <div id="txt_reff" class="modal-body col-6" style="font-size: 12px; padding: 0.5rem;"></div> 
...
<script type="text/javascript">     
    ...
    $.ajax({
    type : 'post',
    url : 'ajax_listfaktur.php',
    data : {'no_dok': no_dok,'no_fak': no_fak,'tgl_fak': tgl_fak},
    ...
    $('#txt_nfaktur').html('No Faktur : ' + no_fak + '');
    $('#txt_tgl').html('Tgl Faktur : ' + tgl_fak + '');
    $('#txt_reff').html('-');                     
});
