...
if ($mj_type2 == 'CMJ001') {
    $sql = "select no_journal,id_pc, profit_center, no_coa, nama_coa, no_cc, cc_name, reff_number, reff_date, buyer,ws,curr,debit, credit, deskripsi from (select a.no_journal,c.no_coa,c.nama_coa,a.kode_bagian no_cc,a.nama_bagian cc_name,'-' reff_number, '' reff_date, '-' buyer, '-' ws, 'IDR' curr,(a.gaji - a.potongan + (a.gaji_neto - (a.gaji + a.tunjangan_karyawan_rupiah + a.total_lembur_rupiah + a.bonus - a.piutang_karyawan - a.piutang_bazzar - a.bpjs_tk - a.bpjs_ks - a.potongan - a.potongan_insentif  - a.potongan_lembur))) debit, '0' credit, CONCAT('UPAH DEPT ',UPPER(a.nama_bagian),' ',UPPER(DATE_FORMAT('$tgl_hris','%M')),' ',DATE_FORMAT('$tgl_hris','%Y')) deskripsi, b.id_pc, b.profit_center from jurnal a inner join b_master_cc b on b.no_cc = a.kode_bagian INNER JOIN mastercoa_v2 c on c.no_coa = b.coa_gaji where periode_payroll = DATE_FORMAT('$tgl_hris','%Y-%m') 
                                        UNION
                                        ...
                                        UNION
                                        select a.no_journal,c.no_coa,c.nama_coa,a.kode_bagian no_cc,a.nama_bagian cc_name,'-' reff_number, '' reff_date, '-' buyer, '-' ws, 'IDR' curr,IF(potongan_lembur < 0,potongan_lembur,0) debit, IF(potongan_lembur > 0,potongan_lembur,0) credit, CONCAT(IF(potongan_lembur > 0,'POTONGAN LEMBUR DEPT ','LEMBUR DEPT '),UPPER(a.nama_bagian),' ',UPPER(DATE_FORMAT('$tgl_hris','%M')),' ',DATE_FORMAT('$tgl_hris','%Y')) deskripsi, b.id_pc, b.profit_center from jurnal a inner join b_master_cc b on b.no_cc = a.kode_bagian INNER JOIN mastercoa_v2 c on c.no_coa = b.coa_lembur where periode_payroll = DATE_FORMAT('$tgl_hris','%Y-%m')
                                        UNION
