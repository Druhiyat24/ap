...
if ($search != '') {
    ...
    $sql = "select no_mj, mj_date, nama_cmj, curr, sum(debit) debit, sum(credit) credit, keterangan, status from (select a.*, ifnull(filter,'-') filter from (select a.id, a.no_mj,a.mj_date,a.id_cmj,b.nama_cmj,concat(c.no_coa,' ', c.nama_coa) as coa , d.cc_name,a.no_coa,a.no_costcenter, a.no_reff, a.reff_date,a.buyer,a.no_ws,a.curr,a.rate,a.debit,a.credit,a.credit_idr,a.debit_idr,a.keterangan,a.status, kode_pc, CONCAT(mp.id_pc,' - ',nama_pc) nama_pc from tbl_memorial_journal_temp a left join master_category_mj b on b.id_cmj = a.id_cmj left join mastercoa_v2 c on c.no_coa = a.no_coa left join b_master_cc d on d.no_cc = a.no_costcenter LEFT JOIN master_pc mp on mp.kode_pc = a.profit_center where a.create_by = '$user') a LEFT JOIN
                                ...
                                ORDER BY no_coa asc) b on b.no_coa = a.no_coa and b.no_cc = a.no_costcenter and b.id_pc = a.kode_pc) b on b.filter = a.id) a $where GROUP BY no_mj ";
