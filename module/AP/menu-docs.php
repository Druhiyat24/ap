        ...
        'items' => [
            'bank-in' => [
                ...
                'doc' => [
                    ...
                    'notes' => [
                        ...
                        'Perbaikan (Sep 2026) — tab BANK OUT: satu Bank Out BISA punya lebih dari satu baris (mis. POS SILANG dipecah per profit center), tapi form dulu hanya mengirim <code>tableData[0]</code> dan endpoint hanya menerima satu baris ($_POST[\'no_coa\'], [\'debit\'], ...). Akibatnya baris ke-2 dst hilang dari jurnal dan selisihnya tertelan jadi baris <b>8.52.01 LABA/(RUGI) SELISIH KURS</b> — terjadi pada BM/BNI2000/NAG/0926/00001 (BK/BCA1979/NAG/0926/00032): POS SILANG 44.100.000 masuk, sisa 3.450.000 salah jadi selisih kurs, padahal dua-duanya IDR rate 1. Sekarang SELURUH baris dikirim sbg JSON (<code>detail</code>) dan dijurnal satu per satu; selisih kurs dihitung dari TOTAL semua baris sehingga hanya muncul kalau memang ada beda kurs sungguhan. Berlaku di create (<code>create-in-bank.php</code> + <code>bank_in/save_bankin_bankout.php</code>) maupun edit (<code>form_edit_bank_in_bk.php</code> + <code>update_bi_bankout.php</code>). Catatan: Amount di form memang diisi otomatis dari TOTAL seluruh baris (get_amount_bank_out.php memakai SUM), jadi semua baris memang harus ikut dijurnal.',
                        'Perbaikan (Sep 2026) — <code>cancel_inbank.php</code> kini MELEPAS kembali Bank Out-nya (<code>b_bankout_h.stat_bi</code> dikembalikan ke \'N\') saat Bank In dibatalkan, tapi HANYA kalau tidak ada Bank In lain yang masih aktif memakai Bank Out itu. Sebelumnya penandaan \'Y\' (dipasang jalur AR Collection) tidak pernah dikembalikan, sedangkan dropdown Bank Out menyaring <code>stat_bi != \'Y\'</code> — akibatnya nomor Bank Out hilang selamanya dari pilihan walau Bank In-nya sudah batal. Data lama yang terlanjur terkunci: BK/BCA1982/NAG/0326/00002 (perlu UPDATE manual, kode baru hanya melepas saat cancel berikutnya).',
                        'Jalur tab Bank Out TIDAK pernah menandai <code>stat_bi=\'Y\'</code> (yang menandai hanya jalur AR Collection lewat insert_bankin_arc.php), jadi secara teknis satu Bank Out masih bisa dipakai dua Bank In lewat tab itu — sampai Sep 2026 belum pernah terjadi di data.',
                    ],
                ],
            ],
            'bank-out' => [
                ...
                'doc' => [
                    ...
                    'notes' => [
                        'Perbaikan (25 Sep 2026) - UPLOAD DOKUMEN GAGAL TANPA KABAR, baris tetap masuk database. <code>insert_doc_bankout.php</code> memanggil <code>move_uploaded_file()</code> DUA KALI dengan tmp_name yang sama - panggilan pertama memindahkan berkasnya, panggilan kedua otomatis gagal (berkas sementara sudah tidak ada) sehingga alur SELALU masuk cabang else. Lebih parah, <code>INSERT</code> ke <code>b_bankout_dok</code> dijalankan TANPA memeriksa hasil pemindahan, dan <code>$_FILES[...][\'error\']</code> tidak pernah dicek sama sekali. Jadi begitu upload gagal (mis. melebihi <code>upload_max_filesize</code>), barisnya tetap tercatat tetapi berkasnya tidak ada: di halaman Bank Out tombol mata tampak normal, lalu <code>&lt;embed&gt;</code>-nya menampilkan "Not Found" dari Apache. Cabang sukses juga memanggil <code>alert()</code> - fungsi JavaScript, bukan PHP - yang akan fatal error kalau sempat dijalankan.',
                        'AKAR MASALAH (ditemukan user): NAMA BERKAS BANK MENGANDUNG SPASI. Berkas dari bank berpola <code>..._PT BINTANG_IDR_...pdf</code>. Nama yang dicatat ke database sudah dibuang spasinya (<code>str_replace(\' \', \'\', ...)</code> -&gt; <code>PTBINTANG</code>), sementara berkas di disk bisa tersimpan dengan nama ASLI yang masih berspasi - jadi yang dicari <code>&lt;embed&gt;</code> tidak sama dengan yang ada di disk, muncullah "Not Found". Buktinya di folder server ada PASANGAN berkas untuk dokumen yang sama, satu berspasi dan satu tidak (mis. <code>..._CV MATAHAR_IDR_...pdf</code> dan <code>..._CVMATAHAR_IDR_...pdf</code>, ukuran sama persis 149.581 byte). Berkas ini (versi repo, sejak Jun 2025) memakai variabel yang SAMA untuk disk dan database, jadi ketidakcocokan itu berasal dari salinan yang jalan di produksi (<code>/ap/</code>) - WAJIB deploy ulang. Setelah perbaikan, nama di disk dan di database dijamin identik karena dua-duanya memakai <code>$filename</code> yang sama, dan barisnya baru ditulis setelah berkasnya benar-benar tersimpan. Sebagai lapis kedua, <code>bank-out.php</code> kini membungkus nama berkas dengan <code>encodeURIComponent()</code> supaya nama berspasi tetap ter-request dengan benar.',
                        'Kasus nyata: BK/BCA1979/NAG/0926/00262 &amp; 00263 (25 Sep 2026, user mandy1). Diperiksa lewat HTTP ke server: berkas keduanya 404, sedangkan 148 unggahan lain dari 150 baris terakhir statusnya 200 - jadi kegagalannya jarang, bukan sistemik. Sekarang: <code>$_FILES[...][\'error\']</code> diperiksa lebih dulu dengan pesan yang jelas per kode (termasuk menyebut batas <code>upload_max_filesize</code> server), berkas dipindahkan SEKALI, ekstensi wajib .pdf, dan baris database HANYA ditulis setelah berkasnya benar-benar tersimpan (kalau INSERT-nya gagal, berkas yang terlanjur tersimpan dihapus lagi). Hasilnya dikirim balik ke <code>bank-out.php</code> lewat query string dan ditampilkan sebagai alert hijau/merah, sehingga upload tidak pernah lagi gagal diam-diam. Diuji 5 skenario di salinan lokal (sukses, melebihi batas, terkirim sebagian, bukan PDF, tidak ada berkas): hanya skenario sukses yang menghasilkan baris + berkas; empat sisanya tidak meninggalkan baris yatim maupun berkas yatim.',
                        'Penguatan lanjutan (25 Sep 2026, permintaan user: "agar masuk ke segala kondisi baik nama maupun besar file"). NAMA: dirapikan lewat <code>rapikanNama()</code> - komponen folder dibuang dgn <code>basename()</code> (mencegah path traversal <code>../../evil.pdf</code>), karakter kontrol dihapus, karakter yang ilegal di Windows (<code>\ / : * ? " &lt; &gt; |</code>) jadi <code>-</code>, spasi jadi <code>_</code>, akhiran titik/spasi dipangkas (Windows menolaknya), nama kosong jadi <code>dokumen.pdf</code>, dan panjang dibatasi 120 karakter supaya tidak menabrak batas path. Nama yang sudah dipakai TIDAK ditimpa - diberi akhiran <code>_2</code>, <code>_3</code>, dst. UKURAN: selain <code>$_FILES[...][\'error\']</code>, ditangani juga kasus POST melebihi <code>post_max_size</code> - saat itu PHP MEMBUANG seluruh isi POST sehingga $_POST &amp; $_FILES kosong dan gejalanya terlihat seperti "form tidak terkirim"; sekarang dideteksi dari <code>CONTENT_LENGTH</code> dan dilaporkan sebagai berkas kebesaran lengkap dgn angka batasnya. JENIS: ekstensi wajib .pdf DAN isinya dicek mengandung tanda <code>%PDF</code> (menolak berkas salah pilih/rusak). Pesan hasil dikirim lewat SESSION (bukan query string) lalu ditampilkan sebagai alert hijau/merah di bank-out.php dan dihapus setelah dibaca. Diuji 14 skenario di salinan lokal - spasi, spasi ganda, karakter ilegal, simbol #%&amp;, tabrakan nama, nama "....pdf", path traversal, nama 300 karakter, .jpg, isi bukan PDF, berkas 0 byte, melebihi batas, terkirim sebagian, tanpa berkas - hasilnya 8 baris database = 8 berkas di disk dengan nama IDENTIK, tanpa baris yatim maupun berkas yatim, dan tidak ada berkas yang lolos ke luar folder.',
                        'JANGAN memakai <code>is_writable()</code> sebagai penjaga folder di sini. Sempat dipasang saat penguatan 25 Sep 2026 dan langsung MEMBLOKIR upload yang valid di produksi ("Folder penyimpanan dokumen tidak bisa ditulis di server") - di Windows fungsi itu hanya melihat atribut read-only dan tidak benar-benar mengevaluasi ACL, apalagi kalau foldernya share/UNC, jadi sering melaporkan "tidak bisa ditulis" untuk folder yang sebenarnya bisa. Penjaganya dicabut; penentu yang sebenarnya adalah hasil <code>move_uploaded_file()</code>, dan kalau itu gagal barulah pesannya menyarankan IT memeriksa izin tulis folder (lengkap dengan pesan galat aslinya). Yang tetap dipertahankan hanya <code>is_dir()</code> + <code>mkdir()</code>, karena itu memang menguji keberadaan folder, bukan izinnya.',
                        'Membersihkan baris yatim yang terlanjur ada: pakai tombol tong sampah di kolom Action - <code>cancel_bankout_dok.php</code> hanya menandai <code>status = \'CANCEL\'</code> (berkas tidak disentuh) dan <code>ajx_bank-out.php</code> menyaring <code>status IS NULL</code>, jadi entri rusaknya langsung hilang dari daftar. Setelah itu unggah ulang dokumennya seperti biasa.',
                        'Perbaikan (22 Sep 2026) - EDIT yang mengubah Cash Flow Category kini ikut memperbarui <code>b_reportbank</code>. Dulu halaman edit hanya memperbarui tabel header (b_bankout_h / tbl_bankin_arcollection), sedangkan baris mutasi di b_reportbank tetap memakai kategori LAMA - padahal Cash Flow Realisation Report membaca <code>b_reportbank.id_cash_flow</code>, jadi laporannya salah kategori. Ditemukan 4 dokumen Bank Out yang sudah telanjur beda, mis. BK/BCA1979/NAG/0826/00284 (dokumen: Pengeluaran Pembayaran Uang Muka Supplier, laporan: Pengeluaran Pembayaran Utang Usaha) dan BK/BNI2000/NAG/0726/00007 3,35 M (kategori di laporan KOSONG sehingga dokumen itu tidak muncul sama sekali di Cash Flow Realisation). Sinkronisasi ditambahkan di update_bankout_none.php, update_bankout_pv.php, update_bi_ar_collection.php, update_bi_bankout.php (update_bi_none.php sudah sinkron sejak awal; update_bankout_lp.php tidak perlu karena form LP tidak punya field Cash Flow). SQL perapihan baris yang sudah telanjur beda: <code>module/AP/fix_cashflow_reportbank.sql</code> (dijalankan user).',
                        'Perbaikan (22 Sep 2026) - form edit Bank In AR Collection &amp; Bank In dari Bank Out dulu mengirim <code>cash_flow</code> HARDCODE (1 = Piutang Usaha, dan 6 = Pemindahbukuan Internal) sehingga mengedit dokumen berkategori lain diam-diam mengubah kategorinya. Padahal data menunjukkan dokumen BM di tbl_bankin_arcollection memakai kategori beragam: 1 (69 dok), 6 (25), 7 (7), 11 (5), 5 (2), 4 (1) - jadi 40 dokumen berisiko salah kategori begitu diedit. Sekarang hidden field diisi dari kategori DOKUMEN (fallback ke 1 / 6 kalau kosong) dan label di sebelahnya menampilkan nama kategori dokumen itu, diambil dari master_cash_flow. Perbaikan ini WAJIB berpasangan dgn sinkronisasi b_reportbank di atas: kalau nilai hardcode tadi ikut disinkronkan ke laporan, laporan justru jadi salah.',
            ...
            'payment-voucher' => [
                ...
                'doc' => [
                    ...
                    'notes' => [
                        'Perbaikan (24 Sep 2026) - "To Account" KOSONG di PDF Payment Voucher padahal rekeningnya terisi. Teks To Account dirangkai dengan <code>CONCAT(beneficiary_name, ...)</code>, sedangkan <code>CONCAT()</code> di MySQL mengembalikan NULL begitu SALAH SATU argumennya NULL - jadi rekening yang <code>beneficiary_name</code>-nya belum diisi membuat SELURUH teks hilang dan PDF menampilkan "-", walaupun nama bank dan nomor rekeningnya ada. Kasus: PV/NAG/0926/00157 (BRI 013201001623307) &amp; 00158 (Mandiri 130-0002077777), dua rekening perusahaan baru di <code>b_masterbank</code> yang beneficiary_name-nya masih NULL. Diganti <code>CONCAT_WS</code> (melewati bagian NULL) plus <code>NULLIF(TRIM(...),\'\')</code> supaya string kosong diperlakukan sama; pemisah tiap cabang dipertahankan apa adanya supaya dokumen yang selama ini benar tidak ikut berubah. Diuji ke SELURUH 7.311 PV Biaya di produksi: hanya 17 PV yang berubah, dan SEMUANYA dari "-" menjadi terisi - tidak ada satu pun yang nilainya bergeser. Catatan: ini BEDA dari kasus To Account kosong di Payment List (23 Sep 2026) yang sebabnya <code>to_akun</code> memang tidak diisi.',
                        'Pelengkap data: <code>fix_beneficiary_masterbank.sql</code> mengisi <code>beneficiary_name</code> = "PT Nirwana Alabare Garment" untuk 2 rekening tsb, mengikuti seluruh rekening perusahaan lainnya. Sifatnya OPSIONAL - tanpa skrip ini pun To Account sudah tampil ("BANK RAKYAT INDONESIA 013201001623307"); dengan skrip ini nama penerima ikut tampil di depannya. Baris "KAS KECIL PABRIK" (bank_account = \'-\', Deactive) sengaja dibiarkan karena itu penanda kas, bukan rekening bank.',
            ...
            'report-cashflow-realisation' => [
                ...
                'doc' => [
                    ...
                    'notes' => [
                        'PENGAMBILAN KURS (semua logika ada di <code>cfr_functions.php</code>, dipakai bersama halaman &amp; export). Ada TIGA konvensi kurs yang sengaja BERBEDA: (1) <b>Saldo Awal</b> memakai kurs <b>HARIAN</b> pada (tanggal_awal - 1 hari) — satu titik kurs tetap untuk seluruh periode, diambil dari <code>masterrate</code> dgn <code>tanggal &lt;= (awal-1)</code> lalu <code>order by tanggal desc limit 1</code> (cfrGetBankBeginBalances untuk bank, cfrGetKasBeginBalances untuk kas kecil, keduanya mencocokkan <code>mr.curr = curr akun</code>). (2) <b>Realisasi</b> dikonversi PER BARIS TRANSAKSI, bukan per akun. (3) <b>Limit fasilitas pinjaman</b> (fac_limit) memakai kurs HARIAN di tanggal AKHIR filter — konvensi "nilai pasar saat ini".',
                        'URUTAN KURS BARIS REALISASI (cfrComputeReportData): <b>a.</b> kalau kolom <code>curr</code> BARIS itu = IDR maka rate = 1 — ini WAJIB, karena baris penyesuaian selisih kurs (no_doc "FX/...") bercurr IDR walau akunnya USD dan nilainya sudah IDR; kalau dikonversi lagi hasilnya dobel dan bisa membengkak sampai ratusan miliar. <b>b.</b> kalau valas, prioritas pertama adalah <b>rate JURNAL</b> — dibaca dari <code>tbl_list_journal</code> yang <code>no_journal</code>-nya sama dgn <code>no_doc</code> transaksi dan <code>no_coa</code>-nya = <code>b_masterbank.id_coa</code> rekening tsb, status bukan Cancel. Ini menangkap kurs MANUAL yang diketik user di form Bank Out, jadi laporan memakai angka yang benar-benar diposting ke GL. <b>c.</b> kalau dokumen itu tidak punya baris jurnal (mis. data lama), baru fallback ke <code>masterrate</code> <b>PAJAK</b> di tanggal transaksi baris tsb.',
                        'FALLBACK cfrGetRate($conn, $date, $type) bertingkat 4: (1) baris <code>masterrate</code> dgn <code>v_codecurr = $type</code> pada tanggal PERSIS; (2) kalau tidak ada -> kurs <b>HARIAN</b> terbaru dgn <code>tanggal &lt;= $date</code>; (3) kalau tetap tidak ada -> kurs HARIAN paling baru apa pun tanggalnya; (4) kalau masterrate kosong -> 1. Perhatikan tingkat 2 &amp; 3 SELALU jatuh ke HARIAN walaupun yang diminta PAJAK — jadi <code>cfrGetRate($d, "PAJAK")</code> bisa mengembalikan kurs HARIAN untuk tanggal yang belum punya baris PAJAK.',
                        'JEBAKAN: <code>cfrGetRate()</code> menanam mati <code>curr = "USD"</code> — fungsi ini dipakai untuk saldo awal akun non-bank/non-kas, Saldo Awal Pinjaman Bank raw, Limit fasilitas, dan fallback PAJAK realisasi. Saat ini aman karena rekening aktif hanya IDR (5) &amp; USD (2) dan kas kecil semuanya IDR, tapi begitu ada rekening EUR/RMB (barisnya sudah ada di masterrate PAJAK) jalur-jalur itu akan memakai kurs USD. Selain itu <code>masterrate</code> hanya punya v_codecurr HARIAN untuk USD — rekening valas selain USD akan menghasilkan subquery NULL di cfrGetBankBeginBalances sehingga Saldo Awal-nya diam-diam terbaca 0.',
                        'Baris ber-no_doc <code>FX/%</code> (hasil auto-jurnal selisih kurs) DIKECUALIKAN dari perhitungan mutasi native saldo awal — itu murni penyesuaian nilai IDR, bukan pergerakan saldo mata uang asli. Kalau ikut dihitung, Saldo Awal jadi salah setiap kali ada jurnal FX bertanggal sebelum awal filter.',
            ...
            'payment-list' => [
                ...
                'doc' => [
                    ...
                    'notes' => [
                        'Perbaikan (23 Sep 2026) - KOLOM "To Account" KOSONG di PDF Payment List. Supplier instansi (kantor pajak/bea cukai) mengisi To Account secara BEBAS berupa kode billing, bukan nomor rekening dari master bank; daftar supplier semacam ini ada di <code>TOCC_MANUAL_SUPPLIERS</code> pada create/edit-paymentvoucher(-exim).php dan disalin ke <code>pdf_payvoucher.php</code> serta <code>pdf_payment_list.php</code>. Salinan di <code>pdf_payment_list.php</code> KETINGGALAN satu nama - "KANTOR PELAYANAN UTAMA BEA DAN CUKAI TIPE A" - sehingga kode billing-nya dicoba di-join ke master_supplier_bank/b_masterbank, gagal, dan keempat kolom To Account (Beneficiary Name, Bank Currency, Bank Name, Bank Account) tampil KOSONG - padahal di PDF Payment Voucher-nya sudah benar. Kasus: PL-PV/0926/00039, PV/NAG/0926/00041-00043. Sesudah diperbaiki, ketiga PV menampilkan kode billing-nya masing-masing (640260805775003 / ...774879 / ...774931) di kolom Bank Account, dan karena kode billing-nya berbeda-beda ketiganya otomatis pecah jadi 3 grup rowspan (Total Amount per PV, bukan lagi digabung 1.578.859.587) - ini memang yang benar karena setiap kode billing dibayar terpisah. Grand Total tidak berubah. Ditelusuri ke seluruh Payment List: hanya PL ini yang terdampak; PL berisi "KANTOR PAJAK"/"KPPBC TMP A BANDUNG" (21 PL) sejak awal sudah benar.',
                        'Lanjutan (23 Sep 2026) - DAFTAR SUPPLIER MANUAL DIPINDAH KE DATA. Akar masalah di atas bukan sekadar satu nama yang lupa ditambahkan, melainkan daftarnya yang HARDCODE dan disalin ke 6 berkas (4 form + 2 cetak) - tiap ada supplier baru, salah satu salinan pasti ketinggalan. Sekarang penandanya kolom baru <code>mastersupplier.to_account_manual</code> (TINYINT, 1 = To Account diisi manual), dibaca lewat satu helper bersama <code>tocc_manual_suppliers.php</code> (<code>toccManualSuppliers()</code> / <code>isToccManualSupplier()</code>, hasilnya di-cache per-request). Keenam berkas sekarang memanggil helper itu, tidak ada lagi daftar nama di dalam kode. MENAMBAH SUPPLIER MANUAL BARU CUKUP <code>UPDATE mastersupplier SET to_account_manual = 1 WHERE Id_Supplier = &lt;id&gt;</code> - tanpa deploy. Helper menerima beberapa koneksi sekaligus (<code>[$conn1, $conn2]</code>) dan menggabungkan hasilnya, karena blok koneksi di conn/conn.php sering ditukar dan sebagian berkas hanya memegang salah satunya; di pdf_payment_list.php $conn1 perlu di-<code>global</code>-kan sebab kodenya ada di dalam fungsi. Bila kolomnya belum ada (kode ter-deploy sebelum ALTER), helper otomatis memakai daftar lama lewat <code>toccManualSuppliersFallback()</code> sehingga perilaku tidak berubah di masa transisi - dua skenario ini sudah diuji dan hasilnya identik. Migrasi: <code>add_to_account_manual.sql</code> (ALTER + menandai 3 supplier awal); sudah dijalankan di lokal dan di produksi.',
                        'Catatan data (23 Sep 2026) - 9 baris PV Biaya atas nama "PT. NIRWANA ALABARE GARMENT" (a.l. PV/NAG/0926/00140 di PL-PV/0926/00041) juga tampil kosong di To Account, tetapi ini BUKAN bug cetak: <code>tbl_pv_h.to_akun</code>-nya memang kosong sejak PV dibuat. Perlu dilengkapi dari sisi data/entri PV, bukan dari kode.',
                        'Perbaikan (21 Sep 2026) - PV LINTAS PROFIT CENTER tidak lagi hilang sebagian. Daftar kandidat dibuat PER (dokumen x profit center) - <code>getDataBiaya()</code> GROUP BY no_pv, profit_center - tetapi penyaring "sudah pernah diambil" dulu hanya memakai nomor dokumen, sehingga begitu satu PC diambil, PC lain pada PV yang sama ikut lenyap dan tidak bisa dipanggil lagi. Kasus: PV/NAG/0926/00119 (BESTINDO EXPRESS) punya 2 baris - NAG 5.160.273 &amp; NAK 7.749.030; PL-PV/0926/00037 hanya menarik bagian NAK, lalu bagian NAG hilang dari pencarian. Penanda <code>tbl_pv_h.status_pl</code> memang HEADER-level (satu penanda untuk seluruh PV), jadi yang diperbaiki adalah kunci penyaringnya: sekarang nomor dokumen + profit center. Diterapkan di 4 tempat dgn pola sama: form_payment-list.php, form_payment-voucher-list.php, edit_payment_list.php, edit_payment_voucher_list.php (yang edit juga memakai kunci sama untuk baris yang sudah ada di dokumen itu sendiri). Aman karena kolom profit_center di pv_payment_list_det (1.180 baris) &amp; pv_payment_voucher_list_det (1.052 baris) SELALU terisi - tidak ada satu pun yang kosong. Diuji simulasi seluruh tanggal: kandidat bertambah TEPAT 1 baris (PV/NAG/0926/00119 [NAG] 5.113.830,54) dan TIDAK ADA baris yang hilang, jadi tidak ada risiko dokumen terklaim dua kali.',
            ...
            'bpb-transferred' => [
                ...
                'doc' => [
                    ...
                    'notes' => [
                        ...
                        'REGRESI &amp; PERBAIKAN (Sep 2026) - dokumen TATP tidak muncul di Accept Pch: commit 165efa8 keliru menyatukan kondisi update status header di insert_trf_fintoacc.php menjadi <code>if ($kode_trans === TFTA || $kode_trans === TATP)</code>, sehingga cabang <code>elseif TATP</code> di bawahnya TIDAK PERNAH tercapai. Akibatnya transfer Acc-ke-Pch menulis header memakai nilai milik TFTA: status jadi "Post Fin To Acc" (seharusnya "Post Acc To Pch") dan tfta_by/tfta_date terisi user+tanggal TATP. Dokumen lenyap dari form_approve_pch.php padahal baris ir_trans_invoice_supp-nya BENAR, dan operator tidak melihat error apa pun - antrian Accept Pch sekadar jadi 0. Kode dikembalikan ke <code>if ($kode_trans === TFTA)</code> saja; guard BPB di atasnya TETAP mencakup TFTA DAN TATP (itu memang benar, yang salah cuma kondisi update status). Data yang terlanjur rusak dibereskan lewat db_migrate_LIVE_fix_tatp_header.sql - di produksi terdampak 14 IR, semuanya batch TATP/NAG/0926/01062.',
                        'ALUR SAH: IR yang BELUM pernah lewat TFTA boleh langsung ditransfer TATP lalu di-approve di Accept Pch. Menu TATP menyaring status IN (Accepted Acc, Received) sehingga IR "Received" ikut tampil, dan baik form_approve_pch.php maupun approve_acctopch.php tidak mensyaratkan adanya baris TFTA. Hitungan TFTA di form_approve_pch.php baris 168 hanya dicetak ke td tersembunyi dan TIDAK menggating apa pun (sisa salinan dari halaman lain). Di produksi ada 4.409 dokumen preseden: punya TATP, tanpa TFTA sama sekali, dan lolos approve PCH.',
                        'Perbaikan (Sep 2026) - TANGGAL: di menu TATP (post_inv_acctopch.php), filter rentang tanggal DAN batas minimum Transfer Date kini memakai TANGGAL IR AKTUAL (<code>DATE(created_date)</code>), bukan <code>tgl_penerimaan</code>. Sebabnya <code>tgl_penerimaan</code> sering digeser maju ke hari Rabu (tanggal kontrabon), sehingga bisa lebih besar dari hari transfer sebenarnya - mis. IR/NAG/2026/09/05726 dibuat 04-Sep tapi tgl_penerimaan 09-Sep, membuat transfer tgl 07-Sep tertolak validasi "tgl_doc &gt;= tgl_fil_inv" padahal IR-nya sudah ada. Kolom "Kontrabon Date" yang TAMPIL tetap tgl_penerimaan; approved_date (TFTA) tetap diprioritaskan bila ada. KONSEKUENSI: IR yang tanggalnya digeser ke Rabu kini muncul di rentang tanggal PEMBUATANNYA, bukan di rentang tanggal kontrabonnya. Menu TFTA (post_inv_fintoacc.php) dan TPTF (post_inv_pchtofin.php) BELUM diubah dan masih memakai tgl_penerimaan - keduanya punya pola pembatasan yang sama.',
...
'Menu ini MENGGANTIKAN dua menu lama yang kini sudah tidak ada di sidebar: BPB Transferred (bpb_received.php) dan Invoice Received lama (invoice_received.php). Keduanya digabung menjadi satu halaman dengan pilihan Type Document - Invoice atau BPB & SJ - dan tombol aksinya (Post/Accept per tahap, Accept BPB, Accept SJ, Reverse) muncul sesuai type dan hak akses menurole pengguna.',
                        ...
                        'Perbaikan (25 Sep 2026) - ACCEPT HANYA TERSIMPAN SEBAGIAN. Tombol Accept dulu punya DUA handler terpisah (approve utk baris yang dicentang, cancel utk yang tidak), masing-masing menembak SATU AJAX PER BARIS di dalam <code>.each()</code>, dan setiap respons sukses langsung menjalankan <code>window.location</code>. Begitu respons PERTAMA datang halaman berpindah dan SELURUH request yang masih berjalan dibatalkan browser - jadi hanya sebagian baris yang tersimpan, dan baris mana yang lolos bersifat acak. Kasus nyata TBPB/NAG/0926/01749: dari 84 baris hanya 35 yang ter-Approve, 44 tetap "Transfer". Alert "Data Berhasil Di Approve" pun muncul SEBELUM satu pun request selesai, sehingga user mengira sudah beres. Sekarang: seluruh nomor dikirim SEKALI sebagai array, <code>approve_whstoacc.php</code> / <code>cancel_whstoacc.php</code> memprosesnya dalam SATU UPDATE ber-<code>IN (...)</code>, tombol dikunci selama proses, ada konfirmasi yang menyebutkan berapa baris di-Accept dan berapa yang akan di-Cancel, dan redirect baru dijalankan SETELAH semua request selesai. Pesannya memakai jumlah yang DIHITUNG ULANG dari database (bukan affected_rows, yang tidak menghitung baris yang statusnya sudah sama).',
                        'Kedua endpoint tetap menerima <code>no_bpb</code> berupa SATU nilai supaya <code>maintain-bpb.php</code> (pemanggil lain <code>cancel_whstoacc.php</code>) tidak ikut berubah - halaman itu masih memakai pola lama satu-request-per-baris dan BELUM diperbaiki. Diuji di DB salinan berisi 84 baris asli: 80 Accept + 4 Cancel tersimpan lengkap dalam sekali kirim; plus uji nilai tunggal (kompatibilitas lama), daftar kosong, dokumen kosong, approve dua kali (idempoten), nomor yang tidak ada, dan escaping kutip tunggal - semuanya sesuai harapan. Penghitungan aman karena pasangan (no_transfer, no_bpb) TIDAK PERNAH ganda: 0 dari 29.944 baris.',
                        'Catatan bug lama yang SENGAJA dibiarkan di <code>cancel_whstoacc.php</code>: ada statement kedua yang mengosongkan <code>bpb.stat_trf</code> memakai variabel <code>$no_transfer</code> yang TIDAK PERNAH didefinisikan (yang ada <code>$no_dok</code>), sehingga kondisinya selalu <code>no_transfer = \'\'</code> dan tidak pernah menyentuh satu baris pun - diverifikasi: 0 baris ir_trans_bpb yang no_transfer-nya kosong. Tidak dihidupkan karena WHERE-nya per no_transfer (bukan per no_bpb), jadi membatalkan SATU BPB akan ikut mereset stat_trf SELURUH BPB dalam transfer tsb. Perlu keputusan user dulu.',
...
'MENU LAMA - sudah TIDAK ada di sidebar. Fungsinya digabung ke menu Document Handover (satu halaman dengan pilihan Type Document: Invoice atau BPB & SJ). Entri ini dipertahankan sebagai rujukan alur lama; halaman invoice_received.php sendiri masih ada di server tetapi tidak lagi ditautkan dari menu.',
            ...
            'payment-voucher-ap' => [
                ...
                'doc' => [
                    ...
                    'notes' => [
                        'Perbaikan (24 Sep 2026) - UANG MUKA CBD/DP DIPOTONG PENUH, BUKAN NETTO. Saat Bank Out, COA uang muka didebit sebesar nilai PENUH karena PPh disetor ke kantor pajak ATAS NAMA SUPPLIER - jadi uang muka ke supplier tetap sebesar nilai penuh. Tetapi daftar FTR di form PV AP mengambil <code>SUM(b_bankout_det.total)</code> yang sudah NETTO, sehingga jurnal PV hanya mengkredit uang muka sebesar netto dan sisa sebesar PPh MENGGANTUNG di COA uang muka (kasus: PV-AP/REG/NAG/2026/09/02282, 02283, 02284 - total 352.500 di COA 1.49.99). Sekarang daftarnya memakai <code>SUM(total + COALESCE(pph,0))</code> di 3 berkas x 2 cabang (CBD &amp; DP): payment_voucher_ap/pv_regular.php, payment_voucher_ap/pv_installment.php, edit_pv_regular_new.php. Nilai ini dipakai form untuk mengisi <code>ttl_ftr</code> (kredit jurnal uang muka) DAN <code>dp_h</code> (debit offset Utang Usaha), jadi jurnal tetap balance dengan sendirinya - dua sisi itu memang harus berubah bersamaan (lihat contoh PV 02283 yang sudah dibetulkan manual oleh user). Diverifikasi ke data produksi: <code>total + pph</code> di b_bankout_det SAMA PERSIS dengan <code>total + pph</code> dokumen untuk seluruh 45 CBD + 2 DP, jadi tidak ada dokumen yang bergeser selain yang ber-PPh (hanya 4 dokumen).',
                        'AWAS saat membetulkan dokumen lama: selain dua baris jurnal, <code>kontrabon_ftr.total_ftr</code> HARUS ikut disetel ke nilai penuh. Kolom itulah yang dipakai form untuk menghitung sisa uang muka yang masih bisa dipotong (<code>a.total - COALESCE(b.total_ftr,0)</code>); kalau hanya jurnalnya yang diedit, sisa sebesar PPh akan terus muncul sebagai "masih bisa dipotong" dan berpotensi terpotong DUA KALI. Skrip <code>fix_uang_muka_full.sql</code> membetulkan ketiganya sekaligus (jurnal Dr Utang Usaha, jurnal Cr Uang Muka, dan total_ftr), idempoten, dan sudah diuji di DB salinan: ketiga jurnal tetap balance (selisih 0) dan tidak ada sisa yang tertinggal.',
                        'Perbaikan (23 Sep 2026) - BEBAN HEADER PECAHAN TERJURNAL DI SISI TERBALIK. Uji tanda beban header (Selisih Kurs, Selisih Qty, Selisih Harga, Materai, Ekspedisi, MOQ, Potongan Beli, Potongan PPN) memakai ambang <code>&gt;= 1</code>, padahal yang dimaksud <code>&gt; 0</code> - yang diuji TANDA-nya, bukan besarannya. Akibatnya nilai POSITIF di bawah 1 - persis kasus selisih pembulatan DPP+PPN vs total faktur, mis. 0,05 - ikut masuk cabang negatif dan dijurnal sebagai CREDIT padahal seharusnya DEBIT. Dua dampaknya: (1) jurnal PV TIDAK BALANCE sebesar 2x nilai itu; (2) di AP Report kolom <b>Deduction Others</b> (di query_pv2.php = <code>SUM(debit - credit)</code> baris ber-nama_coa LIKE %BEBAN%, lalu DITAMBAHKAN ke saldo akhir) muncul MINUS padahal user mengisi plus, sehingga Ending Balance meleset - mis. PV-AP/REG/NAK/2026/09/02362 jadi -0,02 padahal harusnya 0,00. Diperbaiki di 6 berkas yang semuanya masih jadi jalur aktif: insertkbon_core.php (create bulk), insertkbon_bulk_edit.php (Edit New), insertkbon_h.php (Kontra Bon), insertkbon_h_pv_regular.php, insertkbon_h_pv_installment.php, insert_kontrabon_edit_all.php (edit lama) - total 38 uji tanda. Ambang <code>$tax_h &gt;= 1</code> SENGAJA dibiarkan: itu penjaga ada/tidaknya PPN, bukan uji tanda, dan tidak ada satu pun dokumen ber-PPN antara 0 dan 1. Perubahan ini HANYA menyentuh rentang 0 &lt; nilai &lt; 1; nilai &gt;= 1 dan nilai negatif berperilaku persis sama seperti sebelumnya, jadi tidak ada dokumen benar yang jadi salah. Data lama: ditelusuri ke seluruh tbl_list_journal + tbl_list_journal_cancel sejak 1 Juli 2026, TEPAT 6 baris terdampak (PV 02112, 02304, 02358, 02362 + jurnal balik-nya, 02362-REV_01) - diperbaiki lewat <code>fix_beban_sisi_terbalik.sql</code> (idempoten, pakai id + nilai lama sebagai pengaman). Diuji di DB salinan: keenam jurnal jadi balance dan <code>Addition + Deduction Others = utang riil</code> untuk kelimanya.',
                        'Perbaikan (23 Sep 2026) - badge status membedakan PV List yang MASIH DRAFT. Dulu <code>computeStatusLabel()</code> memberi label <b>3rd Approval PV List</b> untuk SEMUA PV yang status_pvl-nya terisi &amp; bukan Cancel - termasuk yang PV List-nya masih Draft. Akibatnya PV yang sudah bisa ditarik ke Payment List dan yang BELUM bisa tampil dengan badge yang sama persis, dan user bingung kenapa PV-nya tidak muncul saat membuat Payment List (picker mensyaratkan status_pvl = APPROVED). Sekarang: status_pvl APPROVED -&gt; <b>3rd Approval PV List</b> (ungu), selain itu -&gt; <b>Waiting 3rd Approval PV List</b> (amber). Filter status di halaman ini juga ditambah opsi baru itu, dan opsi lama dipersempit ke UPPER(status_pvl) = APPROVED. Saat diterapkan: 44 PV berlabel Waiting (tertahan di 3 PV List Draft milik dessy - PVL-PV/0926/00010, 00011 berisi 41 PV senilai 777,8 jt, dan 00012), 135 PV berlabel 3rd Approval PV List. Kasus pemicu: PV-AP/REG/NAG/2026/09/02267 &amp; 02268 (DJONI/DUNIA BARU) dan 02239/02303/02304 (ONLINE SHOP).',
                        ...
                        'Perbaikan (Sep 2026) - No/Tgl FAKTUR turun ke bpb_new saat CREATE: sebelumnya <code>bpbnew_apply_docinfo()</code> di insertkbon_core.php HANYA dipanggil di cabang "BPB tambahan" (<code>$fid &gt; 0</code>), sehingga faktur yang diketik saat MEMBUAT PV tidak pernah tertulis ke bpb_new untuk BPB reguler - jalur EDIT sudah melakukannya utk setiap BPB, jadi create dan edit berperilaku beda. Celah ini lama tak terlihat karena umumnya faktur sudah lebih dulu diisi di tahap IR sehingga bpb_new tampak benar; baru kelihatan kalau BPB-nya TIDAK lewat IR (kasus PV-AP/REG/NAG/2026/09/02212, BPB GEN/IN/0826/02574). Sekarang create menulis utk semua BPB. Penanda <code>$docinfo_done</code> mencegah penulisan ganda: BPB tambahan sudah ditulis di cabangnya sendiri memakai invoice IR, jadi blok reguler (yang memakai invoice header PV) dilewati - tanpa penanda ini nomor invoice dari IR akan tertimpa. Nilai faktur asli MENIMPA yang lama, tapi strip "-" tetap tidak boleh menghapus No/Tgl Invoice/Faktur real (bpb_docinfo_guard.php). CAKUPAN: hanya jalur PV Regular create (pv_regular.php -&gt; insertkbon_bulk.php -&gt; insertkbon_core.php). insertkbon.php lama punya celah sama dan masih dipakai PV Installment serta form Pelunasan/Kontrabon lama - sengaja TIDAK disentuh.',
                        'Perbaikan (16 Sep 2026) - panel "Data FTR (CBD/DP)" menampilkan nilai FTR BERLIPAT sebanyak jumlah baris PO-nya. Penyebabnya <code>b_bankout_det</code> (pembayaran, 1 baris per kontrabon) di-JOIN ke <code>kontrabon_cbd</code>/<code>kontrabon_dp</code> yang berisi SATU BARIS PER PO, lalu dijumlah dgn <code>SUM(bd.total)</code> - jadi pembayaran dihitung sekali per PO. Contoh: FTR/C/NAG/0726/00657 punya 2 PO (86.748,14 + 518.057,76 = 604.805,90) tetapi tampil 1.209.611,80. Perbaikannya: bankout diringkas dulu jadi satu baris per kontrabon lewat sub-query ber-GROUP BY, lalu diambil <code>MAX</code> (bukan SUM). Diterapkan di pv_regular.php, edit_pv_regular_new.php, dan pv_installment.php (varian berkas ini memakai SUM(d.total) - pola fan-out yang sama, ikut dirapikan walau datanya belum menggandakan). DAMPAK DIUJI ke 25 supplier yg memakai jalur bypass Bank Out: hanya 2 baris berubah - PT R-PAC (1.209.611,80 -&gt; 604.805,90) dan PERCETAKAN SOFASCO (FTR/C/NAG/0726/00665 HILANG dari daftar, dan itu benar: FTR-nya sudah dipakai penuh di PV-AP/REG/NAG/2026/08/02061 sehingga penggandaan tadi membuatnya seolah masih bisa diklaim sekali lagi). Riwayat kontrabon_ftr dicek: TIDAK ADA FTR yg terlanjur diklaim melebihi nilainya (7 kasus yg awalnya mencurigakan ternyata pasangan revisi berstatus Updated/-REV_01, bukan klaim dobel).',
                        'Masih ada bug salin-tempel di pv_installment.php: cabang DP-nya mencari <code>b_bankout_det.type_pv = "CBD"</code> (seharusnya "DP"), sehingga FTR DP yg dibayar langsung lewat Bank Out tidak pernah muncul di menu Installment (saat ini ada 1 kontrabon seperti itu). Belum diperbaiki - menunggu keputusan user.',
            ...
            'status' => [
                ...
                'doc' => [
                    ...
                    'notes' => [
                        ...
                        'Query-nya TIDAK lagi ditulis di halaman: disiapkan di <code>status_query.php</code> (status_siapkan_query) lalu dipakai bareng oleh tabel AJAX (<code>ajx_status.php</code>) dan ekspor Excel (<code>ekspor_status.php</code>), supaya isi Excel tidak mungkin berbeda dgn yang tampil di layar. Sebelumnya query yang sama ditulis ulang 16 kali di status.php, dan link Excel menempelkan seluruh teks SQL (~2.200 karakter) di URL.',
                        'Tiap tahap (pelunasan, list payment, peta kontrabon, daftar dokumen, verifikasi) ditumpahkan ke TEMPORARY TABLE lalu diberi INDEX, baru dirakit — bukan dirangkai sbg subquery. Alasannya: tabel turunan di MySQL versi ini tidak punya index, jadi tiap baris sisi kiri memindai seluruh isinya. Filter Payment Date terukur 2.615 detik lalu koneksi diputus ("MySQL server has gone away"); setelah memakai temporary table jadi 0,9 detik dgn hasil yang sama persis (1.012 baris). Temporary table hanya hidup di koneksi pembuatnya, jadi aman dipakai banyak pengguna sekaligus. Dibuat lewat "CREATE TEMPORARY TABLE ... AS SELECT" supaya tipe &amp; collation-nya mewarisi tabel asal — kalau dideklarasikan manual dan collation-nya meleset, join-nya justru berhenti memakai index.',
                    ],
                ],
            ],
            '_s11' => ['section' => 'Report'],
            ...
            'ap-report' => [
                ...
                'doc' => [
                    ...
                    'notes' => [
                        ...
                        'Perbaikan (Sep 2026) - potongan PPN kini ikut dihitung dan MASUK KE KOLOM <b>Deduction Others</b> (atas permintaan user TIDAK dibuat kolom baru). Baris akun PPN Masukan (1.52.04 billed &amp; 1.52.07 unbilled) di jurnal PV dulu tidak dihitung, sehingga PV yang PPN-nya dipotong menyisakan saldo gantung walau sudah lunas. Contoh: PV-AP/REG/NAG/2026/08/02062 (USD) - GR/IR 15.540, Uang Muka -4.662, PPN Unbilled -1.540, dibayar BK 9.338 -&gt; dulu tersisa 1.540, kini Deduction Others -1.540 dan saldo 0. Nilainya BERTANDA: negatif memotong utang, positif menambah utang (mis. 02235: GR/IR hanya DPP 150.400 + PPN 16.544 = utang 166.944, dulu tertulis 150.400). Digabung di CTE <code>mutasi</code> query_pv2.php, jadi semua tab &amp; ekspor ikut tanpa perubahan tampilan. Diuji ke 639 PV sejak Jul 2026: 637 cocok persis dgn utang usaha di jurnal, 2 sisanya selisih pembulatan data sumber (&lt; 1).',
                        'Perbaikan (18 Sep 2026) - PPh ATAS UANG MUKA ikut mengurangi utang. Sejak ada PPh atas uang muka, ada PV yang uang mukanya dijurnal NET sebesar (subtotal - PPh) sehingga utang usaha menyisakan tepat sebesar PPh dan MENGGANTUNG di report. Contoh PV-AP/REG/NAG/2026/09/02282: GR/IR 3.825.000, PPh 95.625, uang muka dijurnal 3.729.375 -&gt; dulu tersisa 95.625, kini 0. PPh TIDAK pernah dijurnal di dokumen PV (pada PV yang dibayar bank, PPh baru muncul di jurnal Bank Out lewat COA 2.52.05 dan pembayaran mendebit utang PENUH - itulah sebabnya kolom <code>pph</code> yang lama SENGAJA tidak masuk perhitungan saldo akhir; kalau dikurangkan tanpa syarat, PV yang dibayar bank jadi minus). Karena itu penyesuaian dibatasi dua syarat di CTE <code>pph_um_all</code>: (a) uang muka yang dijurnal PERSIS = subtotal - PPh, dan (b) belum ada jurnal ber-type Payment yang mereferensikan PV tsb. Syarat (b) membuat penyesuaian otomatis berhenti begitu PPh-nya dibayar lewat BK sehingga tidak pernah dobel. Catatan satuan: <code>pph_idr</code> selalu rupiah sedangkan CTE bekerja dalam mata uang dokumen, jadi untuk dokumen valas dibagi kurs (per Sep 2026 belum ada kontrabon non-IDR ber-PPh). DIUJI ke seluruh PV ber-PPh 2026: hanya 3 PV yang memenuhi syarat (02282/02283/02284, ketiganya FIRST APPROVED dan belum dibayar). Uji banding query lama vs baru periode 1-20 Sep 2026: dari 1.144 baris hanya 3 baris itu berubah (saldo 95.625 / 95.625 / 81.250 -&gt; 0); diuji juga periode 17-30 Sep supaya uang mukanya masuk kolom saldo awal - hasilnya sama-sama 0.',
                        '<code>ekspor_pcs_all2.php</code> (ekspor 7 sheet sekaligus) IKUT memakai query bersama: baris 30-31 meng-include query_pv2.php lalu <code>$sql_pv = $sql;</code>, dan sheet 2-7 membungkus <code>$sql_pv</code> itu. Di dalam berkas itu MEMANG masih ada 6 salinan lama query PV (floor 2025-12-31, tanpa perbaikan PPN/PPh), TETAPI semuanya terkurung di blok <code>if(false){ $sql_oldN = ... }</code> alias KODE MATI - tidak pernah dijalankan. Jadi angka ekspor "all" sama dengan layar. Sudah diuji: 02282/02283/02284 -&gt; saldo 0, sedangkan 02089 &amp; 01843 tetap 0 (tidak jadi minus). Kalau suatu saat blok if(false) itu dihidupkan lagi, dua perbaikan tsb harus disalin ke sana dulu.',
                        'Tambalan lama "- diff_ppn" pada CTE potongan di <code>ap_report2/query_pv2.php</code> DIBUANG - tambalan itu hanya jalan kalau PV kebetulan punya baris akun BEBAN (dan terkurang sekali per baris BEBAN), sehingga 02062 (tanpa baris BEBAN) tetap menggantung. Untuk 38 PV yang dulu tertambal, nilai Deduction Others tetap sama. JANGAN dipasang lagi: bersamaan dgn CTE ppn, 40 PV akan terpotong dobel.',
                        'Catatan tampilan (sudah ada sebelum perbaikan ini): di layar, kolom Deduction Others (bersama Advance &amp; Reverse) SELALU dirender negatif dalam kurung merah apa pun tandanya, sedangkan Excel menampilkan tanda aslinya. Jadi PV dgn Others positif (mis. 02235 +16.544, 02193 +546.436) tampil (xx) merah di layar walau sebenarnya menambah utang.',
                        'PPh SENGAJA tidak dijadikan pengurang: PPh dipotong saat PEMBAYARAN dan jurnal BK tetap mendebit Utang Usaha penuh (PPh dikredit ke 2.52.xx). Ke-64 PV ber-PPh yang sudah dibayar lunas persis - memotong PPh lagi justru membuat saldo minus.',
                        'Dampak ke bulan sebelumnya (diuji): Juli 2026 tidak ada PV yang berubah; Agustus 2026 hanya 02062 (ending 10.878 -&gt; 9.338 USD, ≈ Rp27,26 jt); saldo akhir Juli = saldo awal Agustus utk 746 PV dan Agustus = September utk 984 PV. Periode sebelum Juli 2026 tidak tersentuh.',
                        'Di <code>ap_report2/ekspor_pcs_all2.php</code> ada 6 salinan query PV versi LAMA (kolom ded_lp) di dalam blok <code>if(false){...}</code> - kode mati, sheet yang hidup memakai $sql_pv dari query_pv2.php.',
                    ],
                ],
            ],
            'rekap-pelunasan' => [
        ...
        'items' => [
            'memorial-journal' => [
                ...
                'doc' => [
                    ...
                    'notes' => [
                        ...
                        'Perbaikan (Sep 2026) - VERIFIKASI MJ jadi BULK: formverifikasimj.php dulu memanggil approvemj.php SATU KALI PER DOKUMEN dan MENUNGGU tiap panggilan (await di dalam for). Biaya terukur per dokumen: ~189 ms include conn.php (ikut membuka koneksi PostgreSQL ke 10.10.5.62 yang tidak dipakai proses ini), ~84 ms query penomoran (MONTH()/YEAR() = full scan 174rb baris), ~64 ms baca tbl_memorial_journal (TIDAK ADA index no_mj, full scan 110rb), ~8 ms cek status_memorial_journal (juga tanpa index) = ~345 ms/dokumen, sehingga centang 50 baris memakan ~17 detik hanya untuk overhead. Sekarang semua dikirim dalam SATU request ke <code>approvemj_bulk.php</code> dan diproses sebagai himpunan: cek "sudah pernah" dengan WHERE no_mj IN (...), penomoran SEKALI per bulan lalu dinaikkan di PHP, dan 3 INSERT ... SELECT lewat tabel peta sementara. Jumlah query jadi TETAP (~6), tidak tumbuh mengikuti jumlah dokumen. approvemj.php lama TIDAK diubah dan sudah tidak dipakai halaman ini.',
                        'PENTING (sumber keluhan "sudah diceklis tapi tidak terverifikasi"): dokumen yang nomornya SUDAH ada di status_memorial_journal dilewati oleh <code>if ($count_mj &lt;= 0)</code> - dulu DIAM-DIAM, tanpa pesan apa pun, sehingga user mengira gagal. Endpoint bulk kini memisahkan hasil menjadi Verified / Skipped (already verified) / Failed beserta daftar nomornya, dan jumlah yang dicentang ditampilkan SEBELUM approve sebagai pembanding. Seluruh notifikasi memakai bahasa Inggris.',
                        'CATATAN COLLATION di approvemj_bulk.php: tbl_memorial_journal.no_mj &amp; tbl_list_journal.no_journal = utf8mb4_general_ci, sedangkan status_memorial_journal.no_mj &amp; sb_memorial_journal.no_mj = latin1_swedish_ci. Tabel peta sementara sengaja dibuat utf8mb4_general_ci supaya JOIN ke tbl_* tidak memicu konversi di sisi KOLOM yang akan mematikan pemakaian index. conn1 &amp; conn2 menunjuk database yang SAMA (signalbit_bk_jul26), jadi seluruh proses memakai satu koneksi - temporary table hanya hidup di koneksi pembuatnya.',
            ...
            'list-journal' => [
                ...
                'doc' => [
                    ...
                    'notes' => [
                        'Perubahan (commit 5243204, Sep 2026) - KOLOM SUPPLIER: tbl_list_journal dan tbl_list_journal_cancel kini punya kolom <code>supplier</code> (posisi paling belakang, WAJIB sama di kedua tabel karena banyak file cancel memakai INSERT..SELECT *). Diisi LANGSUNG DARI KODE di semua jalur input jurnal - bukan lewat trigger - dari variabel yang sudah ada di tiap file ($nama_supp / $supp / $customer), dan selalu di-escape karena nama supplier bisa mengandung apostrof.',
                        'Cakupan pengisian: PV/Kontrabon (create+edit), Bank Out, Bank In (input+edit), Petty Cash In/Out, Repayment, BPB, dan SELURUH jurnal balik (23 jalur). File cancel_* tidak diubah karena memakai INSERT..SELECT * sehingga supplier ikut tersalin sendiri.',
                        'TIDAK diisi (memang tidak ada sumbernya): Cash In / Cash Out - tabel c_cash_in dan c_cash_out tidak punya kolom supplier sama sekali, hanya buyer, ws, dan req_by; Memorial Journal (gaji, BPJS, penyusutan, akrual); FX Selisih Kurs; Jurnal Meals. Invoice, Alokasi, dan AR Collection juga kosong karena lawan transaksinya CUSTOMER, bukan supplier - nilainya ada di kolom buyer.',
                        'Untuk jalur yang tidak punya variabel supplier, nilainya diambil dari dokumen induk: insert_bankout_det dari b_bankout_h, Petty Cash settlement dari c_petty_cashout_h (PCO yang di-settle), dan jurnal PPN dari baris PV yang sudah ada.',
                        'Data lama: jalankan sekali module/AP/backfill_journal_supplier.php (set-based lewat tabel pemetaan sementara, sekitar 30 detik). Skema produksi: module/AP/db_migrate_LIVE_supplier_journal.sql - Langkah 1, 2, dan 4 saja yang wajib; Langkah 3 (trigger) OPSIONAL karena pengisian sudah ditangani kode.',
                        'JEBAKAN COLLATION - kalau nanti membuat query pencocokan no_journal ke tabel sumber: tbl_list_journal.no_journal bertipe utf8mb4, sedangkan bpb.bpbno_int, memo_h.nm_memo, dan c_petty_cashout_h.no_pco bertipe latin1. Tanpa CONVERT() ke charset kolomnya, MariaDB mengonversi sisi KOLOM sehingga indeks TIDAK terpakai - terukur 92 ms/baris tanpa CONVERT vs 0,44 ms dengan CONVERT (sekitar 200x).',
                        'Bug lama yang ikut diperbaiki: cancel_bank_out.php dan proses_approve_update_bpb_fabric.php melakukan INSERT..SELECT TANPA daftar kolom, sehingga sudah gagal diam-diam sejak kolom faktur_pajak/tgl_faktur_pajak ditambah. Keduanya kini memakai daftar kolom eksplisit.',
            ...
            'general-ledger-new' => [
                ...
                'doc' => [
                    ...
                    'notes' => [
                        ...
                        'Popup detail jurnal (klik No Journal, via <code>ajx_gl_detail.php</code>) menampilkan Created By &amp; Created Date dari kolom <code>create_by</code>/<code>create_date</code> tbl_list_journal (kolom <code>created_at</code> selalu kosong, tidak dipakai). Diambil dari baris yang PALING AWAL dibuat, bukan baris pertama urutan tampil.',
                        'Arti "system" di Created By: keempat penyimpan Memorial Journal (save_mj_input/hris/upload_fix/ppn.php) memakai <code>$_SESSION[\'username\'] ?? \'system\'</code> — artinya sesi login pengguna sudah habis saat Save ditekan, tapi penyimpanan tetap diteruskan. Sepanjang 2026 terjadi di 3 jurnal GM (dua di antaranya jurnal gaji besar yang lama disiapkan). Sedangkan Created By KOSONG pada jurnal MEALS berasal dari proses terjadwal harian (±10:55), bukan pengguna.',
                    ],
                ],
            ],
            'trial-balance' => [
            ...
            'other-receivable' => [
                ...
                'doc' => [
                    ...
                    'notes' => [
                        'COA 1.34.05 — kolom <b>Forex Gain / (Loss)</b> pada dasarnya SELALU 0 untuk BPB bermata uang USD. Rumusnya <code>tot_dn - total_alk2</code> membandingkan dua angka yang sama-sama dikali kurs ALOKASI, jadi selisih kurs antara tanggal BPB dan tanggal alokasi tidak pernah tertangkap. Rumus itu memang dirancang untuk kasus sebaliknya (BPB IDR + alokasi valas) dan dibiarkan apa adanya. Selisih kurs pada praktiknya ditutup akuntansi lewat jurnal reklas GM, sehingga muncul di kolom Deduction (GM).',
                        'PENGECUALIAN PENYAJIAN (16 Sep 2026, permintaan user): untuk 12 BPB yang alokasinya di 0043/ALK/NAG/0826, selisih kurs DIHITUNG SENDIRI dari <b>jumlah USD x (kurs PAJAK tgl BPB - kurs alokasi 17.796)</b> lalu ditampilkan di kolom <b>Forex Gain / (Loss)</b>, tanpa mengambil angka dari jurnal <code>GM/NAG/0826/00197</code> (31-Ags-2026, total 472.818,49), karena jurnal itu akan dibatalkan user. Jurnal GM-nya SENGAJA TETAP DITAMPILKAN apa adanya di kolom Deduction (GM) — atas permintaan user — supaya perbedaan sebelum &amp; sesudah cancel terlihat: <b>selama GM belum dibatalkan</b> nilainya terhitung dua kali sehingga saldo akhir 12 baris ini tampil MINUS 472.818,49 (disengaja, sebagai penanda bahwa jurnal GM masih perlu dibatalkan — bukan bug); <b>sesudah GM dibatalkan</b> Deduction (GM) jadi 0, tinggal kolom Forex, saldo akhir Agustus 2026 = 0 dan saldo awal September 2026 = 0. Sudah diuji di dua kondisi: database live (GM masih ada) memberi saldo akhir -472.818,49, database lokal (GM tidak ada) memberi 0. Dibandingkan baris-per-baris dgn versi sebelumnya, hanya 12 baris ini yang berubah di periode Agustus; periode Juli &amp; September tidak ada satu baris pun yang bergeser. JANGAN menambah perhitungan selisih kurs lain di atas ini — nilainya akan terhitung dua kali dan saldo akhir jadi minus.',
                        'Kalau ada laporan "saldo menggantung" di menu ini, cek dulu tanggal To filter-nya: jurnal reklas GM sering bertanggal akhir bulan (mis. 31-Ags), sehingga export yang berhenti di tanggal 30 belum memuat pengurangnya.',
            ...
            'purchase-advance' => [
                ...
                'doc' => [
                    ...
                    'notes' => [
                        'Perubahan (24 Sep 2026) - UANG MUKA CBD/DP DIAMBIL PENUH, SEBELUM DIPOTONG PPh. Nilai uang muka untuk dokumen CBD/DP dulu dibaca dari <code>kontrabon_h_cbd/_dp.total</code> yang sudah NETTO (setelah potong PPh), padahal jurnalnya mendebit COA uang muka sebesar nilai PENUH - PPh-nya disetor ke kantor pajak atas nama supplier, jadi uang muka ke supplier tetap sebesar nilai penuh. Akibatnya report tidak cocok dengan buku besar. Sekarang memakai <code>(total + COALESCE(pph,0))</code> di kedua berkas (purchase_advance_report.php &amp; ekspor_pa_report_new.php, masing-masing 2 tempat: blok Pemasukan Before dan Pemasukan). Contoh BK/BCA1979/NAG/0826/00296: dulu 3.729.375, sekarang 3.825.000 - sama persis dengan debit jurnal 1.49.99. Diuji dengan menjalankan query LAMA vs BARU berdampingan ke data produksi: jumlah baris SAMA (2.332) dan hanya 4 baris yang nilainya berubah, yaitu tepat 4 dokumen CBD ber-PPh yang ada (dari 682 CBD + 28 DP); dokumen tanpa PPh tidak tersentuh karena <code>pph = 0</code>. Sengaja TIDAK memakai kolom <code>subtotal</code> (80 baris CBD &amp; 26 baris DP punya subtotal != total + pph) dan TIDAK mengambil dari jurnal langsung (sempat dicoba, tetapi ikut memunculkan 4 baris lama yang atribusi per-FTR-nya tertukar - di luar lingkup permintaan).',
                        'Catatan lanjutan: sisi PENGURANG (Deduction KB) mengikuti jurnal PV penyelesainya. Selama jurnal PV masih mengkredit nilai netto, Ending Balance akan menyisakan sebesar PPh - dan itu memang BENAR, karena di buku besar pun masih menggantung. User menyesuaikan jurnal PV-nya sendiri supaya mengkredit penuh (per 24 Sep 2026: PV-AP/REG/NAG/2026/09/02283 sudah, 02282 kurang 95.625, 02284 kurang 81.250). Penyesuaian PPh uang muka di AP Report (CTE <code>pph_um_all</code> pada ap_report2/query_pv2.php) otomatis MATI per dokumen begitu jurnalnya diperbaiki, karena syarat HAVING-nya hanya terpenuhi saat jurnal masih netto - jadi tidak akan dobel hitung. Sudah diverifikasi: 02283 kini mati, 02282 &amp; 02284 masih aktif.',
            ...
            'project-dashboard' => [
                ...
                'doc' => [
                    ...
                    'flow' => [
                        ...
                        ['title' => 'Hitung status khusus', 'desc' => 'Overdue = status Planned/On Progress dan target_date sudah lewat (dihitung di SQL ajax_project.php; Done &amp; Live tidak pernah overdue). At Risk = On Progress, belum overdue, tapi progress tertinggal lebih dari 15 poin dari yang seharusnya menurut waktu berjalan. Completion tile mengeluarkan item overdue dari basis hitungan.'],
                        ...
                        ['title' => 'Export Excel', 'desc' => 'ekspor_project.php mengikuti filter module/bulan/pencarian, tetapi isinya SELALU hanya project yang SUDAH SELESAI — sejak 24 Sep 2026 berarti status Done <b>dan</b> Live (permintaan user: semua yang done ikut, baik sudah maupun belum live), keduanya dicetak sebagai STATUS = DONE. Filter status di layar tidak mengubah isi file. Kolom tanggalnya memakai <code>actual_date</code> = TANGGAL DONE, bukan tanggal live; bulan juga diambil dari actual_date sehingga project yang selesai Juli lalu baru live September tetap muncul di bulan Juli.'],
                    ],
                    'status_flow' => [
                    ...
                    'notes' => [
                        'Perubahan (24 Sep 2026) - KOLOM "On Hold" DIHAPUS dan "Done" DIPECAH DUA. Papan sekarang: Planned &rarr; On Progress &rarr; <b>Done</b> (pekerjaan selesai tetapi BELUM di-deploy) &rarr; <b>Live</b> (sudah jalan di produksi). Saat diterapkan, produksi tidak punya satu pun baris "On Hold" (Done 34, Planned 5, On Progress 1), jadi tidak ada data yang perlu dipindahkan; <code>migrasi_status_project.sql</code> tetap menyediakan jaring pengaman yang mengubah sisa "On Hold" jadi Planned. Semua baris Done yang sudah ada DIBIARKAN sebagai Done - tinggal digeser ke kolom Live lewat drag &amp; drop, atau lewat salah satu contoh UPDATE di berkas SQL yang sama.',
                        'Perubahan (24 Sep 2026) - TANGGAL DONE DIPISAH DARI TANGGAL LIVE. Kolom baru <code>master_project.live_date</code>; <code>actual_date</code> tetap berarti TANGGAL SELESAI DIKERJAKAN. Pemisahan ini yang menjamin Export Excel selalu memakai tanggal done: saat kartu digeser ke kolom Live, <code>quick_update_status.php</code> memakai <code>actual_date = COALESCE(actual_date, CURDATE())</code> sehingga tanggal selesai yang sudah ada TIDAK tertimpa, sementara <code>live_date = COALESCE(live_date, CURDATE())</code> menyimpan tanggal go-live secara terpisah. Di formulir, "Go-Live Date" hanya muncul kalau statusnya Live. Kalau sebuah project langsung dibuat/dipindah ke Live tanpa pernah melewati Done, dua-duanya distempel hari yang sama - tidak ada informasi lain yang bisa dipakai. Menggeser Live kembali ke Done sengaja TIDAK menghapus live_date, supaya tanggal yang pernah diisi manual tidak hilang karena salah geser.',
                        'Semua perhitungan "sudah selesai" (completion rate, on-time delivery, lead time, sembunyikan hitungan mundur due date, isi Export Excel) memakai satu helper <code>isDoneStatus()</code> di project.php yang mengembalikan true untuk Done MAUPUN Live - supaya tidak ada satu pun tempat yang cuma mengecek "Done" lalu melewatkan "Live" dan membuat angka completion anjlok begitu kartu dipindah ke Live. Tile "Done + Live" menampilkan gabungan keduanya, tile "Live" hanya yang sudah live.',
