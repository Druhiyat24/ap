...
<table id="mytable2" class="table table-striped table-bordered" role="grid" cellspacing="0" width="100%">
 ...
 $sql = mysqli_query($conn2,"SELECT nm_memo,tgl_memo,no_invoice,supplier,buyer,IF(tgl_memo < '$start_date',total_memo * rate,0) beg_balance, IF(tgl_memo >= '$start_date',total_memo * rate,0) addition,(total_bk * rate) deduction, (total_bk2 * rate) deduction2 FROM (SELECT a.nm_memo,tgl_memo,no_invoice,supplier,buyer,total_memo,COALESCE(ttl_amount,0) total_bk, COALESCE(ttl_amount2,0) total_bk2, a.curr, IFNULL(mr.rate,1) rate FROM (
