...
<div class="tableFix" style="height: 350px;">        
    <table id="datatable" class="table table-striped table-bordered text-nowrap" style="font-size: 12px;" role="grid" cellspacing="0" width="100%">
        ...
        <tbody>
            ...
            $sql = mysqli_query($conn1,"select * from (select * from(
                ...
                (select no_kbon no_journal, no_coa,nama_coa from kontrabon_h where no_coa != '') b on b.no_journal = a.no_kbon where nama_supp = '$nama_supp'");
        }
        ...
        $sqldel2 = "ALTER TABLE rpt_ap_kbon";
    ...
    // $no_bpbap12 = isset($rowap12['no_kbon']) ? $rowap12['no_kbon'] : null;
            ...
            $sql_rvs = mysqli_query($conn1,"select no_kbon,created_date, sum(total) total, sum(total_new) total_new from (select a.no_kbon, created_date, (COALESCE(total_before,0) - coalesce(total_ftr,0) + coalesce(c.total,0)) total, total_new from (SELECT 
                ...
                SUM(CASE WHEN q1.no_bpb IS NULL THEN q2.total ELSE 0 END) AS total_new
                FROM (
                SELECT 
                a.no_kbon_new no_kbon, 
                DATE_FORMAT(b.create_date,'%Y-%m-%d') created_date, 
                c.no_bpb, 
                (c.subtotal + c.tax) total
                FROM ap_edit_kontrabon_log a 
                INNER JOIN kontrabon_h b ON b.no_kbon = a.no_kbon_new 
                INNER JOIN kontrabon c ON c.no_kbon = a.no_kbon_new 
                ...
                FROM ap_edit_kontrabon_log a 
                INNER JOIN kontrabon_h b ON b.no_kbon = a.no_kbon_new 
                INNER JOIN kontrabon c ON c.no_kbon = a.no_kbon 
                WHERE b.status != 'Cancel'
                ) q1 
