<div style="margin-left: 10px;margin-bottom: 10px;">
        ...
        echo '<a target="_blank" href="pcs_detail_summary.php?nama_supp='.$nama_supp.' && start_date='.$start_date.' && end_date='.$end_date.' && user='.$user.'"><button type="button" class="btn btn-success"><i class="fa fa-file-excel-o" aria-hidden="true" style="padding-right: 10px; padding-left: 5px;font-size:11px;"> EXCEL SUMMARY </i></button></a>';
        ?>
    </div> 
...
<table id="datatable" class="table table-striped table-bordered text-nowrap" style="font-size: 12px;" role="grid" cellspacing="0" width="100%">
    ...
    $jml_rate = isset($row_rate['rate']) ? $row_rate['rate'] : 1;
        ...
        $sql = mysqli_query($conn1,"select Supplier, curr, saldo_awal,addition, deduction_adj, deduction_lp, adjustment, (saldo_awal + addition + deduction_adj + deduction_lp + adjustment) saldo_akhir from (select Supplier,curr,(sal_awal_bpb + sal_awal_kbon + sal_awal_lp) saldo_awal, add_bpb addition, adj_kbon deduction_adj,ded_lp deduction_lp, ((add_lp - ded_kbon) + (add_kbon - ded_bpb) + reverse) adjustment from (select Supplier, supp.curr, COALESCE(sal_awal_bpb,0) sal_awal_bpb, COALESCE(add_bpb,0) add_bpb, COALESCE(ded_bpb,0) ded_bpb, COALESCE(sal_akhir_bpb,0) sal_akhir_bpb, COALESCE(sal_awal_kbon,0) sal_awal_kbon, COALESCE(add_kbon,0) add_kbon, COALESCE(adj_kbon,0) adj_kbon, COALESCE(ded_kbon,0) ded_kbon, COALESCE(sal_akhir_kbon,0) sal_akhir_kbon, COALESCE(sal_awal_lp,0) sal_awal_lp, COALESCE(add_lp,0) add_lp, COALESCE(ded_lp * -1,0) ded_lp, COALESCE(sal_akhir_lp,0) sal_akhir_lp, COALESCE(reverse,0) reverse from (select DISTINCT b.Supplier,a.curr from bpb a inner join mastersupplier b on b.Id_Supplier = a.id_supplier where a.curr != '' order by b.Supplier asc) supp 
left join
(select nama_supp supp_bpb, curr, sum(beg_balance) sal_awal_bpb,sum(addition) add_bpb,sum(deduction) ded_bpb,sum(end_balance) sal_akhir_bpb from rpt_ap_bpb where user_create = '$user' GROUP BY curr,nama_supp) bpb on bpb.supp_bpb = supp.Supplier and bpb.curr = supp.curr
left join
(select nama_supp supp_kbon, curr, sum(beg_balance) sal_awal_kbon,sum(add_bpb) add_kbon,sum(add_adj) adj_kbon,sum(deduction) ded_kbon,sum(end_balance) sal_akhir_kbon, sum(reverse) reverse from rpt_ap_kbon where user_create = '$user' GROUP BY curr,nama_supp) kbon on kbon.supp_kbon = supp.Supplier and kbon.curr = supp.curr
left join
(select nama_supp supp_lp, curr, sum(beg_balance) sal_awal_lp,sum(addition) add_lp,sum(deduction) ded_lp,sum(end_balance) sal_akhir_lp from rpt_ap_lp where user_create = '$user' GROUP BY curr,nama_supp) lp on lp.supp_lp = supp.Supplier and lp.curr = supp.curr) a) a");
