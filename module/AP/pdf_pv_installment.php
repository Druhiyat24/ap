...
$no_kbon_det = $_GET['no_kbon_det'];
ob_start();
error_reporting(E_ALL);
ini_set('display_errors', 1);

// BPB/RO/FTR/jurnal Installment disimpan di tabel yang sama seperti Regular
// (tidak dipecah per cicilan) - jadi seluruh ringkasan finansial di bawah ini
// sama persis dengan pdf_pv_regular.php, bersumber dari $no_kbon (induk).
// Yang khas Installment cuma dua baris NOTE paling bawah (cicilan ke berapa
// dari berapa & nominal cicilan ini), bersumber dari kontrabon_h_installment_detail
// via $no_kbon_det (anak) yang diklik di listing.
$sqlDet = mysqli_query($conn2, "select no_kbon, cicilan_ke, total_cicilan, tgl_tempo, total from kontrabon_h_installment_detail where no_kbon_det = '$no_kbon_det'");
$rowDet = mysqli_fetch_assoc($sqlDet);
$no_kbon = $rowDet['no_kbon'];
$cicilan_ke = $rowDet['cicilan_ke'];
$total_cicilan_count = $rowDet['total_cicilan'];
$tgl_tempo_cicilan = $rowDet['tgl_tempo'];
$total_cicilan_amount = $rowDet['total'];
?>
...
*/
	...
	<title>PAYMENT VOUCHER INSTALLMENT</title>
</head>
<body style=" padding-left:5%; padding-right:5%;">
	...
	<table width="100%">
		<tr>
			<td ><h5>PAYMENT VOUCHER : <?php echo $no_kbon_det ?></h5></td>
			<td>&nbsp;</td>
			<td>&nbsp;</td>
			<td>&nbsp;</td>
			<td align="right"><h5>
...
<table style="font-size:11px;">
	...
	<td style="text-align:left;padding-top: -15px;padding-bottom: -10px;">
		<?php
		echo date("d M Y", strtotime($tgl_tempo_cicilan));
		?>
	</td>
	<td style="text-align:left;padding-top: -15px;padding-bottom: -10px;">
      <?php
...
<table  border="1" cellspacing="0" style="width:100%;font-size:10px;border-spacing:2px;">
  ...
  <tbody >
  	...
  	while($data=mysqli_fetch_array($query)){
  		...
  		echo '<tr>
  		<td style="width:18%;text-align:left;vertical-align:top;">'.$no_po.'</td>
  		<td style="width:9%;text-align:left;vertical-align:top;">'.$tgl_po.'</td>
  		<td style="width:15%;text-align:left;vertical-align:top;">'.$no_bpb.'</td>
  		<td style="width:9%;text-align:left;vertical-align:top;">'.date("d M Y",strtotime($tgl_bpb)).'</td>
  		<td style="width:20%;text-align:left;vertical-align:top;">'.$item_desc.'</td>
  		<td style="width:8%;text-align:left;vertical-align:top;">'.number_format($qty, 2).' '.$uom.'</td>
  		<td style="width:9%;text-align:right;vertical-align:top;">'.number_format($price, 2).'</td>
  		<td style="width:12%;text-align:right;vertical-align:top;">'.number_format($subtotal, 2).'</td>
  		</tr>';
  	};
  	?>
...
<div style="margin-bottom: 2.54cm; page-break-inside: avoid;">
	<table width="100%" border="0" style="page-break-inside: avoid;font-size:11px;">
		...
		<tr>
			...
			<td width="46%">: <?php echo $bank_name; ?></td>
		...
		<tr>
			...
			<td width="46%">: <?php echo $bank_currency; ?></td>
		...
		<tr>
			...
			<td style="font-weight: bold;">
				Installment <?php echo $cicilan_ke." of ".$total_cicilan_count ?>
			</td>
			<td style="width:1%">:</td>
			<td style="text-align:right;font-weight: bold;">
				<?php echo $curr." ".number_format($total_cicilan_amount, 2) ?>
			</td>
		</tr>
...
<table style="page-break-inside: avoid; font-size:11px;" border="0">
	...
	<tr>
		<td>Payment Voucher Number : <?php echo $no_kbon_det ?></td>
	</tr>
	<tr>
		<td>Total Payment Voucher : <?php echo $curr." ".number_format($jml_total, 2) ?></td>
	</tr>
	<tr>
		<td>Installment : <?php echo $cicilan_ke." of ".$total_cicilan_count ?></td>
	</tr>
	<tr>
		<td>Installment Amount : <?php echo $curr." ".number_format($total_cicilan_amount, 2) ?></td>
	</tr>
</table>
</div>
