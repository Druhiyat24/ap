...
$rvs_number=$_GET['rvs_number'];
?>

<?php
$sql= "select rvs_number, doc_number, doc_date, nama_supp, curr, total, deskripsi from ap_reverse_det where rvs_number = '$rvs_number' and status = 'Y'";

$rs=mysqli_fetch_array(mysql_query($conn2,$sql));

$sql_header = mysqli_query($conn2,"select DISTINCT rvs_number, rvs_date, DATE_FORMAT(created_date,'%Y-%m-%d') created_date, upper(type_doc) type_doc, upper(deskripsi) deskripsi from ap_reverse_h where rvs_number = '$rvs_number'");
$rows_header = mysqli_fetch_array($sql_header);
$rvs_number = $rows_header['rvs_number'];
$rvs_date = $rows_header['rvs_date'];
$created_date = $rows_header['created_date'];
$type_doc = $rows_header['type_doc'];
$deskripsi = $rows_header['deskripsi'];
ob_start();
?>
...
<table style="font-size:12px;">
		...
		<tbody>
			<tr>  	      
				<td style="text-align:left;padding-left: 15px;padding-top: -10px;padding-bottom: -10px;">
					<?php
					echo strtoupper(date("d M Y", strtotime($rvs_date)));
					?>		
				</td>
				<td style="text-align:left;padding-left: 25px;padding-top: -10px;padding-bottom: -10px;">
					<?php
					echo strtoupper(date("d M Y", strtotime($created_date)));
					?>		
				</td>
				<td style="text-align:left;padding-left: 15px;padding-top: -10px;padding-bottom: -10px;"><?php echo $type_doc; ?></td>								      
			</tr>
		</tbody>
	</table>
	<hr />
	...
	<table style="font-size:12px;">
		...
		<table  border="1" cellspacing="0" style="width:100%;font-size:11px;border-spacing:2px;">
			<tr>
				<th style="width:23%;border: 1px solid black;text-align:center;">Supplier</th>
				<th style="width:20%;border: 1px solid black;text-align:center;">Document Number</th>
				<th style="width:12%;border: 1px solid black;text-align:center;">Document Date</th>
				<th style="width:10%;border: 1px solid black;text-align:center;">Curr</th> 
				<th style="width:15%;border: 1px solid black;text-align:center;">Total</th>     
				<th style="width:20%;border: 1px solid black;text-align:center;">Description</th>
			...
			<tbody>
				...
				while($data=mysqli_fetch_array($query)){
					...
					<td style="text-align:center;">'.date("d M Y",strtotime($doc_date)).'</td>
