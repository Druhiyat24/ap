...
<?php
$sql= "select no_bpb,tgl_bpb,nama_supp,no_po,curr,total,status,if(keterangan = '','-',keterangan) ket,CASE
...
END as keterangan from ir_trans_bpb where no_transfer = '$doc_number' GROUP BY id order by no_bpb asc";
...
<table style="font-size:12px;">
	...
	<td style="text-align:left;padding-left: 25px;padding-top: -10px;padding-bottom: -10px;">
      <?php
      $sql2 = mysqli_query($conn2,"select DISTINCT DATE_FORMAT(created_at,'%Y-%m-%d') created_date from ir_trans_bpb where no_transfer = '$doc_number'");
	...
	<td style="text-align:left;padding-left: 15px;padding-top: -10px;padding-bottom: -10px;">
      <?php
      $sql3 = mysqli_query($conn2,"select DISTINCT tgl_transfer from ir_trans_bpb where no_transfer = '$doc_number'");
      $rows3 = mysqli_fetch_array($sql3);
      	$tglftrcbd = $rows3['tgl_transfer'];
		echo date("d M Y", strtotime($tglftrcbd));
		?>		
	</td>
	<td style="text-align:left;padding-left: 15px;padding-top: -10px;padding-bottom: -10px;">TRANSFER BPB</td>								      
	</tr>
</tbody>
</table>
<hr />
...
<table  border="1" cellspacing="0" style="width:100%;font-size:12px;border-spacing:2px;">
  <tr>
      ...
      <!-- <th style="width:8%;border: 1px solid black;text-align:center;">Curr</th>
...
while($data=mysqli_fetch_array($query)){
   ...
   echo '<tr >
      <td style="text-align:center;">'.$no_bpb.'</td>
