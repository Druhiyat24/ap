...
$akun      = mysqli_real_escape_string($conn2, $_POST['account1']);
$curr      = mysqli_real_escape_string($conn2, $_POST['currency1']);
...
$sqlChk = dbExec($conn2, "SELECT status, coa_akun FROM c_petty_cashin_h WHERE no_pci = '$doc_num' AND reff = 'Cash Out' LIMIT 1 FOR UPDATE");
...
/* =========================
   ...
   reset label jurnal ke 'Cash Out' lagi (jangan lanjutkan Rev dari nomor lama).
========================= */
...
if ($willRenumber) {
    ...
    $type_journal = "Cash Out (Rev $nextRev)";
}
...
$buyer  = $_POST['buyer1'];
$no_ws  = $_POST['no_ws1'];
$curr_d = $_POST['currenc1'];
$ket    = $_POST['keterangan1'];
$debit  = $_POST['txt_amount1'];
$credit = $_POST['txt_credit1'];

$detRows     = [];
$journalRows = ["('$doc_num', '$doc_date', '$type_journal', '$akun', '$nama_coa', '-', '-', '$reff_doc', '', '-', '-', 'IDR', '1', '$amount', '0', '$amount', '0', 'Draft', '$desc', '$user', '$create_date', '', '', '', '', '$pc_kas')"];
...
for($i=0;$i<count($coa);$i++){
    ...
    $journalRows[] = "('$doc_num', '$doc_date', '$type_journal', '$no_coa', '$nama_coa_d', '$cc_i', '$nama_cc', '$reff_doc', '', '$buyer_i', '$ws_i', '$curr_i', '1', '$d_debit', '$d_credit', '$d_debit', '$d_credit', 'Draft', '$ket_i', '$user', '$create_date', '', '', '', '', '$pc_i')";
