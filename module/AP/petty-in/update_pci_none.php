...
$sqlChk = dbExec($conn2, "SELECT status, coa_akun FROM c_petty_cashin_h WHERE no_pci = '$doc_num' AND reff = 'None' LIMIT 1 FOR UPDATE");
...
/* =========================
   ...
   reset label jurnal ke 'None' lagi (jangan lanjutkan Rev dari nomor lama).
========================= */
...
SET tgl_pci = '$doc_date', coa_akun = '$akun', curr = '$curr', amount = '$amount', deskripsi = '$desc'
...
$detRows     = [];
$journalRows = ["('$doc_num', '$doc_date', '$type_journal', '$akun', '$nama_coa', '-', '-', '-', '', '-', '-', 'IDR', '1', '$amount', '0', '$amount', '0', 'Draft', '$desc', '$user', '$create_date', '', '', '', '', '$pc_kas')"];
...
for($i=0;$i<count($coa);$i++){
    ...
    $journalRows[] = "('$doc_num', '$doc_date', '$type_journal', '$no_coa', '$nama_coa_d', '$cc_i', '$nama_cc', '-', '', '$buyer_i', '$ws_i', '$curr_i', '1', '$d_debit', '$d_credit', '$d_debit', '$d_credit', 'Draft', '$ket_i', '$user', '$create_date', '', '', '', '', '$pc_i')";
