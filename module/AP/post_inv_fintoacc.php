<?php include '../header.php' ?>
    ...
    <!-- MAIN -->
    <div class="col p-4">
        <h3 class="text-center">TRANSFER INVOICE FROM FINANCE TO ACCOUNTING</h3>
<div class="box">
...
<form id="form-data" method="post">
        <div class="form-row">
            ...
            <?php
            $sql = mysqli_query($conn2,"select max(substr(no_trans,15)) no_trans from ir_log_trans where kode_trans = 'TFTA'");
    ...
    <div class="box body mt-2">
        <div class="row">
            ...
            <table id="mytable" class="table table-striped table-bordered text-nowrap" cellspacing="0" width="100%" style="font-size: 12px;text-align:center;">
                    <thead>
                        <tr class="bg-dark text-white">
                            ...
                            <th style="width:20%;">No Kontrabon</th>
                            <th style="width:20%;">Kontrabon Date</th>                            
            ...
            if ($nama_supp == 'ALL') {
                $sql = mysqli_query($conn2,"select doc_number,tgl_penerimaan,nama_supp,total_amount, status, CONCAT(created_by,' (',created_date,')') create_user from ir_invoice_supp_h where status = 'received' and tgl_penerimaan between '$start_date' and '$end_date' order by tgl_penerimaan asc");
            }else{
                $sql = mysqli_query($conn2,"select doc_number,tgl_penerimaan,nama_supp,total_amount, status, CONCAT(created_by,' (',created_date,')') create_user from ir_invoice_supp_h where status = 'received' and nama_supp = '$nama_supp' and tgl_penerimaan between '$start_date' and '$end_date' order by tgl_penerimaan asc");
            }
...
$("input[type=checkbox]").change(function(){
    ...
    var amount = parseFloat($(this).closest('tr').find('td:eq(4)').attr('value')) || 0; 
            ...
            success: function(response){
                console.log(response);
                 $("input[type=checkbox]:checked").each(function () {
                    ...
                    var nama_supp = $(this).closest('tr').find('td:eq(3)').attr('value');
                    var amount = $(this).closest('tr').find('td:eq(4)').attr('value');
                    var create_user = '<?php echo $user; ?>';     
                    var unik_code = document.getElementById('unik_code').value;
                    var kode_trans = 'TFTA';    
...
<script type="text/javascript">     
    ...
    var no_document = $(this).closest('tr').find('td:eq(1)').attr('value'); 
    var tgl_doc = $(this).closest('tr').find('td:eq(2)').attr('value'); 
