...
<style>
  ...
  /* ---------- Stat tiles ---------- */
  .stat-tile {
    position: relative; border-radius: 14px; padding: 15px 16px; height: 100%; overflow: hidden;
    background: var(--surface); border: 1px solid var(--line);
    box-shadow: 0 2px 10px rgba(23,27,46,.05);
    transition: box-shadow .18s ease, transform .18s ease;
  }
  .stat-tile:hover { box-shadow: 0 8px 20px rgba(23,27,46,.10); transform: translateY(-2px); }
  .stat-tile .icon-badge {
    width: 34px; height: 34px; border-radius: 10px; display:flex; align-items:center; justify-content:center;
    font-size: 14px; color:#fff; margin-bottom: 10px;
  }
  .stat-tile .num  { font-size: 24px; font-weight: 800; line-height: 1; color: var(--ink); font-variant-numeric: tabular-nums; display:flex; align-items:center; }
  .stat-tile .lbl  { font-size: 11px; color: var(--ink-soft); margin-top: 4px; font-weight: 700; letter-spacing: .03em; text-transform: uppercase; }
  ...
  .tile-rate   { background: linear-gradient(135deg,#8a4b0a,#c17f2e); color:#fff; display:flex; align-items:center; justify-content:space-between; }
  .tile-rate .num, .tile-rate .lbl { color: #fff; }
  .ring { --deg: 0deg; width: 46px; height: 46px; border-radius: 50%; background: conic-gradient(#fff var(--deg), rgba(255,255,255,.28) 0); display:flex; align-items:center; justify-content:center; flex-shrink:0; }
  .ring span { width: 36px; height: 36px; border-radius: 50%; background: rgba(76,29,149,.95); display:flex; align-items:center; justify-content:center; font-size: 11px; font-weight: 800; color:#fff; }
  ...
  .search-wrap input {
    ...
    background: #fbfbf9; transition: all .15s ease;
  }
  ...
  .proj-card .footer-row { display:flex; align-items:center; justify-content: flex-end; margin-top: 9px; padding-top: 9px; border-top: 1px dashed var(--line); }
  ...
  .proj-table thead th {
    ...
    background: #fbfbf9; border-bottom: 2px solid var(--line); border-right: 1px solid var(--line-soft);
  }
  .proj-table thead th:hover { color: var(--accent); background: var(--line-soft); }
  .proj-table thead th.no-sort { cursor: default; }
  .proj-table thead th.no-sort:hover { color: var(--ink-soft); background: #fbfbf9; }
  ...
  .proj-table tbody tr:nth-child(even) { background: #fafbf8; }
  ...
  .cal-dow-row { display:grid; grid-template-columns: repeat(7, 1fr); border-bottom:1px solid var(--line); background:#fbfbfd; }
  ...
  .cal-daycell.outside { background:#fbfbfd; }
  ...
  .tl-head-row { display:flex; position: sticky; top:0; z-index: 4; background: #fbfbf9; border-bottom: 2px solid var(--line); }
  .tl-head-label {
    flex: 0 0 210px; position: sticky; left:0; z-index: 6; background:#fbfbf9;
  ...
  .tl-row:hover { background: #fafbf8; }
  .tl-row-label {
  ...
  .tl-row:hover .tl-row-label { background-color: #fafbf8; }
...
<div class="proj-page">
  ...
  <div class="row mt-3" id="stat-tiles">
    ...
    <div class="col-6 col-lg mb-3">
      <div class="stat-tile" id="overdue-tile">
...
<script>
  ...
  function updateMonthList() {
    ...
    allProjects.forEach(function (p) {
      ...
      var m = p.target_date.substring(0, 7); // YYYY-MM
  ...
  function getFiltered(opts) {
    ...
    return allProjects.filter(function (p) {
      ...
      if (month && (!p.target_date || p.target_date.substring(0, 7) !== month)) return false;
  ...
  function cardHtml(p, idx, compact) {
    ...
    return '' +
      '<div class="' + cardCls + '"' + dragAttrs + ' style="--mod-color:' + mc.c + ';--mod-color-soft:' + mc.s + ';animation-delay:' + Math.min((idx||0) * 35, 350) + 'ms;">' +
        ...
        '</div>' +
        '<div class="progress"><div class="progress-bar" style="width:' + p.progress + '%;background:' + progressColor(p.status) + ';"></div></div>' +
        '<div class="footer-row">' +
          '<button class="icon-btn mr-1" onclick="openEdit(' + p.id + ')" title="Edit"><i class="fa fa-pencil"></i></button>' +
          '<button class="icon-btn danger" onclick="doDelete(' + p.id + ',\'' + escJs(p.project_name) + '\')" title="Delete"><i class="fa fa-trash"></i></button>' +
        '</div>' +
      '</div>';
  }
  ...
  function renderList() {
    ...
    list.forEach(function (p, idx) {
      ...
      html += '<tr>' +
        ...
        '<td><select class="status-select" style="background-color:' + sc.bg + ';color:' + sc.fg + ';" onchange="quickStatusChange(' + p.id + ', this.value)">' + statusOpts + '</select></td>' +
        '<td>' + (fmtDate(p.start_date) || '-') + '</td>' +
        '<td>' + (fmtDate(p.target_date) || '-') + '</td>' +
        '<td>' + (secondary || '<span class="text-muted">&mdash;</span>') + '</td>' +
        '<td class="text-right"><button class="icon-btn mr-1" onclick="openEdit(' + p.id + ')" title="Edit"><i class="fa fa-pencil"></i></button>' +
  ...
  function renderCalendar() {
    ...
    for (var w = 0; w < numWeeks; w++) {
      ...
      segs.forEach(function (seg) {
        ...
        var cls = 'cal-bar' + (seg.contLeft ? ' cont-left' : '') + (seg.contRight ? ' cont-right' : '');
        html += '<span class="' + cls + '" style="grid-column:' + (seg.startCol + 1) + ' / ' + (seg.endCol + 2) + ';grid-row:' + (seg.lane + 1) + ';background:' + mc.s + ';color:' + mc.c + ';" onclick="openEdit(' + seg.p.id + ')" title="' + escJs(seg.p.project_name) + '">' +
          '<span class="bdot" style="background:' + mc.c + ';"></span>' + escHtml(seg.p.project_name) +
          '</span>';
      });
      html += '</div></div>';
    }
    ...
    rows.forEach(function (r) {
      ...
      html += '<div class="tl-row">' +
        ...
        '<div class="tl-row-track">' + todayLine +
          '<div class="tl-bar' + (r.contLeft ? ' cont-left' : '') + (r.contRight ? ' cont-right' : '') +
            '" style="left:calc(' + leftPct + '% + 2px);width:calc(' + widthPct + '% - 4px);background:' + mc.s + ';" onclick="openEdit(' + r.p.id + ')" title="' + escJs(r.p.project_name + ' — ' + r.p.progress + '%') + '">' +
  ...
  $(document).on('input change', '#pj-name, #pj-category, #pj-description, #pj-start, #pj-target, #pj-actual', updatePreview);
  ...
  function openCreate() {
    ...
    $('#pj-id').val('');
    $('#pj-name, #pj-category, #pj-description, #pj-start, #pj-target, #pj-actual').val('');
