...
bpb_credit as (select '' id, bpbno_int no_journal, bpbdate tgl_journal, 'AP - BPB RETURN' type_journal, c.no_coa, c.nama_coa, IF(SUBSTR(bpbno_int,1,3) = 'WIP','DEP04SUB004','-') no_costcenter, IF(SUBSTR(bpbno_int,1,3) = 'WIP','DISTRIBUTION CENTER','-') nama_costcenter, '-' reff_doc, '' reff_date, '-' buyer, '-' no_ws, a.curr, IFNULL(rate,1) rate, 0 debit, a.total credit, 0 debit_idr, ROUND(a.total * IFNULL(rate,1),2) credit_idr, 'APPROVED' status, CONCAT(kata1,' ',bpbno_int,' DARI ', UPPER(supplier)) keterangan, username create_by, dateinput create_date, confirm_by approve_by, confirm_date approve_date, '-' cancel_by, '' cancel_date, CURRENT_TIMESTAMP() created_at, CURRENT_TIMESTAMP() updated_at, profit_center from bpb a LEFT JOIN rate b on b.curr = a.curr and b.tanggal = a.bpbdate INNER JOIN ap_mapping_coa_jurnal c on c.mattype = a.mattype and c.ctg_supp = a.ctg_supp and c.matclass = a.matclass and c.n_code_ctg = a.n_code_category where c.type_mapping = 'bpb_credit' ),
                        
bpb_debit as (select '' id, bpbno_int no_journal, bpbdate tgl_journal, 'AP - BPB RETURN' type_journal, c.no_coa, c.nama_coa, '-' no_costcenter, '-' nama_costcenter, '-' reff_doc, '' reff_date, '-' buyer, '-' no_ws, a.curr, IFNULL(rate,1) rate, a.dpp debit, 0 credit, ROUND(a.dpp * IFNULL(rate,1),2) debit_idr, 0 credit_idr, 'APPROVED' status, CONCAT(kata1,' ',bpbno_int,' DARI ', UPPER(supplier)) keterangan, username create_by, dateinput create_date, confirm_by approve_by, confirm_date approve_date, '-' cancel_by, '' cancel_date, CURRENT_TIMESTAMP() created_at, CURRENT_TIMESTAMP() updated_at, profit_center from bpb a LEFT JOIN rate b on b.curr = a.curr and b.tanggal = a.bpbdate INNER JOIN ap_mapping_coa_jurnal c on c.mattype = a.mattype and c.ctg_supp = a.ctg_supp and c.matclass = a.matclass and c.n_code_ctg = a.n_code_category where c.type_mapping = 'bpb_debit'),
                        
ppn as (select '' id, bpbno_int no_journal, bpbdate tgl_journal, 'AP - BPB RETURN' type_journal, '1.52.07' no_coa, 'PAJAK DIBAYAR DIMUKA PPN MASUKAN (UNBILLED)' nama_coa, '-' no_costcenter, '-' nama_costcenter, '-' reff_doc, '' reff_date, '-' buyer, '-' no_ws, a.curr, IFNULL(rate,1) rate, a.ppn debit, 0 credit, ROUND(a.ppn * IFNULL(rate,1),2) debit_idr, 0 credit_idr, 'APPROVED' status, CONCAT(kata1,' ',bpbno_int,' DARI ', UPPER(supplier)) keterangan, username create_by, dateinput create_date, confirm_by approve_by, confirm_date approve_date, '-' cancel_by, '' cancel_date, CURRENT_TIMESTAMP() created_at, CURRENT_TIMESTAMP() updated_at, profit_center from bpb a LEFT JOIN rate b on b.curr = a.curr and b.tanggal = a.bpbdate INNER JOIN ap_mapping_coa_jurnal c on c.mattype = a.mattype and c.ctg_supp = a.ctg_supp and c.matclass = a.matclass and c.n_code_ctg = a.n_code_category where c.type_mapping = 'bpb_credit'),

bpb_debit_buyer as (select '' id, bpbno_int no_journal, bpbdate tgl_journal, 'AP - BPB RETURN' type_journal, c.no_coa, c.nama_coa, IF(SUBSTR(bpbno_int,1,3) = 'WIP','DEP04SUB004','-') no_costcenter, IF(SUBSTR(bpbno_int,1,3) = 'WIP','DISTRIBUTION CENTER','-') nama_costcenter, '-' reff_doc, '' reff_date, '-' buyer, '-' no_ws, a.curr, IFNULL(rate,1) rate, a.total debit, 0 credit, ROUND(a.total * IFNULL(rate,1),2) debit_idr, 0 credit_idr, 'APPROVED' status, CONCAT(kata1,' ',bpbno_int,' DARI ', UPPER(supplier)) keterangan, username create_by, dateinput create_date, confirm_by approve_by, confirm_date approve_date, '-' cancel_by, '' cancel_date, CURRENT_TIMESTAMP() created_at, CURRENT_TIMESTAMP() updated_at, profit_center from bpb a LEFT JOIN rate b on b.curr = a.curr and b.tanggal = a.bpbdate INNER JOIN ap_mapping_coa_jurnal c on c.mattype = a.mattype and c.ctg_supp = a.ctg_supp and c.matclass = a.matclass and c.n_code_ctg = a.n_code_category where c.type_mapping = 'bpb_credit'),
...
ppn_buyer as (select '' id, bpbno_int no_journal, bpbdate tgl_journal, 'AP - BPB RETURN' type_journal, '8.07.01' no_coa, 'PENDAPATAN LAIN-LAIN' nama_coa, '-' no_costcenter, '-' nama_costcenter, '-' reff_doc, '' reff_date, '-' buyer, '-' no_ws, a.curr, IFNULL(rate,1) rate, 0 debit, a.ppn credit, 0 debit_idr, ROUND(a.ppn * IFNULL(rate,1),2) credit_idr, 'APPROVED' status, CONCAT(kata1,' ',bpbno_int,' DARI ', UPPER(supplier)) keterangan, username create_by, dateinput create_date, confirm_by approve_by, confirm_date approve_date, '-' cancel_by, '' cancel_date, CURRENT_TIMESTAMP() created_at, CURRENT_TIMESTAMP() updated_at, profit_center from bpb a LEFT JOIN rate b on b.curr = a.curr and b.tanggal = a.bpbdate INNER JOIN ap_mapping_coa_jurnal c on c.mattype = a.mattype and c.ctg_supp = a.ctg_supp and c.matclass = a.matclass and c.n_code_ctg = a.n_code_category where c.type_mapping = 'bpb_credit'),
