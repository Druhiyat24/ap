...
<!--             <button type="button" id="reset" value=" Reset " style="height: 35px; margin-top: 30px; margin-bottom: 5px;margin-right: 15px;border: 0;
    ...
    if ($tanggal_awal >= '2025-09-01') {
        ...
        echo '<a style="padding-right: 10px;" target="_blank" href="ekspor_pa_report_old.php?start_date='.$start_date.'&&end_date='.$end_date.'&&profit_center='.$h_profit_center.'&&where='.$where.'"><button type="button" class="btn btn-info " style= "margin-top: 30px;"><i class="fa fa-file-excel-o" aria-hidden="true" style="padding-right: 10px; padding-left: 5px;font-size: 1rem;color: #fff;text-shadow: 1px 1px 1px #000"> Excel</i></button></a>';
            ...
            <table id="mytable2" class="table table-striped table-bordered text-nowrap" role="grid" cellspacing="0" width="100%">
                <thead>
                    <tr class="thead-dark">
                        ...
                        <th style="text-align: center;vertical-align: middle;width: 12%;">Description</th>
...
LEFT JOIN (
    select no_kbon, no_ftr, no_po, no_bankout, tgl_kbon, SUM(credit) ded_bfr_ftr, ROUND(SUM(credit * rate), 2) ded_bfr_ftr_idr from tbl_list_journal a INNER JOIN (select a.no_kbon, no_ftr, a.no_po, no_bankout, DATE_FORMAT(a.create_date, '%Y-%m-%d') tgl_kbon from kontrabon_h a INNER JOIN kontrabon_ftr b on b.no_kbon = a.no_kbon where a.status != 'Cancel' and b.status != 'Updated' and DATE_FORMAT(a.create_date, '%Y-%m-%d') < '$start_date' and DATE_FORMAT(a.create_date, '%Y-%m-%d') > '2025-08-31' GROUP BY no_kbon, no_ftr, no_bankout) b on b.no_kbon = a.no_journal and b.no_ftr = a.reff_doc where tgl_journal < '$start_date' and (no_journal like '%SI/APR%' OR no_journal like '%PV-AP/%') and reff_doc like '%FTR/%' GROUP BY no_ftr, no_bankout) bb on bb.no_ftr = a.no_ftr and bb.no_bankout = a.no_bankout
...
LEFT JOIN (select no_kbon, no_ftr, no_po, no_bankout, tgl_kbon, SUM(credit) ded_ftr, ROUND(SUM(credit * rate), 2) ded_ftr_idr from tbl_list_journal a INNER JOIN (select a.no_kbon, no_ftr, a.no_po, no_bankout, DATE_FORMAT(a.create_date, '%Y-%m-%d') tgl_kbon from kontrabon_h a INNER JOIN kontrabon_ftr b on b.no_kbon = a.no_kbon where a.status != 'Cancel' and b.status != 'Updated' and DATE_FORMAT(a.create_date, '%Y-%m-%d') BETWEEN '$start_date' and '$end_date' and DATE_FORMAT(a.create_date, '%Y-%m-%d') > '2025-08-31' GROUP BY no_kbon, no_ftr, no_bankout) b on b.no_kbon = a.no_journal and b.no_ftr = a.reff_doc where tgl_journal BETWEEN '$start_date' and '$end_date' and (no_journal like '%SI/APR%' OR no_journal like '%PV-AP/%') and reff_doc like '%FTR/%' GROUP BY no_ftr, no_bankout) cc on cc.no_ftr = a.no_ftr and cc.no_bankout = a.no_bankout
#total Deduction GM Before
LEFT JOIN (select * from (select reff_doc, curr, sum((credit - debit)) tot_gm_bfr_ocy, if(curr != 'IDR',sum((credit * rate) - (debit * rate)),sum(credit - debit)) tot_gm_bfr from tbl_list_journal where nama_coa like '%uang muka pembelian%' and (no_journal like '%GM/%' or no_journal like '%BM/%'  or no_journal like '%PAY/%' or no_journal like '%BK/%') and (reff_doc not like '%BK%' and reff_doc not like '%KK%') and tgl_journal > '2025-08-31' and tgl_journal < '$start_date' GROUP BY reff_doc
    UNION
    select CONCAT(no_coa,reff_doc) reff_doc, curr, sum((credit - debit)) tot_gm_bfr_ocy, if(curr != 'IDR',sum((credit * rate) - (debit * rate)),sum(credit - debit)) tot_gm_bfr from tbl_list_journal where nama_coa like '%uang muka pembelian%' and (no_journal like '%GM/%' or no_journal like '%BM/%'  or no_journal like '%PAY/%') and (reff_doc like '%BK%' OR reff_doc like '%KK%') and tgl_journal > '2025-08-31' and tgl_journal < '$start_date' GROUP BY reff_doc, no_coa
    ) a where reff_doc != '-') d on d.reff_doc = a.no_ftr
#total Deduction GM 
LEFT JOIN (select * from (select reff_doc, curr, sum((credit - debit)) tot_gm_ocy, if(curr != 'IDR',sum((credit * rate) - (debit * rate)),sum(credit - debit)) tot_gm from tbl_list_journal where nama_coa like '%uang muka pembelian%' and (no_journal like '%GM/%' or no_journal like '%BM/%' or no_journal like '%PAY/%' or no_journal like '%BK/%') and (reff_doc not like '%BK%' and reff_doc not like '%KK%') and tgl_journal > '2025-08-31' and tgl_journal BETWEEN '$start_date' and '$end_date' GROUP BY reff_doc
    UNION
    select CONCAT(no_coa,reff_doc) reff_doc, curr, sum((credit - debit)) tot_gm_ocy, if(curr != 'IDR',sum((credit * rate) - (debit * rate)),sum(credit - debit)) tot_gm from tbl_list_journal where nama_coa like '%uang muka pembelian%' and (no_journal like '%GM/%' or no_journal like '%BM/%'  or no_journal like '%PAY/%') and (reff_doc like '%BK%' OR reff_doc like '%KK%') and tgl_journal > '2025-08-31' and tgl_journal BETWEEN '$start_date' and '$end_date' GROUP BY reff_doc, no_coa) a where reff_doc != '-') e on e.reff_doc = a.no_ftr) a) a order by tgl_bankout asc) a LEFT JOIN
...
#total Deduction By PO
LEFT JOIN (select no_po, kbon_date, curr, ded, IF(curr != 'IDR',round(ded * COALESCE(rate,1),2),ded) ded_idr from (select no_po, curr, dp_value ded, DATE_FORMAT(create_date, '%Y-%m-%d') kbon_date from kontrabon_h where dp_value > 0 and status != 'Cancel' and DATE_FORMAT(create_date, '%Y-%m-%d') BETWEEN '$start_date' and '$end_date' GROUP BY no_po) a left join (select tanggal, rate from masterrate where v_codecurr = 'PAJAK' GROUP BY tanggal) b on b.tanggal = a.kbon_date
    UNION
    select CONCAT(no_coa,oth_doc) reff_doc, tgl_journal, curr, sum(credit - debit) ded, sum(credit - debit) ded_idr from (select no_journal, tgl_journal, no_coa, nama_coa,curr, rate, debit, credit from tbl_list_journal where type_journal = 'Settlement' and nama_coa like '%UANG MUKA PEMBELIAN%') a INNER JOIN (select no_pci, oth_doc from (select no_pci, oth_doc from c_petty_cashin_h where reff = 'Settlement' and status != 'Cancel' GROUP BY no_pci, oth_doc
        UNION
        select no_pco, reff_doc from c_petty_cashout_h where reff = 'Settlement' and status != 'Cancel' GROUP BY no_pco, reff_doc) a) b on b.no_pci = a.no_journal where tgl_journal BETWEEN '$start_date' and '$end_date' GROUP BY no_coa, oth_doc) c on c.no_po = a.no_po
#total Deduction GM Before
LEFT JOIN (select reff_doc, curr, sum((credit - debit)) tot_gm_bfr_ocy, if(curr != 'IDR',sum((credit * rate) - (debit * rate)),sum(credit - debit)) tot_gm_bfr from tbl_list_journal where nama_coa like '%uang muka pembelian%' and (no_journal like '%GM/%' or no_journal like '%BM/%' or no_journal like '%BK/%') and (reff_doc not like '%BK%' and reff_doc not like '%KK%') and tgl_journal < '$start_date' GROUP BY reff_doc
    UNION
    select CONCAT(no_coa,reff_doc) reff_doc, curr, sum((credit - debit)) tot_gm_bfr_ocy, if(curr != 'IDR',sum((credit * rate) - (debit * rate)),sum(credit - debit)) tot_gm_bfr from tbl_list_journal where nama_coa like '%uang muka pembelian%' and (no_journal like '%GM/%' or no_journal like '%BM/%') and (reff_doc like '%BK%' OR reff_doc like '%KK%') and tgl_journal < '$start_date' GROUP BY reff_doc
    ) d on d.reff_doc = a.no_ftr
#total Deduction GM 
LEFT JOIN (select reff_doc, curr, sum((credit - debit)) tot_gm_ocy, if(curr != 'IDR',sum((credit * rate) - (debit * rate)),sum(credit - debit)) tot_gm from tbl_list_journal where nama_coa like '%uang muka pembelian%' and (no_journal like '%GM/%' or no_journal like '%BM/%' or no_journal like '%BK/%') and (reff_doc not like '%BK%' and reff_doc not like '%KK%') and tgl_journal BETWEEN '$start_date' and '$end_date' GROUP BY reff_doc
    UNION
    select CONCAT(no_coa,reff_doc) reff_doc, curr, sum((credit - debit)) tot_gm_ocy, if(curr != 'IDR',sum((credit * rate) - (debit * rate)),sum(credit - debit)) tot_gm from tbl_list_journal where nama_coa like '%uang muka pembelian%' and (no_journal like '%GM/%' or no_journal like '%BM/%') and (reff_doc like '%BK%' OR reff_doc like '%KK%') and tgl_journal BETWEEN '$start_date' and '$end_date' GROUP BY reff_doc) e on e.reff_doc = a.no_ftr) a) a order by tgl_bankout asc) a LEFT JOIN
(select no_coa, no_po, no_ftr, no_bankout, sum(total) total_adj from tbl_adjust_purchase_advance where tanggal BETWEEN '$start_date' and '$end_date' and status = 'Y' GROUP BY no_coa, no_po, no_ftr, no_bankout) b on b.no_coa = a.no_coa and b.no_po = a.no_po and b.no_ftr = a.no_ftr and a.no_bankout = b.no_bankout LEFT JOIN
(select no_coa, no_po, no_ftr, no_bankout, sum(total) total_adj_bfr from tbl_adjust_purchase_advance where tanggal < '$start_date'  and status = 'Y' GROUP BY no_coa, no_po, no_ftr, no_bankout) c on c.no_coa = a.no_coa and c.no_po = a.no_po and c.no_ftr = a.no_ftr and a.no_bankout = c.no_bankout ) a left join master_pc b on b.kode_pc = a.profit_center $where");
}
...
while($row2 = mysqli_fetch_array($sql)){
    ...
    if ($row2['saldo_awal'] == 0 && $row2['addition'] == 0 && $row2['ded'] == 0 && $row2['ded_gm'] == 0 && $row2['forex'] == 0 && $row2['saldo_akhir'] == 0) {
        ...
        <td style="text-align : right;" value = "'.$row2['ded'].'">'.number_format($dednya,2).'</td>
...
if ($coa_number == 'ALL') {
                $sql = mysqli_query($conn2,"SELECT * from (select no_coa, no_ftr, tgl_ftr, no_pi, a.no_po, tgl_po, supp, no_bankout, tgl_bankout,(saldo_awal - COALESCE(ded_bfr,0) - COALESCE(tot_gm_bfr,0)) saldo_awal, addition, COALESCE(ded,0) ded,  COALESCE(tot_gm,0) ded_gm, ((saldo_awal - COALESCE(ded_bfr,0) - COALESCE(tot_gm_bfr,0)) + addition  - (COALESCE(ded,0) + COALESCE(tot_gm,0))) saldo_akhir from (select no_ftr_cbd no_ftr, tgl_ftr_cbd tgl_ftr, no_pi, no_po, tgl_po, supp, no_journal no_bankout, tgl_journal tgl_bankout, debit saldo_awal, debit_idr saldo_awal_idr, 0 addition, 0 addition_idr, rate,a.curr,b.curr curr_bk, no_coa from (select a.no_ftr_cbd, a.tgl_ftr_cbd, a.supp, a.no_po, a.tgl_po, a.no_pi, a.total, b.no_kbon, c.no_payment, a.curr from (select no_ftr_cbd, tgl_ftr_cbd, supp, no_po, tgl_po, no_pi, curr, sum(total) total from ftr_cbd GROUP BY no_ftr_cbd) a INNER JOIN kontrabon_cbd b on b.no_cbd = a.no_ftr_cbd INNER JOIN list_payment_cbd c on c.no_kbon = b.no_kbon GROUP BY no_ftr_cbd
                    UNION
                    select a.no_ftr_dp, a.tgl_ftr_dp, a.supp, a.no_po, a.tgl_po, a.no_pi, a.total, b.no_kbon, c.no_payment, a.curr from (select no_ftr_dp, tgl_ftr_dp, supp, no_po, tgl_po, no_pi, curr, sum(total) total from ftr_dp GROUP BY no_ftr_dp) a INNER JOIN kontrabon_dp b on b.no_dp = a.no_ftr_dp INNER JOIN list_payment_dp c on c.no_kbon = b.no_kbon GROUP BY no_ftr_dp) a INNER JOIN (select no_coa, no_journal, tgl_journal, reff_doc, curr, debit,rate, debit_idr from tbl_list_journal where reff_doc like '%CBD%'
                    UNION
                    select no_coa, no_journal, tgl_journal, reff_doc, curr, debit,rate, debit_idr from tbl_list_journal where reff_doc like '%DP%') b on b.reff_doc = a.no_payment where b.tgl_journal < '$start_date'
                    UNION
                    select no_ftr_cbd no_ftr, tgl_ftr_cbd tgl_ftr, no_pi, no_po, tgl_po, supp, no_journal no_bankout, tgl_journal tgl_bankout, 0 saldo_awal, 0 saldo_awal_idr, debit addition, debit_idr addition_idr,rate,a.curr,b.curr curr_bk, no_coa from (select a.no_ftr_cbd, a.tgl_ftr_cbd, a.supp, a.no_po, a.tgl_po, a.no_pi, a.total, b.no_kbon, c.no_payment, a.curr from (select no_ftr_cbd, tgl_ftr_cbd, supp, no_po, tgl_po, no_pi, curr, sum(total) total from ftr_cbd GROUP BY no_ftr_cbd) a INNER JOIN kontrabon_cbd b on b.no_cbd = a.no_ftr_cbd INNER JOIN list_payment_cbd c on c.no_kbon = b.no_kbon GROUP BY no_ftr_cbd
                        UNION
                        select a.no_ftr_dp, a.tgl_ftr_dp, a.supp, a.no_po, a.tgl_po, a.no_pi, a.total, b.no_kbon, c.no_payment, a.curr from (select no_ftr_dp, tgl_ftr_dp, supp, no_po, tgl_po, no_pi, curr, sum(total) total from ftr_dp GROUP BY no_ftr_dp) a INNER JOIN kontrabon_dp b on b.no_dp = a.no_ftr_dp INNER JOIN list_payment_dp c on c.no_kbon = b.no_kbon GROUP BY no_ftr_dp) a INNER JOIN (select no_coa, no_journal, tgl_journal, reff_doc, curr, debit,rate,debit_idr from tbl_list_journal where reff_doc like '%CBD%'
                        UNION
                        select no_coa, no_journal, tgl_journal, reff_doc, curr, debit,rate,debit_idr from tbl_list_journal where reff_doc like '%DP%') b on b.reff_doc = a.no_payment where b.tgl_journal BETWEEN '$start_date' and '$end_date') a 
