...
(select no_coa, no_po, no_ftr, no_bankout, sum(total) total_adj_bfr from tbl_adjust_purchase_advance where tanggal < '$start_date'  and status = 'Y' GROUP BY no_coa, no_po, no_ftr, no_bankout) c on c.no_coa = a.no_coa and c.no_po = a.no_po and c.no_ftr = a.no_ftr and a.no_bankout = c.no_bankout");
}else{
    $sql = mysqli_query($conn2,"select a.coa, a.no_coa, a.no_ftr, a.tgl_ftr, a.no_pi, a.no_po, a.tgl_po, a.supp, a.no_bankout, a.tgl_bankout, a.deskripsi, a.curr, a.saldo_awal, a.addition, (a.ded + COALESCE(b.total_adj,0)) ded, a.ded_gm, a.forex, (a.saldo_akhir - COALESCE(b.total_adj,0)) saldo_akhir from (SELECT coa, no_coa, no_ftr, tgl_ftr, no_pi, no_po, tgl_po, supp, no_bankout, tgl_bankout, deskripsi, curr, saldo_awal, addition_idr addition, ded_idr ded, tot_gm ded_gm, forex, (saldo_awal + addition_idr - (ded_idr + tot_gm) - forex) saldo_akhir from (select coa, no_coa, no_ftr, tgl_ftr, no_pi, no_po, tgl_po, supp, no_bankout, tgl_bankout, deskripsi, curr, if ((saldo_awal - ded_bfr - tot_gm_bfr - tot_gfb_bfr) <= 0, 0, (saldo_awal_idr - ded_bfr_idr - tot_gm_bfr - tot_gfb_bfr)) saldo_awal, saldo_awal_idr, addition_idr, ded_bfr_idr, ded_idr, (tot_gm_bfr + tot_gfb_bfr) tot_gm_bfr, (tot_gm + tot_gfb) tot_gm, IF(curr ='USD' and (saldo_awal + addition - (ded_bfr + ded + tot_gm_bfr_ocy + tot_gm_ocy + tot_gfb_bfr_ocy + tot_gfb_ocy)) = 0 ,(if ((saldo_awal - ded_bfr - tot_gm_bfr) <= 0, 0, (saldo_awal_idr - ded_bfr_idr - tot_gm_bfr - tot_gfb_bfr)) + addition_idr - (ded_idr + tot_gm + tot_gfb)),0) forex from 
        -- tot_gfb_bfr_ocy tot_gfb_bfr tot_gfb_ocy tot_gfb
        (select coa, no_coa, no_ftr, tgl_ftr, no_pi, a.no_po, tgl_po, supp, no_bankout, tgl_bankout, deskripsi, a.curr, COALESCE(saldo_awal,0) saldo_awal, COALESCE(saldo_awal_idr,0) saldo_awal_idr, COALESCE(addition,0) addition, COALESCE(addition_idr,0) addition_idr, b.curr curr_ded_po_bfr, COALESCE(ded_bfr,0) ded_bfr, COALESCE(ded_bfr_idr,0) ded_bfr_idr, c.curr curr_ded_po, COALESCE(ded,0) ded, COALESCE(ded_idr,0) ded_idr, d.curr curr_ded_gm_bfr, COALESCE(tot_gm_bfr_ocy,0) tot_gm_bfr_ocy, COALESCE(tot_gm_bfr,0) tot_gm_bfr, e.curr curr_ded_gm, COALESCE(tot_gm_ocy,0) tot_gm_ocy, COALESCE(tot_gm,0) tot_gm, COALESCE(tot_gfb_bfr_ocy,0) tot_gfb_bfr_ocy, COALESCE(tot_gfb_bfr,0) tot_gfb_bfr, COALESCE(tot_gfb_ocy,0) tot_gfb_ocy, COALESCE(tot_gfb,0) tot_gfb from (
#Pemasukan Before
        select * from (select CASE
        WHEN reff_doc like '%CBD%' OR reff_doc like '%DP%' THEN
        no_ftr_cbd
...
#total Deduction By PO
        ...
        select no_pco, reff_doc from c_petty_cashout_h where reff = 'Settlement' and status != 'Cancel' GROUP BY no_pco, reff_doc) a) b on b.no_pci = a.no_journal where tgl_journal BETWEEN '$start_date' and '$end_date' GROUP BY no_coa, oth_doc) c on c.no_po = a.no_po
#total Deduction GM Before
        LEFT JOIN (select reff_doc, curr, sum((credit - debit)) tot_gm_bfr_ocy, if(curr = 'USD',sum((credit * rate) - (debit * rate)),sum(credit - debit)) tot_gm_bfr from tbl_list_journal where nama_coa like '%uang muka pembelian%' and no_journal like '%GM/%' and (reff_doc not like '%BK%' and reff_doc not like '%KK%') and tgl_journal < '$start_date' GROUP BY reff_doc
        UNION
        select CONCAT(no_coa,reff_doc) reff_doc, curr, sum((credit - debit)) tot_gm_bfr_ocy, if(curr = 'USD',sum((credit * rate) - (debit * rate)),sum(credit - debit)) tot_gm_bfr from tbl_list_journal where nama_coa like '%uang muka pembelian%' and (no_journal like '%GM/%' or no_journal like '%BM/%' or no_journal like '%BK/%') and (reff_doc like '%BK%' OR reff_doc like '%KK%') and tgl_journal < '$start_date' GROUP BY reff_doc
        ) d on d.reff_doc = a.no_ftr
#total Deduction GM 
        LEFT JOIN (select reff_doc, curr, sum((credit - debit)) tot_gm_ocy, if(curr = 'USD',sum((credit * rate) - (debit * rate)),sum(credit - debit)) tot_gm from tbl_list_journal where nama_coa like '%uang muka pembelian%' and (no_journal like '%GM/%' or no_journal like '%BM/%' or no_journal like '%BK/%') and (reff_doc not like '%BK%' and reff_doc not like '%KK%') and tgl_journal BETWEEN '$start_date' and '$end_date' GROUP BY reff_doc
        UNION
        select CONCAT(no_coa,reff_doc) reff_doc, curr, sum((credit - debit)) tot_gm_ocy, if(curr = 'USD',sum((credit * rate) - (debit * rate)),sum(credit - debit)) tot_gm from tbl_list_journal where nama_coa like '%uang muka pembelian%' and (no_journal like '%GM/%' or no_journal like '%BM/%') and (reff_doc like '%BK%' OR reff_doc like '%KK%') and tgl_journal BETWEEN '$start_date' and '$end_date' GROUP BY reff_doc) e on e.reff_doc = a.no_ftr
        #total Deduction GM FTR-BK Before
        LEFT JOIN (select SUBSTRING_INDEX(reff_doc, '-', 1) AS reff_doc1, SUBSTRING_INDEX(reff_doc, '-', -1) AS reff_doc2, curr, sum((credit - debit)) tot_gfb_bfr_ocy, if(curr = 'USD',sum((credit * rate) - (debit * rate)),sum(credit - debit)) tot_gfb_bfr from tbl_list_journal where nama_coa like '%uang muka pembelian%' and (no_journal like '%GM/%') and (reff_doc like '%BK%' and reff_doc like '%FTR%') and tgl_journal < '$start_date' GROUP BY reff_doc) g on g.reff_doc1 = a.no_ftr and g.reff_doc2 = a.no_bankout
#total Deduction GM FTR-BK 
        LEFT JOIN (select SUBSTRING_INDEX(reff_doc, '-', 1) AS reff_doc1, SUBSTRING_INDEX(reff_doc, '-', -1) AS reff_doc2, curr, sum((credit - debit)) tot_gfb_ocy, if(curr = 'USD',sum((credit * rate) - (debit * rate)),sum(credit - debit)) tot_gfb from tbl_list_journal where nama_coa like '%uang muka pembelian%' and (no_journal like '%GM/%') and (reff_doc like '%BK%' and reff_doc like '%FTR%') and tgl_journal BETWEEN '$start_date' and '$end_date' GROUP BY reff_doc) f on f.reff_doc1 = a.no_ftr and f.reff_doc2 = a.no_bankout) a) a where no_coa = '$coa_number' order by tgl_bankout asc) a LEFT JOIN
