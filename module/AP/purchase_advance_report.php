...
LEFT JOIN (select * from (select reff_doc, curr, sum((credit - debit)) tot_gm_bfr_ocy, if(curr = 'USD',sum((credit * rate) - (debit * rate)),sum(credit - debit)) tot_gm_bfr from tbl_list_journal where nama_coa like '%uang muka pembelian%' and (no_journal like '%GM/%' or no_journal like '%BM/%'  or no_journal like '%PAY/%' or no_journal like '%BK/%') and (reff_doc not like '%BK%' and reff_doc not like '%KK%') and tgl_journal > '2025-08-31' and tgl_journal < '$start_date' GROUP BY reff_doc
    UNION
    select CONCAT(no_coa,reff_doc) reff_doc, curr, sum((credit - debit)) tot_gm_bfr_ocy, if(curr = 'USD',sum((credit * rate) - (debit * rate)),sum(credit - debit)) tot_gm_bfr from tbl_list_journal where nama_coa like '%uang muka pembelian%' and (no_journal like '%GM/%' or no_journal like '%BM/%'  or no_journal like '%PAY/%') and (reff_doc like '%BK%' OR reff_doc like '%KK%') and tgl_journal > '2025-08-31' and tgl_journal < '$start_date' GROUP BY reff_doc
    ) a where reff_doc != '-') d on d.reff_doc = a.no_ftr
#total Deduction GM 
LEFT JOIN (select * from (select reff_doc, curr, sum((credit - debit)) tot_gm_ocy, if(curr = 'USD',sum((credit * rate) - (debit * rate)),sum(credit - debit)) tot_gm from tbl_list_journal where nama_coa like '%uang muka pembelian%' and (no_journal like '%GM/%' or no_journal like '%BM/%' or no_journal like '%PAY/%' or no_journal like '%BK/%') and (reff_doc not like '%BK%' and reff_doc not like '%KK%') and tgl_journal > '2025-08-31' and tgl_journal BETWEEN '$start_date' and '$end_date' GROUP BY reff_doc
    UNION
    select CONCAT(no_coa,reff_doc) reff_doc, curr, sum((credit - debit)) tot_gm_ocy, if(curr = 'USD',sum((credit * rate) - (debit * rate)),sum(credit - debit)) tot_gm from tbl_list_journal where nama_coa like '%uang muka pembelian%' and (no_journal like '%GM/%' or no_journal like '%BM/%'  or no_journal like '%PAY/%') and (reff_doc like '%BK%' OR reff_doc like '%KK%') and tgl_journal > '2025-08-31' and tgl_journal BETWEEN '$start_date' and '$end_date' GROUP BY reff_doc) a where reff_doc != '-') e on e.reff_doc = a.no_ftr) a) a order by tgl_bankout asc) a LEFT JOIN
