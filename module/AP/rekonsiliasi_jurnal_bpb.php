...
<div class="container-fluid mt-4 p-4">
  ...
  <div class="card shadow border-0">
    ...
    <h5 class="mb-0"><i class="fas fa-file-contract"></i> REKONSILIASI JOURNAL - BPB</h5>
</div>
...
<script type="text/javascript">
  ...
  let datatable = $("#table-data").DataTable({
    ...
    fixedColumns: {
      ...
      ajax: {
        url: 'ajx_rekonsiliasi_jurnal_bpb.php',
        type: 'POST',
        data: function (d) {
      ...
      columnDefs: [
        ...
        let total_jurnal = parseFloat(data.total_jurnal) || 0;

        let selisih = Math.abs(total - total_jurnal);

        // 🟢 HIJAU (Balance)
        if (total !== 0 && total_jurnal !== 0 && selisih < 0.01) {
            $(row).css('background-color', '#d4edda');
        }

        // 🟡 KUNING (Ada selisih)
        else if (total !== 0 && total_jurnal !== 0 && selisih >= 0.01) {
            $(row).css('background-color', '#fff3cd');
        }

        // 🔴 MERAH (Salah satu nol)
        else if (
            (total !== 0 && total_jurnal === 0) ||
            (total === 0 && total_jurnal !== 0)
        ) {
    ...
    $(api.column(0).footer()).html('<b>TOTAL</b>').css({
      'text-align': 'center',
      'font-weight': 'bold'
    });
}
...
document.getElementById('btnExportData').addEventListener('click', function(e) {
  ...
  this.href = `ekspor_rekonsiliasi_jurnal_bpb.php?status=${status}&start_date=${start_date}&end_date=${end_date}`;
});
...
<script type="text/javascript">
    document.getElementById('btncreate').onclick = function () {
        location.href = "rekonsiliasi_jurnal_bpb.php";
    };
</script>
