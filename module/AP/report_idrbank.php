...
<body>
    ...
    $sqlswl = mysqli_query($conn1,"select amount from b_saldoawal_bank where account = '$accountid'");
     ...
     $saldoakhir = isset($rows6['saldo_akhir']) ? $rows6['saldo_akhir'] : $saldoswal;
...
FROM
   (select id, transaksi_date as date, no_doc as doc_num,deskripsi,debit,credit,curr from b_reportbank where akun = '$accountid' and transaksi_date between '$start_date' and '$end_date' and status != 'Cancel') AS q1 JOIN
