...
// Verify PL is still Draft
$check = mysqli_query($conn2, "SELECT status FROM pv_payment_list_h WHERE pl_number = '$pl_esc' LIMIT 1");
$row = mysqli_fetch_assoc($check);
if (!$row || $row['status'] !== 'Draft') {
    echo 'Error: Payment List bukan Draft, tidak bisa diedit';
    exit;
}
...
try {
    ...
    foreach ($removes as $rm) {
        ...
        if (!mysqli_query($conn2, "DELETE FROM pv_payment_list_det WHERE pl_number = '$pl_esc' AND no_kbon = '$rm_kbon_esc'")) {
    ...
    // After commit: update status_pl in source tables
    foreach ($removes as $rm) {
        if (!empty($rm['no_kbon'])) {
            updateStatusPl($conn2, $rm['type_pv'] ?? '', $rm['no_kbon'], null);
        }
    }
    foreach ($adds as $row_data) {
        if (!empty($row_data['no_kbon'])) {
            updateStatusPl($conn2, $row_data['type_pv'] ?? '', $row_data['no_kbon'], 'Draft');
        }
    }
