...
try {
    ...
    $sqlNum = mysqli_query($conn2,
        "SELECT COALESCE(MAX(CAST(RIGHT(pl_number, 5) AS UNSIGNED)), 0) + 1 AS next_seq
         FROM pv_payment_voucher_list_h
    ...
    // Insert header
    $qh = "INSERT INTO pv_payment_voucher_list_h (pl_number, pl_date, deskripsi, status, created_by, created_date)
            VALUES ('$pl_esc', '$pl_date', '$deskripsi', 'Draft', '$create_user', '$create_date')";
    ...
    // Update status_pvl untuk setiap PV setelah commit
    foreach ($rows as $row) {
        updateStatusPvl($conn2, $row['type_pv'], $row['no_kbon'], 'Draft');
    }

    // Return nomor aktual yang dipakai agar client bisa tampilkan di Swal sukses
    echo 'OK:' . $pl_number;
