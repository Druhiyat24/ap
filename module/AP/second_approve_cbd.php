...
$sqlCheck = mysqli_query($conn2, "select status from kontrabon_h_cbd where no_kbon = '$no_kbon_esc' limit 1");
$rowCheck = mysqli_fetch_assoc($sqlCheck);

if (!$rowCheck) {
    echo 'Error: Payment Voucher CBD tidak ditemukan';
    exit;
}
...
$update1 = mysqli_query($conn2, "update kontrabon_cbd set status = 'SECOND APPROVED', confirm_user = '$approve_user_esc', confirm_date = '$second_approve_date' where no_kbon = '$no_kbon_esc'");
if (!$update1) {
    echo 'Error: ' . mysqli_error($conn2);
    exit;
}

$update2 = mysqli_query($conn2, "update kontrabon_h_cbd set status = 'SECOND APPROVED', confirm_user = '$approve_user_esc', confirm_date = '$second_approve_date', second_approve_user = '$approve_user_esc', second_approve_date = '$second_approve_date' where no_kbon = '$no_kbon_esc'");
if (!$update2) {
    echo 'Error: ' . mysqli_error($conn2);
    exit;
}
