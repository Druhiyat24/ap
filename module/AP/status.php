...
if ($filter == 'tgl_bpb') {
    $filterr = "BPB Date";
            if ($nama_supp == 'ALL') {
                $sql = "select nama_supp, bpbno_int no_bpb, bpbdate tgl_bpb, a.confirm_date approve_bpb, verif_date, c.no_kbon, c.tgl_kbon, c.confirm_date approve_kbon, d.no_payment, d.tgl_payment, d.confirm_date approve_lp, d.closed_date close_lp, e.no_bankout no_pelunasan, e.bankout_date tgl_pelunasan, no_sj, no_ws from (select supplier nama_supp, bpbno_int, bpbdate, confirm_date, invno no_sj, COALESCE(GROUP_CONCAT(DISTINCT tmpjo.kpno),'-') no_ws from bpb a INNER JOIN mastersupplier b on b.id_supplier = a.id_supplier left join (select id_jo,kpno,styleno from act_costing ac inner join so on ac.id=so.id_cost inner join jo_det jod on so.id=jod.id_so group by id_jo) tmpjo on tmpjo.id_jo=a.id_jo where bpbdate BETWEEN '$start_date' AND '$end_date' and confirm = 'Y' and cancel = 'N' GROUP BY bpbno_int) a LEFT JOIN
