...
if ($filter == 'tgl_bpb') {
    $filterr = "BPB Date";
            if ($nama_supp == 'ALL') {
                $sql = "select nama_supp, bpbno_int no_bpb, bpbdate tgl_bpb, a.confirm_date approve_bpb, verif_date, c.no_kbon, c.tgl_kbon, c.confirm_date approve_kbon, d.no_payment, d.tgl_payment, d.confirm_date approve_lp, d.closed_date close_lp, e.no_bankout no_pelunasan, e.bankout_date tgl_pelunasan from (select supplier nama_supp, bpbno_int, bpbdate, confirm_date from bpb a INNER JOIN mastersupplier b on b.id_supplier = a.id_supplier where bpbdate BETWEEN '$start_date' AND '$end_date' and confirm = 'Y' and cancel = 'N' GROUP BY bpbno_int) a LEFT JOIN
         ...
         if ($nama_supp == 'ALL') {
             $sql = "select nama_supp, bpbno_int no_bpb, bpbdate tgl_bpb, a.confirm_date approve_bpb, verif_date, c.no_kbon, c.tgl_kbon, c.confirm_date approve_kbon, d.no_payment, d.tgl_payment, d.confirm_date approve_lp, d.closed_date close_lp, e.no_bankout no_pelunasan, e.bankout_date tgl_pelunasan from (select supplier nama_supp, bpbno_int, bpbdate, confirm_date from bpb a INNER JOIN mastersupplier b on b.id_supplier = a.id_supplier where confirm = 'Y' and cancel = 'N' GROUP BY bpbno_int) a LEFT JOIN
