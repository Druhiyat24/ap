...
if ($doc_num) {
	...
	if ($total_nag != 0) {
		...
		VALUES 
		('$doc_num', '$date', '$type_journal', '$no_coa1', '$nama_coa1', '-', '-', '-', '', '-', '-', '$curr', '$rate', '0', '$total_nag', '0', '$total_nag_idr', 'Draft', '$deskripsi', '$create_user', '$create_date', '', '', '', '', 'NAG')";
	...
	if ($total_nak != 0) {
		...
		VALUES 
		('$doc_num', '$date', '$type_journal', '$no_coa1', '$nama_coa1', '-', '-', '-', '', '-', '-', '$curr', '$rate', '0', '$total_nak', '0', '$total_nak_idr', 'Draft', '$deskripsi', '$create_user', '$create_date', '', '', '', '', 'NAK')";

		$execute_nak = mysqli_query($conn2,$query_nak);
	}


	$query_bk = mysqli_query($conn2,"insert into b_bankout_none_cancel (select * from b_bankout_none where no_bankout='$doc_num')");
	$query_delete = mysqli_query($conn2,"Delete from b_bankout_none where no_bankout='$doc_num'");
	...
	foreach ($details as $d) {
		...
		$query_det = "INSERT INTO b_bankout_none (no_bankout,tgl_bankout,reff_doc,no_coa,no_costcntr,buyer,no_ws,curr,debit,credit,deskripsi, profit_center) 
		VALUES 
		('$doc_num', '$date', 'None', '$coa', '$cost_ctr', '$buyer', '$ws', '$currency', '$debit', '$credit', '$ket', '$prof_ctr')";
		...
		VALUES 
		('$doc_num', '$date', '$type_journal', '$coa', '$nama_coa', '$cost_ctr', '$nama_cc', '-', '', '$buyer', '$ws', '$currency', '$rate', '$debit', '$credit', '$t_debit', '$t_credit', 'Draft', '$ket', '$create_user', '$create_date', '', '', '', '', '$prof_ctr')";
