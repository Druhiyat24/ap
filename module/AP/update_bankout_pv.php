...
if ($doc_num) {
	...
	$jurnal_tambahan = mysqli_query($conn2,"INSERT into tbl_list_journal select '', no_journal, '$date' tgl_journal, '$type_journal' type_journal, no_coa, nama_coa, no_costcenter, nama_costcenter, reff_doc, reff_date, buyer, no_ws, curr, rate, debit, credit, debit_idr, credit_idr, 'Draft' status, keterangan, '$create_user' create_by, '$create_date' create_date, '' approve_by, CURRENT_TIMESTAMP() approve_date, cancel_by, cancel_date, created_at, updated_at, profit_center from tbl_list_journal where no_journal = '$doc_num' and reff_doc like '%PV/%' and type_journal not like '%Reverse%' and status = 'Draft'");	
