...
if ($doc_num) {
	...
	$sql = "UPDATE c_petty_cashin_h SET tgl_pci = '$date', oth_doc = '$reff_doc', profit_center = '$profit_center', amount = '$amount', deskripsi = '$deskripsi' WHERE no_pci = '$doc_num'";
	...
	$sqlpco = mysqli_query($conn1,"select no_pco, tgl_pco from c_petty_cashout_h where no_pco = '$reff_doc' GROUP BY no_pco");
	...
	foreach ($details as $d) {
		...
		VALUES 
		('$doc_num', '$date', '$type_journal', '$coa', '$nama_coa', '$cost_ctr', '$nama_cc', '$reff_doc', '$txttgl_pco', '$buyer', '$ws', '$currency', '1', '$debit', '$credit', '$debit', '$credit', 'Draft', '$ket', '$create_user', '$create_date', '', '', '', '', '$prof_ctr')";
