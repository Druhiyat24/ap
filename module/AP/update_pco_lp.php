...
if ($doc_num) {
	...
	$query_bk = mysqli_query($conn2,"insert into c_petty_cashout_adj_det_cancel (select * from c_petty_cashout_adj_det where no_pco='$doc_num')");
	$query_delete = mysqli_query($conn2,"Delete from c_petty_cashout_adj_det where no_pco='$doc_num'");
	...
	foreach ($details as $d) {
		...
		$sqlcoa2 = mysqli_query($conn1,"select nama_coa from mastercoa_v2 where no_coa = '$coa'");
		...
		$query_det = "INSERT INTO c_petty_cashout_adj_det (no_pco,id_coa,no_cc,reff_doc,reff_date,deskripsi,t_debit,t_credit, profit_center)
		VALUES 
		...
		VALUES 
		('$doc_num', '$date', '$type_journal', '$coa', '$nama_coa2', '$cost_ctr', '$nama_cc', '$refferensi', '$tgl_refferensi', '-', '-', 'IDR', '1', '$debit', '$credit', '$debit', '$credit', 'Draft', '$ket', '$create_user', '$create_date', '', '', '', '', '$prof_ctr')";
