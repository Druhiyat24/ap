...
if ($doc_num) {
	...
	$sql = "UPDATE c_petty_cashout_h SET tgl_pco = '$date', nama_supp = '$customer', profit_center = '$profit_center', amount = '$amount', deskripsi = '$deskripsi' WHERE no_pco = '$doc_num'";

	$Update_report = mysqli_query($conn2,"UPDATE c_report_pettycash set transaksi_date = '$date', credit = '$amount', deskripsi = '$deskripsi' where no_doc = '$doc_num'");
	...
	if ($total_nag != 0) {
		...
		VALUES 
		('$doc_num', '$date', '$type_journal', '$akun', '$nama_coa', '-', '-', '-', '', '-', '-', 'IDR', '1', '0', '$total_nag', '0', '$total_nag', 'Draft', '$deskripsi', '$create_user', '$create_date', '', '', '', '', 'NAG')";
	...
	if ($total_nak != 0) {
		$queryss2 = "INSERT INTO tbl_list_journal (no_journal, tgl_journal, type_journal, no_coa, nama_coa, no_costcenter, nama_costcenter, reff_doc, reff_date, buyer, no_ws, curr, rate, debit, credit, debit_idr, credit_idr, status, keterangan, create_by, create_date, approve_by, approve_date, cancel_by, cancel_date, profit_center) 
		VALUES 
		('$doc_num', '$date', '$type_journal', '$akun', '$nama_coa', '-', '-', '-', '', '-', '-', 'IDR', '1', '0', '$total_nak', '0', '$total_nak', 'Draft', '$deskripsi', '$create_user', '$create_date', '', '', '', '', 'NAK')";

		$executess2 = mysqli_query($conn2,$queryss2);
	}

	$query_bk = mysqli_query($conn2,"insert into c_petty_cashout_none_cancel (select * from c_petty_cashout_none where no_pco='$doc_num')");
	$query_delete = mysqli_query($conn2,"Delete from c_petty_cashout_none where no_pco='$doc_num'");
	...
	foreach ($details as $d) {
		...
		VALUES 
		('$doc_num', '$date', 'None', '$coa', '$prof_ctr', '$cost_ctr', '$buyer', '$ws', '$currency', '$debit', '$credit', '$ket')";
		...
		VALUES 
		('$doc_num', '$date', '$type_journal', '$coa', '$nama_coa', '$cost_ctr', '$nama_cc', '', '', '$buyer', '$ws', '$currency', '1', '$debit', '$credit', '$debit', '$credit', 'Draft', '$ket', '$create_user', '$create_date', '', '', '', '', '$prof_ctr')";
