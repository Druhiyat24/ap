<?php include '../header.php' ?>
    ...
    <!-- MAIN -->
    <div class="col p-4">
        <h2 class="text-center">LIST VERIFIKASI BPB KNITTING</h2>
<div class="box">
    <div class="box header">
<form id="form-data" action="verifikasibpb_knitting.php" method="post">        
        <div class="form-row">
            <div class="col-md-6">
...
//    echo $kondisi;
    ...
    elseif ($nama_supp == 'ALL' and $status == 'ALL' and !empty($start_date) and !empty($end_date)) {
    $sql = mysqli_query($conn2,"select no_bpb, pono, tgl_bpb, supplier, create_date, confirm_date, create_user, confirm1, confirm2, status, top, curr, tgl_po, SUM(qty * price) as total,upt_dok_inv,IF(upt_no_faktur = '',upt_no_faktur2,upt_no_faktur) upt_no_faktur from bpb_new where profit_center = 'NAK' and tgl_bpb between '$start_date' and '$end_date' group by no_bpb, create_date");
    }    
    elseif ($nama_supp != 'ALL' and $status == 'ALL' and $start_date == '1970-01-01' and $end_date == '1970-01-01') {
    $sql = mysqli_query($conn2,"select no_bpb, pono, tgl_bpb, supplier, create_date, confirm_date, create_user, confirm1, confirm2, status, top, curr, tgl_po, SUM(qty * price) as total,upt_dok_inv,IF(upt_no_faktur = '',upt_no_faktur2,upt_no_faktur) upt_no_faktur from bpb_new where profit_center = 'NAK' and supplier = '$nama_supp' group by no_bpb, create_date");
    }
    elseif ($nama_supp != 'ALL' and $status == 'ALL' and !empty($start_date) and !empty($end_date)) {
    $sql = mysqli_query($conn2,"select no_bpb, pono, tgl_bpb, supplier, create_date, confirm_date, create_user, confirm1, confirm2, status, top, curr, tgl_po, SUM(qty * price) as total,upt_dok_inv,IF(upt_no_faktur = '',upt_no_faktur2,upt_no_faktur) upt_no_faktur from bpb_new where profit_center = 'NAK' and supplier = '$nama_supp' and tgl_bpb between '$start_date' and '$end_date' group by no_bpb, create_date");
    }    
    ...
    elseif ($nama_supp == 'ALL' and $status != 'ALL' and !empty($start_date) and !empty($end_date)) {
    $sql = mysqli_query($conn2,"select no_bpb, pono, tgl_bpb, supplier, create_date, confirm_date, create_user, confirm1, confirm2, status, top, curr, tgl_po, SUM(qty * price) as total,upt_dok_inv,IF(upt_no_faktur = '',upt_no_faktur2,upt_no_faktur) upt_no_faktur from bpb_new where profit_center = 'NAK' and status = '$status' and tgl_bpb between '$start_date' and '$end_date' group by no_bpb, create_date");
    }
    elseif ($nama_supp != 'ALL' and $status != 'ALL' and $start_date == '1970-01-01' and $end_date == '1970-01-01') {
    ...
    elseif ($nama_supp == 'ALL' and $status == 'ALL' and !empty($start_date) and !empty($end_date)) {
    $sql = mysqli_query($conn2,"select no_bpb, pono, tgl_bpb, supplier, create_date, confirm_date, create_user, confirm1, confirm2, status, top, curr, tgl_po, SUM(qty * price) as total,upt_dok_inv,IF(upt_no_faktur = '',upt_no_faktur2,upt_no_faktur) upt_no_faktur from bpb_new where profit_center = 'NAK' and create_date between '$start_date' and '$end_date' group by no_bpb, create_date");
    }    
    elseif ($nama_supp != 'ALL' and $status == 'ALL' and $start_date == '1970-01-01' and $end_date == '1970-01-01') {
    $sql = mysqli_query($conn2,"select no_bpb, pono, tgl_bpb, supplier, create_date, confirm_date, create_user, confirm1, confirm2, status, top, curr, tgl_po, SUM(qty * price) as total,upt_dok_inv,IF(upt_no_faktur = '',upt_no_faktur2,upt_no_faktur) upt_no_faktur from bpb_new where profit_center = 'NAK' and supplier = '$nama_supp' group by no_bpb, create_date");
    }
    ...
    elseif ($nama_supp == 'ALL' and $status != 'ALL' and $start_date == '1970-01-01' and $end_date == '1970-01-01') {
    $sql = mysqli_query($conn2,"select no_bpb, pono, tgl_bpb, supplier, create_date, confirm_date, create_user, confirm1, confirm2, status, top, curr, tgl_po, SUM(qty * price) as total,upt_dok_inv,IF(upt_no_faktur = '',upt_no_faktur2,upt_no_faktur) upt_no_faktur from bpb_new where profit_center = 'NAK' and status = '$status' group by no_bpb, create_date");
    }
    elseif ($nama_supp == 'ALL' and $status != 'ALL' and !empty($start_date) and !empty($end_date)) {
    $sql = mysqli_query($conn2,"select no_bpb, pono, tgl_bpb, supplier, create_date, confirm_date, create_user, confirm1, confirm2, status, top, curr, tgl_po, SUM(qty * price) as total,upt_dok_inv,IF(upt_no_faktur = '',upt_no_faktur2,upt_no_faktur) upt_no_faktur from bpb_new where profit_center = 'NAK' and status = '$status' and create_date between '$start_date' and '$end_date' group by no_bpb, create_date");
    }
    elseif ($nama_supp != 'ALL' and $status != 'ALL' and $start_date == '1970-01-01' and $end_date == '1970-01-01') {
    $sql = mysqli_query($conn2,"select no_bpb, pono, tgl_bpb, supplier, create_date, confirm_date, create_user, confirm1, confirm2, status, top, curr, tgl_po, SUM(qty * price) as total,upt_dok_inv,IF(upt_no_faktur = '',upt_no_faktur2,upt_no_faktur) upt_no_faktur from bpb_new where profit_center = 'NAK' and supplier = '$nama_supp' and status = '$status' group by no_bpb, create_date");
    }        
    ...
    elseif ($nama_supp == 'ALL' and $status == 'ALL' and !empty($start_date) and !empty($end_date)) {
    $sql = mysqli_query($conn2,"select no_bpb, pono, tgl_bpb, supplier, create_date, confirm_date, create_user, confirm1, confirm2, status, top, curr, tgl_po, SUM(qty * price) as total,upt_dok_inv,IF(upt_no_faktur = '',upt_no_faktur2,upt_no_faktur) upt_no_faktur from bpb_new where profit_center = 'NAK' and confirm_date between '$start_date' and '$end_date' group by no_bpb, create_date");
    }    
    elseif ($nama_supp != 'ALL' and $status == 'ALL' and $start_date == '1970-01-01' and $end_date == '1970-01-01') {
    $sql = mysqli_query($conn2,"select no_bpb, pono, tgl_bpb, supplier, create_date, confirm_date, create_user, confirm1, confirm2, status, top, curr, tgl_po, SUM(qty * price) as total,upt_dok_inv,IF(upt_no_faktur = '',upt_no_faktur2,upt_no_faktur) upt_no_faktur from bpb_new where profit_center = 'NAK' and supplier = '$nama_supp' group by no_bpb, create_date");
    }
    ...
    elseif ($nama_supp == 'ALL' and $status != 'ALL' and !empty($start_date) and !empty($end_date)) {
    $sql = mysqli_query($conn2,"select no_bpb, pono, tgl_bpb, supplier, create_date, confirm_date, create_user, confirm1, confirm2, status, top, curr, tgl_po, SUM(qty * price) as total,upt_dok_inv,IF(upt_no_faktur = '',upt_no_faktur2,upt_no_faktur) upt_no_faktur from bpb_new where profit_center = 'NAK' and status = '$status' and confirm_date between '$start_date' and '$end_date' group by no_bpb, create_date");
    }
    elseif ($nama_supp != 'ALL' and $status != 'ALL' and $start_date == '1970-01-01' and $end_date == '1970-01-01') {
    $sql = mysqli_query($conn2,"select no_bpb, pono, tgl_bpb, supplier, create_date, confirm_date, create_user, confirm1, confirm2, status, top, curr, tgl_po, SUM(qty * price) as total,upt_dok_inv,IF(upt_no_faktur = '',upt_no_faktur2,upt_no_faktur) upt_no_faktur from bpb_new where profit_center = 'NAK' and supplier = '$nama_supp' and status = '$status' group by no_bpb, create_date");
    }        
...
<script type="text/javascript">
    ...
    location.href = "formverifikasibpb_knitting.php";
};
</script>

<script type="text/javascript">
    document.getElementById('reset').onclick = function () {
    location.href = "verifikasibpb_knitting.php";
};
</script>
