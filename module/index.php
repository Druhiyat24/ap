    ...
    var options = {
      series: [{
          ...
          data: [<?php 
              ...
              $sql1 = mysqli_query($conn2,"select CONCAT(COALESCE(saldo_jan,0),',',COALESCE(saldo_feb,0),',',COALESCE(saldo_mar,0),',',COALESCE(saldo_apr,0),',',COALESCE(saldo_may,0),',',COALESCE(saldo_jun,0),',',COALESCE(saldo_jul,0),',',COALESCE(saldo_aug,0),',',COALESCE(saldo_sep,0),',',COALESCE(saldo_oct,0),',',COALESCE(saldo_nov,0),',',COALESCE(saldo_dec,0)) data from (select no_coa,nama_coa,round(sum(saldo_jan /1000000),2) saldo_jan,round(sum(saldo_feb /1000000),2) saldo_feb,round(sum(saldo_mar /1000000),2) saldo_mar,round(sum(saldo_apr /1000000),2) saldo_apr,round(sum(saldo_may /1000000),2) saldo_may,round(sum(saldo_jun /1000000),2) saldo_jun,round(sum(saldo_jul /1000000),2) saldo_jul,round(sum(saldo_aug /1000000),2) saldo_aug,round(sum(saldo_sep /1000000),2) saldo_sep,round(sum(saldo_oct /1000000),2) saldo_oct,round(sum(saldo_nov /1000000),2) saldo_nov,round(sum(saldo_dec /1000000),2) saldo_dec from b_trial_balance_$tahun where no_coa IN ('1.01.01','1.01.02','1.01.03')) a");
              $row1 = mysqli_fetch_array($sql1);
              $data_bar1 = isset($row1['data']) ? $row1['data'] :0;
              echo $data_bar1;
